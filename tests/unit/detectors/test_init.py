...
try:
    ...
    def create_detector(detector_name, params=None):
        ...
        try:
            ...
            raise DetectorError(
                f"Failed to instantiate detector class {detector_name}: {e}"
            ) from e
...
def test_register_detector_duplicate_name():
    ...
    with pytest.raises(
        ValueError, match="Detector name 'DuplicateName' already registered"
    ):
...
def test_load_detector_from_file_success(mock_importlib, mock_sys_path):
    ...
    assert loaded_classes == [
        LoadedDetector
    ]  # Function should return the loaded class(es)
...
def test_create_detector_success():
    ...
    with patch(
        "src.detectors.get_detector_class", return_value=mock_class
    ) as mock_get_class:
...
def test_create_detector_init_error():
    ...
    with patch(
        ...
        with pytest.raises(
            DetectorError, match=f"Failed to instantiate detector class {detector_name}"
        ) as excinfo:
...
def test_create_detector_class_not_found():
    ...
    with patch(
        "src.detectors.get_detector_class", side_effect=DetectorError(error_msg)
    ) as mock_get_class:
