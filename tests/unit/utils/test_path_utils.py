...
def test_is_safe_path_component(component, expected, caplog):
    ...
    if not expected and component not in ["", "."]:
        ...
        assert len(caplog.records) == 0 # Should not log for safe components or early returns
...
def test_validate_path_within_allowed_success(allowed_dirs_setup):
    ...
    with patch('src.utils.path_utils._project_root', None):
        ...
        result = validate_path_within_allowed_dirs(str(safe_file), [allowed_dir1, allowed_dir3], allow_absolute=True)
        assert isinstance(result, Path)
        assert result.resolve() == safe_file.resolve()
        
        # ディレクトリが存在する場合のテスト
        result = validate_path_within_allowed_dirs(str(sub_path), [allowed_dir1, allowed_dir3], allow_absolute=True)
        assert isinstance(result, Path)
...
def test_validate_path_outside_allowed_failure(allowed_dirs_setup):
    ...
    with patch('src.utils.path_utils._project_root', None):
        ...
        with pytest.raises((ValueError, FileError)):
            validate_path_within_allowed_dirs(str(unsafe_path), [allowed_dir1, allowed_dir3])
...
def test_validate_path_parent_traversal_resolves_inside(allowed_dirs_setup):
    ...
    with patch('src.utils.path_utils._project_root', None):
        ...
        result = validate_path_within_allowed_dirs(str(tricky_path), [allowed_dir1, allowed_dir3], allow_absolute=True)
...
def test_validate_path_non_existent(allowed_dirs_setup):
    ...
    with patch('src.utils.path_utils._project_root', None):
        ...
        try:
            ...
            result = validate_path_within_allowed_dirs(str(non_existent_path), [allowed_dir1, allowed_dir3], allow_absolute=True)
...
def test_validate_path_is_file_check(allowed_dirs_setup):
    ...
    with patch('src.utils.path_utils.Path.is_absolute', return_value=False):
        ...
        with pytest.raises(FileError, match=f"指定されたパスはディレクトリではありません: {test_file.resolve()}"):
            validate_path_within_allowed_dirs(
                test_file, [allowed_dir1], check_is_file=False, allow_absolute=True
            )
        
        # ディレクトリパスとcheck_is_file=Trueを指定した場合はエラー
        with pytest.raises(FileError, match=f"指定されたパスはファイルではありません: {test_dir.resolve()}"):
            validate_path_within_allowed_dirs(
    ...
    with monkeypatch.context() as m:
        ...
        with patch('src.utils.path_utils._workspace_dir', None):
            with patch('src.utils.path_utils.get_project_root', return_value=project_root):
                ...
                with patch('src.utils.path_utils.ensure_dir', side_effect=FileError("ディレクトリ作成エラー")):
                    # ConfigErrorが発生することを確認
                    with pytest.raises(ConfigError) as exc_info:
                        get_workspace_dir()
                    # 実際の実装に合わせてエラーメッセージを確認
                    assert "ワークスペースディレクトリの設定またはアクセスに問題があります" in str(exc_info.value)
...
def test_ensure_dir_writable_check_failure(monkeypatch, tmp_path):
    ...
    with patch('tempfile.NamedTemporaryFile', side_effect=OSError("書き込み権限がありません")):
        ...
        assert ("ディレクトリ確認中に予期せぬエラー" in error_msg or 
                "ディレクトリに書き込み権限がありません" in error_msg)
...
def test_get_detector_path_original(detector_test_setup, monkeypatch):
    ...
    monkeypatch.setattr("src.utils.path_utils.get_detectors_src_dir", mock_get_detectors_src_dir)
    monkeypatch.setattr("src.utils.path_utils.get_improved_versions_dir", mock_get_improved_versions_dir)
...
def dataset_test_setup(tmp_path):
    ...
    config = {
        "paths": {
            "datasets_base": str(datasets_dir)
        },
        "datasets": {
            "test_dataset": {
...
def test_get_dataset_paths_basic(dataset_test_setup, monkeypatch):
    ...
    # テスト実行
    audio_dir, label_dir, file_pairs = get_dataset_paths(setup["config"], "test_dataset")
...
def test_get_dataset_paths_with_filelist(dataset_test_setup, monkeypatch):
    ...
    # テスト実行
    audio_dir, label_dir, file_pairs = get_dataset_paths(setup["config"], "test_dataset_with_filelist")
...
def test_get_dataset_paths_missing_section(monkeypatch, tmp_path):
    ...
    with monkeypatch.context() as m:
        with patch('src.utils.path_utils.get_project_root', return_value=project_root):
            ...
            # 実際の実装に合わせてエラーメッセージを確認
            assert "config.yaml に 'datasets' セクションが見つからないか、無効です" in str(exc_info.value)
...
def test_get_dataset_paths_missing_dataset(monkeypatch, tmp_path):
    ...
    with monkeypatch.context() as m:
        with patch('src.utils.path_utils.get_project_root', return_value=project_root):
            ...
            # 実際の実装に合わせてエラーメッセージを確認
            assert "config.yaml の 'datasets' セクションにデータセット 'test_dataset' の定義が見つかりません" in str(exc_info.value)
...
def test_get_dataset_paths_missing_required_dirs(monkeypatch, tmp_path):
    ...
    with monkeypatch.context() as m:
        with patch('src.utils.path_utils.get_project_root', return_value=project_root):
            ...
            # 実際の実装に合わせてエラーメッセージを確認
            assert "データセット 'test_dataset' の設定に 'audio_dir' または 'label_dir' がありません" in str(exc_info.value)
    ...
    for label_format in label_formats:
        ...
        with monkeypatch.context() as m:
            with patch('src.utils.path_utils.get_project_root', return_value=project_root):
                ...
                audio_path, label_path, file_pairs = get_dataset_paths(config, "test_dataset")
                ...
                if label_format is None:
                    assert len(file_pairs) == 2 # Default format finds two pairs (.txt and .csv)
                    # ファイルペアのラベルパスを確認
                    ...
                    assert len(file_pairs) == 1  # 特定のフォーマットでは1つのファイルペアが見つかる
...
def test_setup_python_path(monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr("src.utils.path_utils.load_environment_variables", mock_load_env)
...
def test_setup_python_path_error_handling(monkeypatch):
    ...
    # モックを適用
    monkeypatch.setattr("src.utils.path_utils.get_project_root", mock_get_project_root_error)
    monkeypatch.setattr("src.utils.path_utils.load_environment_variables", mock_load_env)
...
class TestGetAbsolutePath:
    ...
    def test_relative_input_default_base(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("src.utils.path_utils.get_project_root", lambda: mock_project_root)
...
def test_get_allowed_upload_directories(monkeypatch):
    ...
    # 環境変数のモック
    env_vars = {
        "MIREX_ALLOWED_UPLOAD_DIRS": "/path/to/upload1:/path/to/upload2"
    }
