...
def logger_for_test():
    ...
    logger = setup_logger(
        logger_name, level=logging.DEBUG, console_level=logging.DEBUG
    )  # Ensure DEBUG level
...
def test_log_exception(caplog, logger_for_test):
    ...
    assert (
        error_message in record.exc_text
    )  # Ensure the error message is in the traceback text
...
def test_retry_on_exception_success():
    ...
    # デコレータを適用
    decorated_func = retry_on_exception(max_attempts=3, logger=mock_logger)(mock_func)
...
def test_retry_on_exception_max_attempts_reached():
    ...
    with patch("time.sleep"):
        # デコレータを適用
        decorated_func = retry_on_exception(
            max_attempts=2, initial_delay=0.1, logger=mock_logger
        )(mock_func)
...
def test_wrap_exceptions_no_exception():
    ...
    decorated_func = wrap_exceptions(
        [ValueError, TypeError], EvaluationError, "変換中にエラーが発生しました"
    )(success_func)
...
def test_wrap_exceptions_default_message():
    ...
    # メッセージを指定せずにデコレータを適用
    decorated_func = wrap_exceptions([ValueError, TypeError], EvaluationError)(
        error_func
    )
...
def test_subprocess_error_str_representation():
    ...
    error3 = SubprocessError(
        "コマンドの実行に失敗しました", returncode=2, stderr="エラー出力"
    )
    str_rep3 = str(error3)
    ...
    long_text = "a" * 1000
    error5 = SubprocessError(
        "コマンドの実行に失敗しました", stdout=long_text, stderr=long_text
    )
    str_rep5 = str(error5)
