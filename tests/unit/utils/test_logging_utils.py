...
def test_setup_logger_separate_levels(cleanup_loggers):
    ...
    reset_logger(logger_name)  # 前のテストの影響を確実に排除
    logger = setup_logger(logger_name, level=logging.INFO, console_level=logging.WARNING)

    # コンソールハンドラのレベルを確認
    console_handlers = [h for h in logger.handlers if isinstance(h, logging.StreamHandler)]
    assert len(console_handlers) == 1  # file_levelがあってもoutput_dirがない場合はファイルハンドラは追加されない
...
def test_setup_logger_with_file_output(tmp_path: Path, cleanup_loggers):
    ...
    assert any(isinstance(h, logging.FileHandler) for h in handlers), "ファイルハンドラが追加されていません"
...
def test_setup_logger_idempotency(cleanup_loggers):
    ...
    console_handlers1 = [h for h in logger1.handlers if isinstance(h, logging.StreamHandler)]
    ...
    # レベルも変更されないはず
    console_handlers2 = [h for h in logger2.handlers if isinstance(h, logging.StreamHandler)]
