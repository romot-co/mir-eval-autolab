...
def test_normalize_basic_conversion(raw_detection_dict):
    ...
    np.testing.assert_array_equal(
        result.intervals, np.array(raw_detection_dict["intervals"])
    )
    np.testing.assert_array_equal(
        result.note_pitches, np.array(raw_detection_dict["note_pitches"])
    )
    np.testing.assert_array_equal(
        result.frame_times, np.array(raw_detection_dict["frame_times"])
    )
    np.testing.assert_array_equal(
        result.frame_frequencies, np.array(raw_detection_dict["frame_frequencies"])
    )
...
def test_normalize_minimal_input():
    ...
    np.testing.assert_array_equal(
        result.note_pitches, np.array(minimal_dict["note_pitches"])
    )
    np.testing.assert_array_equal(
        result.frame_times, np.array(minimal_dict["frame_times"])
    )
    np.testing.assert_array_equal(
        result.frame_frequencies, np.array(minimal_dict["frame_frequencies"])
    )
...
def test_normalize_already_numpy(raw_detection_dict):
    ...
    numpy_dict = {
        ...
        for k, v in raw_detection_dict.items()
    }
    # extra_data を追加
    ...
    np.testing.assert_array_equal(
        result.frame_frequencies, numpy_dict["frame_frequencies"]
    )
    assert result.detector_name == numpy_dict["detector_name"]
    # 追加データの確認
    assert "extra_data" in result.additional_data
    np.testing.assert_array_equal(
        result.additional_data["extra_data"], numpy_dict["extra_data"]
    )
    assert "some_info" in result.additional_data
    assert "ignored_key" in result.additional_data
...
def test_normalize_handles_none_values():
    ...
    np.testing.assert_array_equal(
        result.note_pitches, np.array(none_dict["note_pitches"])
    )
...
@pytest.mark.parametrize("missing_key", ["intervals", "note_pitches"])
...
def test_normalize_invalid_interval_times(raw_detection_dict):
    ...
    with pytest.raises(
        ValueError, match="インターバルの開始時刻が終了時刻より後になっています"
    ):
...
def test_normalize_invalid_frame_times(raw_detection_dict):
    ...
    invalid_dict_non_mono["frame_frequencies"] = [440.0] * len(
        invalid_dict_non_mono["frame_times"]
    )
    ...
    invalid_dict_negative_start["frame_frequencies"] = [440.0] * len(
        invalid_dict_negative_start["frame_times"]
    )
...
def test_normalize_length_mismatch_interval_pitch(raw_detection_dict, caplog):
    ...
    invalid_dict_short_pitch["note_pitches"] = [
        60.0
    ]  # Make pitches shorter (len=1 vs intervals len=2)

    invalid_dict_short_interval = raw_detection_dict.copy()
    invalid_dict_short_interval["intervals"] = [
        [0.1, 0.5]
    ]  # Make intervals shorter (len=1 vs pitches len=2)
...
def test_normalize_detection_result_with_additional_data():
    ...
    additional_data = {
        ...
        "settings": {"threshold": 0.5, "window_size": 2048},
    }
...
def test_get_detector_class_success():
    ...
    # レジストリ関数をモック
    with patch(
        "src.utils.detector_utils.get_registered_detector", return_value=DummyDetector
    ):
        # 有効な検出器名を指定
...
def test_get_detector_class_not_subclass():
    ...
    # レジストリ関数をモック
    with patch(
        "src.utils.detector_utils.get_registered_detector", return_value=NotADetector
    ):
        # 無効なクラスを持つ検出器名を指定
        with pytest.raises(ImportError) as exc_info:
...
def test_create_detector_with_params():
    ...
    with patch(
        "src.utils.detector_utils.get_detector_class", return_value=DummyDetector
    ):
        # パラメーターを指定して検出器を作成
        detector_params = {"threshold": 0.7, "window_size": 4096}
...
def test_create_detector_error_handling():
    ...
    # get_detector_classをモック
    with patch(
        "src.utils.detector_utils.get_detector_class", return_value=ErrorDetector
    ):
        # 例外が発生することを確認
