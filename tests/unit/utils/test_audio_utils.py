...
def dummy_ref_df():
    ...
    return pd.DataFrame(
        {"onset": [0.1, 0.6], "offset": [0.5, 1.0], "pitch": [60.0, 72.0]}
    )
...
def dummy_ref_json():
    ...
    return {"intervals": [[0.1, 0.5], [0.6, 1.0]], "note_pitches": [60.0, 72.0]}
...
def test_load_audio_file_error(mock_logger):
    ...
    with patch(
        "librosa.load", side_effect=Exception("Mocked Exception")
            ...
            with patch("os.path.splitext", return_value=("dummy_ref", ".csv")):
                with patch(
                    ...
                    np.testing.assert_array_equal(
                        result["intervals"], np.array([[0.1, 0.5], [0.6, 1.0]])
                    )
                    np.testing.assert_array_equal(
                        result["note_pitches"], np.array([60.0, 72.0])
                    )
...
def test_load_reference_data_csv_error():
    ...
    with patch("os.path.splitext", return_value=("dummy_ref", ".csv")):
        with patch("os.path.exists", return_value=True):
            with patch(
                "pandas.read_csv", side_effect=Exception("Mocked CSV Error")
            ) as mock_read_csv:
                with patch(
                    "logging.getLogger", return_value=MagicMock()
                ) as mock_get_logger:
...
def test_load_reference_data_permission_error(tmp_path, caplog):
    ...
    with patch("os.path.exists", return_value=True):
        with patch("builtins.open", mock_open):
            with patch("os.path.splitext", return_value=("dummy_ref", ".csv")):
                with patch(
                    "pandas.read_csv", side_effect=PermissionError("権限がありません")
                ):
                    result = load_reference_data(str(csv_path))
...
def test_load_audio_and_reference_success(
    dummy_audio_data, dummy_ref_data, mock_logger
):
    """音声と参照データの読み込み成功ケース"""

    with patch(
        "src.utils.audio_utils.load_audio_file", return_value=dummy_audio_data
    ) as mock_load_audio:
        with patch(
            "src.utils.audio_utils.load_reference_data", return_value=dummy_ref_data
        ) as mock_load_reference:
            with patch("os.path.exists", return_value=True):
                audio_path = "dummy_audio.wav"
                ref_path = "dummy_ref.csv"

                audio, ref_data, sr = load_audio_and_reference(
                    audio_path, ref_path, mock_logger
                )
...
def test_load_audio_and_reference_audio_error(dummy_ref_data, mock_logger):
    ...
    with patch(
        "src.utils.audio_utils.load_audio_file", return_value=(None, None)
    ) as mock_load_audio:
        with patch(
            "src.utils.audio_utils.load_reference_data", return_value=dummy_ref_data
        ) as mock_load_reference:
