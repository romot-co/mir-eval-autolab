...
try:
    ...
    def midi_to_hz(midi_notes):
        ...
        if isinstance(midi_notes, np.ndarray):
            ...
            result[valid_mask] = 440.0 * (2.0 ** ((midi_notes[valid_mask] - 69.0) / 12.0))
            return result
        else:
            if midi_notes <= 0:
                return 0.0
...
def test_cents_to_ratio_scalar():
    ...
    assert np.isclose(cents_to_ratio(700), 2.0 ** (7/12))  # 700セントは5度（比率約1.5）
...
def test_cents_to_ratio_negative():
    """負のセント値を周波数比に変換する"""
    assert np.isclose(cents_to_ratio(-1200), 0.5)  # -1200セントは1オクターブ下（比率0.5）
...
def test_ratio_to_cents_scalar():
    ...
    assert np.isclose(ratio_to_cents(0.5), -1200.0)  # 比率0.5は-1200セント（1オクターブ下）
