...
@patch('logging.error')
def test_detection_result_creation(mock_error, mock_warning, mock_debug, sample_detection_data):
    ...
    np.testing.assert_array_equal(np.array(result_dict["extra_array"]), sample_detection_data["extra_array"])

@patch('logging.debug')
@patch('logging.warning')
@patch('logging.error')
def test_detection_result_from_dict(mock_error, mock_warning, mock_debug, sample_detection_data):
...
@patch('logging.warning')
@patch('logging.error')
def test_detection_result_to_dict(mock_error, mock_warning, mock_debug, sample_detection_data):
...
def test_to_note_list_missing_pitches(mock_warning, mock_debug, caplog):
    ...
    with caplog.at_level(logging.ERROR):
        with pytest.raises(ValueError, match="intervals と note_pitches の長さが一致しません。"):
