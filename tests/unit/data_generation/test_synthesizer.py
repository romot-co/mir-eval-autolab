...
try:
    ...
    def generate_adsr_envelope(
        total_samples, attack_time, decay_time, sustain_level, release_time, sample_rate
    ):
    ...
    def generate_silence(
        output_dir, file_index, instrument_name, duration, sample_rate
    ):
        pass
    ...
    def save_audio_and_label(
        output_dir, file_index, instrument_name, audio_data, labels, sample_rate
    ):
        pass
...
pytestmark = pytest.mark.skipif(
    SKIP_TESTS, reason="必要なモジュールがインポートできませんでした"
)
...
def test_generate_adsr_envelope():
    ...
    assert np.isclose(
        envelope[attack_samples - 1], 1.0
    )  # Should reach peak at end of attack
    # Sustain level check
...
def test_save_audio_and_label_success(mock_soundfile, mock_csv, mock_open, tmp_path):
    ...
    mock_soundfile.write.assert_called_once_with(
        expected_audio_path, audio_data, sample_rate
    )
...
def test_save_audio_and_label_audio_write_error(
    mock_soundfile, mock_csv, mock_open, tmp_path
):
...
def test_save_audio_and_label_label_write_error(
    mock_soundfile, mock_csv, mock_open, tmp_path
):
    ...
    with pytest.raises(IOError):  # Expect the original error to propagate or be wrapped
        save_audio_and_label(
            tmp_path, 2, "error_label", np.zeros(100), [(0, 1, 60)], 16000
        )
