...
try:
    ...
    pytest.fail(
        f"Failed to import mirai or related modules: {e}. Ensure PYTHONPATH is correct."
    )
...
def mock_mcp_client(mocker):
    ...
    mock_client.get_job_status = MagicMock(
        return_value={"status": JobStatus.COMPLETED.value, "result": {}}
    )
    mock_client.get_session_info = MagicMock(
        return_value={
    ...
    mock_client.start_session = MagicMock(
        return_value={"session_id": MOCK_SESSION_ID, "status": "active"}
    )
        ...
        return_value=json.dumps(
            {
                "action": "improve_code",
                "parameters": {
                    "suggestion": "LLM mock suggestion",
                    "code": "print('llm mock code')",
                },
            }
        )
    )
    mock_llm.extract_code_from_text = AsyncMock(
        return_value="print('extracted llm code')"
    )
...
def mock_determine_next_action(mocker):
    ...
    mock_action = AsyncMock(
        return_value={
            "action": "improve_code",
            "parameters": {
                "suggestion": "Default mock suggestion",
...
def mock_execute_analyze_evaluation(mocker):
    ...
    return mocker.patch(
        "src.cli.mirai.execute_analyze_evaluation", AsyncMock(return_value=True)
    )
...
def mock_execute_generate_hypotheses(mocker):
    ...
    return mocker.patch(
        "src.cli.mirai.execute_generate_hypotheses", AsyncMock(return_value=True)
    )
...
def mock_execute_improve_code(mocker):
    ...
    return mocker.patch(
        "src.cli.mirai.execute_improve_code", AsyncMock(return_value=True)
    )
...
def mock_execute_run_evaluation(mocker):
    ...
    return mocker.patch(
        "src.cli.mirai.execute_run_evaluation", AsyncMock(return_value=True)
    )
...
def mock_execute_optimize_parameters(mocker):
    ...
    return mocker.patch(
        "src.cli.mirai.execute_optimize_parameters", AsyncMock(return_value=True)
    )
...
class TestEvaluateCommand:
    ...
    def test_evaluate_standalone_success(
        ...
        # act
        evaluate(
            detector_name=detector, audio_path=audio, ref_path=ref, output_dir=output
        )
    ...
    def test_evaluate_standalone_file_not_found(
        self, mocker, mock_path_utils, mock_config, caplog
    ):
    ...
    def test_evaluate_server_missing_dataset(
        self, mock_mcp_client, mock_config, caplog
    ):
    ...
    def test_evaluate_server_job_timeout(self, mock_mcp_client, mock_config, caplog):
        ...
        with patch(
            "src.cli.mirai.poll_job_status",
            side_effect=TimeoutError("Job polling timeout"),
        ) as mock_poll:
            with pytest.raises(typer.Exit) as e:
                evaluate(
                    detector_name=MOCK_DETECTOR_NAME,
    ...
    def test_evaluate_cli_runner(
        self, runner, mock_mcp_client, mock_config, caplog, mocker
    ):
...
class TestGridSearchCommand:
    ...
    def test_grid_search_success(self, mock_mcp_client, mock_config, caplog, mocker):
        ...
        with patch("yaml.safe_load", return_value=grid_config_data) as mock_load, patch(
            "src.cli.mirai.poll_job_status",
            return_value={
                "status": JobStatus.COMPLETED.value,
                "result": {
                    "summary": "Grid search done",
                    "best_params_path": "/server/best.json",
                },
            },
        ) as mock_poll:
        ...
        mock_mcp_client.run_tool.assert_called_once_with(
            "run_grid_search", {"grid_config": grid_config_data}
        )
        mock_poll.assert_called_once()
    ...
    def test_grid_search_config_not_found(
        self, mock_mcp_client, mocker, mock_config, caplog
    ):
    ...
    def test_grid_search_yaml_error(
        self, mock_mcp_client, mock_yaml_load, mock_config, caplog
    ):
    ...
    def test_grid_search_job_fails(
        self, mock_mcp_client, mock_yaml_load, mock_config, caplog
    ):
        ...
        with patch(
            ...
            with pytest.raises(typer.Exit) as e:
                grid_search(
                    server_url=MOCK_SERVER_URL, config_path=Path(MOCK_CONFIG_FILE)
                )
    ...
    def test_grid_search_empty_config(
        self, mock_mcp_client, mocker, mock_config, caplog
    ):
    ...
    def test_grid_search_missing_job_id(
        self, mock_mcp_client, mock_yaml_load, mock_config, caplog
    ):
    ...
    def test_grid_search_unknown_status(
        self, mock_mcp_client, mock_yaml_load, mock_config, caplog
    ):
        ...
        with patch(
            "src.cli.mirai.poll_job_status",
            return_value={"status": "unknown", "result": {}},  # 不明なステータス
        ) as mock_poll:
            with pytest.raises(typer.Exit) as e:
                grid_search(
                    server_url=MOCK_SERVER_URL, config_path=Path(MOCK_CONFIG_FILE)
...
class TestConfigurationHandling:
    ...
    def test_empty_config_fallback(
        self, mocker, mock_path_utils, mock_evaluate_detector, caplog
    ):
    ...
    def test_partially_missing_config(
        self, mocker, mock_path_utils, mock_evaluate_detector, caplog
    ):
    ...
    def test_invalid_config_type(self, mocker, mock_path_utils, caplog):
        ...
        # 警告ログが出ているか確認
        assert any(
            "型が不正" in record.message or "Invalid type" in record.message
            for record in caplog.records
        )

    def test_improve_default_values(
        self,
        mocker,
        mock_mcp_client,
        mock_llm_client,
        mock_determine_next_action,
        caplog,
    ):
        ...
        assert any(
            f"max_cycles: {DEFAULT_MAX_CYCLES}" in record.message
            for record in caplog.records
        ) or any(
            f"LLM model: {DEFAULT_LLM_MODEL}" in record.message
            for record in caplog.records
        )
...
class TestImproveCommand:
    ...
    async def test_improve_start_resume_session(
        ...
        with patch(
            "src.cli.mirai.execute_run_evaluation", return_value=True
        ) as mock_exec_eval:
            await improve(
                server_url=MOCK_SERVER_URL,
                detector_name=MOCK_DETECTOR_NAME,
    ...
    async def test_improve_start_user_confirm_rejects(
        self,
        mock_mcp_client,
        mock_determine_next_action,
        mock_typer_confirm,
        ...
        assert (
            "User rejected code changes." in caplog.text
        )  # Check log message (assuming execute_improve_code logs this)
        assert "Improvement loop finished." in caplog.text
        assert "Final session state:" in caplog.text
    ...
    async def test_improve_determines_other_actions(
        ...
        mock_determine_next_action,
        mock_config,
        mock_execute_analyze_evaluation,
        mock_execute_generate_hypotheses,
        mock_execute_run_evaluation,
        mock_execute_optimize_parameters,
    ):
        ...
        actions_to_test = [
            (
                "analyze_evaluation",
                mock_execute_analyze_evaluation,
                {"evaluation_results": {"f1": 0.5}},
            ),
            (
                "generate_hypotheses",
                mock_execute_generate_hypotheses,
                {"current_metrics": {"f1": 0.6}},
            ),
    ...
    async def test_improve_session_ends(
        self, mock_mcp_client, mock_determine_next_action, mock_config, caplog, capsys
    ):
        ...
        mock_determine_next_action.assert_awaited_once()
        assert (
            f"Session {active_session_id} has reached a terminal state (completed)."
            in caplog.text
        )
        captured = capsys.readouterr()
    ...
    async def test_improve_error_timeout_error(
        ...
        assert (
            "src.cli.mirai",
            logging.ERROR,
            "Error running evaluation: Job polling timed out",
        ) in caplog.record_tuples

    async def test_improve_error_value_error(
        ...
        caplog.set_level(logging.INFO)
        mock_determine_next_action.side_effect = ValueError(
            "Missing data for an action"
        )
        ...
        assert (
            "src.cli.mirai",
            logging.ERROR,
            "Error improving code: LLM internal error",
        ) in caplog.record_tuples

    async def test_improve_no_confirm(
        self,
        mock_mcp_client,
        mock_llm_client,
        mock_typer_confirm,
    ...
    @pytest.mark.parametrize(
        "error_type, side_effect, expected_error_log_fragment",
        [
            # Match the actual ERROR/CRITICAL log message fragments from the correct source
            (
                McpError,
                McpError(type("DummyError", (), {"message": "MCP Test Error"})()),
                "Error in improvement loop (cycle 1): MCP Test Error",
            ),  # Caught in main loop
            (
                ToolError,
                ToolError("Tool Test Error"),
                "Error running evaluation: Tool Test Error",
            ),  # Caught in execute_run_evaluation
            (
                LLMClientError,
                LLMClientError("LLM Test Error", error_type="APIError"),
                "Error in improvement loop (cycle 1): LLM Test Error",
            ),  # Caught in main loop
            (
                TimeoutError,
                TimeoutError("Timeout Test Error"),
                "Error running evaluation: Timeout Test Error",
            ),  # Caught in execute_run_evaluation
            (
                ValueError,
                ValueError("Value Test Error"),
                "Error in improvement loop (cycle 1): Value Test Error",
            ),  # Caught in main loop
        ],
    )
    async def test_improve_errors(
        self,
        mock_mcp_client,
        mock_determine_next_action,
        error_type,
        side_effect,
        expected_error_log_fragment,
        caplog,
        mocker,
        mock_poll_job_status,
    ):
        ...
        caplog.set_level(logging.INFO)
        active_session_id = MOCK_SESSION_ID
        mock_mcp_client.start_session.return_value = {
            "session_id": active_session_id,
            "status": "active",
        }
        mock_mcp_client.get_session_info.return_value = {
            "session_id": active_session_id,
            "status": "active",
            "best_metrics": {},
        }
        ...
        if error_type in [
            ValueError
        ]:  # Errors raised in main loop before action dispatch
            ...
            mocker.patch(
                "src.cli.mirai.execute_run_evaluation",
    ...
    async def test_llm_json_error(
        self, mock_mcp_client, mock_llm_client, mock_config, caplog
    ):
        ...
        # LLMが不正なJSONを返す
        mock_llm_client.generate.side_effect = json.JSONDecodeError(
            "不正なJSON", "{不正な形式", 0
        )
...
class TestPollJobStatus:
    ...
    def test_poll_job_status_completed(self, mock_mcp_client):
        ...
        result = poll_job_status(
            mock_mcp_client, "test_job_id", poll_interval=0.01, timeout=1
        )
    ...
    def test_poll_job_status_timeout(self, mock_mcp_client):
        ...
        with pytest.raises(TimeoutError):
            poll_job_status(
                mock_mcp_client, "test_job_id", poll_interval=0.01, timeout=0.05
            )
    ...
    def test_poll_job_status_unknown(self, mock_mcp_client, caplog):
        ...
        with pytest.raises(TimeoutError) as excinfo:
            poll_job_status(
                mock_mcp_client, "test_job_id", poll_interval=0.01, timeout=0.1
            )
...
class TestCreateGridConfigFromSuggestions:
    ...
    def test_suggested_range_int(self):
        """整数型の範囲指定テスト"""
        suggestion = {
            "parameters": [{"name": "param1", "type": "int", "suggested_range": [1, 5]}]
        }
    ...
    def test_missing_name(self):
        ...
        suggestion = {"parameters": [{"type": "int", "suggested_range": [1, 10]}]}
        config = create_grid_config_from_suggestions("Detector", suggestion)
