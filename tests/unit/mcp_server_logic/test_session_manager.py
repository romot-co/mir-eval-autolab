...
try:
    ...
    from src.mcp_server_logic.schemas import (
        SessionInfoResponse, HistoryEntry, SessionStatus, # 必要に応じて追加
    ...
    print("Warning: Using dummy implementations for session_manager.py and dependencies.")
    ...
    async def start_session(config: SessionManagerConfig, db_utils_mock, misc_utils_mock) -> SessionInfoResponse:
    ...
    async def get_session_info(session_id: str, config: SessionManagerConfig, db_utils_mock) -> Optional[SessionInfoResponse]:
        ...
        if history_rows:
             history = [
                 ...
                 HistoryEntry(event_id=row[0] if isinstance(row, tuple) else row.get("event_id"),
                              timestamp=row[2] if isinstance(row, tuple) else row.get("timestamp"),
                              event_type=row[3] if isinstance(row, tuple) else row.get("event_type"),
                              details=json.loads(row[4]) if isinstance(row, tuple) else json.loads(row.get("details_json")))
    ...
    async def add_session_history(session_id: str, event_type: str, details: Dict[str, TypingAny], config: SessionManagerConfig, db_utils_mock, misc_utils_mock, validate_func_mock):
...
def mock_config() -> SessionManagerConfig:
    ...
    return SessionManagerConfig(db_path=":memory:", session_timeout_hours=1) # Short timeout for cleanup test
...
async def test_get_session_info_found(mock_config, mock_db_utils):
    ...
    session_data = {
        "session_id": session_id, "status": SessionStatus.RUNNING.value,
        "created_at": now_iso, "last_update": now_iso,
        "best_metrics_json": '{"f_measure": 0.8}', "stagnation_count": 1
    }
    history_data = [
    ...
    mock_db_utils.db_fetch_one_async.return_value = session_data # Simulate finding session
    mock_db_utils.db_fetch_all_async.return_value = history_data # Simulate finding history
...
async def test_get_session_info_db_error(mock_config, mock_db_utils):
    ...
    mock_db_utils.db_fetch_one_async.side_effect = StateManagementError("DB read failed")
...
async def test_add_session_history_success(mock_config, mock_db_utils, mock_misc_utils, mock_validate):
    ...
    session_update_call_args, _ = mock_db_utils.db_execute_commit_async.call_args_list[1]
    assert session_update_call_args[0] == mock_config.db_path
    assert "UPDATE sessions SET last_update = ?" in session_update_call_args[1] # Basic check
...
async def test_cleanup_old_sessions(mock_config, mock_db_utils):
    ...
    expected_cutoff = datetime.now(timezone.utc) - timedelta(hours=mock_config.session_timeout_hours)
    # Allow some tolerance for test execution time
    assert abs(datetime.fromisoformat(cutoff_time_param) - expected_cutoff) < timedelta(seconds=10) 