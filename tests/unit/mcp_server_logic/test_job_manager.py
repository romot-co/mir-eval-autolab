...
try:
    ...
    from src.mcp_server_logic.schemas import (
        JobInfo, JobStatus, JobStartResponse, ToolInput, ToolOutput, ErrorInfo, SessionInfoResponse
    )
    ...
    async def start_async_job(session_id: str, tool_name: str, params: Dict[str, TypingAny], config: JobManagerConfig, db_utils_mock, misc_utils_mock, session_manager_mock) -> JobStartResponse:
        ...
        await db_utils_mock.db_execute_commit_async(
            config.db_path,
            ANY, # Query doesn't matter for dummy
            ANY
        )
        # Simulate queue put
    ...
    async def get_job_status(job_id: str, config: JobManagerConfig, db_utils_mock) -> Optional[JobInfo]:
    ...
    async def job_worker(worker_id: int, config: JobManagerConfig, db_utils_mock, session_manager_mock, llm_tools_mock, code_tools_mock, evaluation_tools_mock, misc_utils_mock):
        while True:
            ...
            try:
                ...
                tool_func = getattr(llm_tools_mock, f"_run_{job_info.tool_name}_async", None)
                if tool_func:
                    input_params = json.loads(job_info.input_params_json or '{}')
                    # Dummy tool call needs SessionInfoResponse, create a basic one
                    dummy_session_info = SessionInfoResponse(session_id=job_info.session_id, status="running", history=[])
                    tool_output = await tool_func(dummy_session_info, ToolInput(params=input_params)) # Pass dummy session info
                else:
                    raise ValueError(f"Unknown tool: {job_info.tool_name}")
                job_info.status = JobStatus.COMPLETED
                job_info.result_json = json.dumps(tool_output.result) # Assuming ToolOutput has 'result' field
            except Exception as e:
                job_info.status = JobStatus.FAILED
                error_info = ErrorInfo(error_type=type(e).__name__, message=str(e))
                job_info.error_json = json.dumps(error_info.__dict__) # Use dummy ErrorInfo dataclass
                 # Simulate session status update on failure
                await session_manager_mock.update_session_status_internal(job_info.session_id, "failed", config.db_path, db_utils_mock)
            ...
            await session_manager_mock.add_session_history(
                 ...
                 event_type=f"{job_info.tool_name}_" + ("complete" if job_info.status == JobStatus.COMPLETED else "failed"),
...
def mock_db_utils():
    ...
    mock = MagicMock()
    mock.db_execute_commit_async = AsyncMock(return_value=1) # Simulate successful commit
...
def mock_session_manager():
    ...
    dummy_session_info = SessionInfoResponse(session_id="sid", status="running", history=[]) # Dummy
...
async def test_start_async_job_success(mock_config, mock_db_utils, mock_misc_utils, mock_session_manager):
    ...
    response = await start_async_job(session_id, tool_name, params, mock_config, mock_db_utils, mock_misc_utils, mock_session_manager)
...
async def test_start_async_job_db_error(mock_config, mock_db_utils, mock_misc_utils, mock_session_manager):
    ...
    with pytest.raises(StateManagementError, match="DB connection failed"):
        await start_async_job("sid", "tool", {}, mock_config, mock_db_utils, mock_misc_utils, mock_session_manager)
...
async def test_get_job_status_active(mock_config, mock_db_utils):
    ...
    active_job_info = JobInfo(job_id=job_id, session_id="s1", tool_name="t1", status=JobStatus.RUNNING)
...
async def test_get_job_status_from_db(mock_config, mock_db_utils):
    ...
    mock_db_utils.db_fetch_one_async.return_value = db_data # Simulate DB returning data as dict
