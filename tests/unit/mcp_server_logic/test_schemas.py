...
# Broad exception for initial setup, refine if possible
    ...
    class BaseToolInput(BaseModelAlias): pass
...
def test_job_info_missing_required():
    ...
    with pytest.raises(ValidationError):
        JobInfo(job_id="job_123", tool_name="test") # Missing status, created_at, updated_at

def test_job_info_invalid_status():
    """Test JobInfo validation error on invalid status enum."""
    invalid_data = {
        "job_id": "job_123", "tool_name": "test", "status": "finished", # Invalid status
...
def test_job_info_optional_fields():
    ...
    minimal_data = {
        "job_id": "job_min", "tool_name": "minimal", "status": JobStatus.PENDING,
...
def test_session_info_response_empty_history_and_metrics():
    ...
    minimal_data = {
        "session_id": "sess_min", "status": SessionStatus.STOPPED,
        "created_at": "ts1", "last_update": "ts2",
...
def test_evaluate_detector_input_valid():
    ...
    data = {"detector_name": "MyDet", "dataset_name": "MyData", "parameters": {"sens": 0.5}}
...
def test_run_grid_search_input_invalid_grid():
    ...
    with pytest.raises(ValidationError):
        RunGridSearchInput(detector_name="GSDet", dataset_name="GSData", param_grid="invalid")
