...
def mock_config():
    """テスト用の設定を提供する"""
    return {"paths": {"workspace": "./test_workspace", "db_dir": "./test_workspace/db"}}
...
async def test_db_fetch_one_success(test_db):
    ...
    row = await db_fetch_one_async(
        test_db, "SELECT * FROM sessions WHERE session_id = ?", (session_id,)
    )
...
async def test_db_fetch_one_not_found(test_db):
    """存在しないデータを検索した場合のテスト"""
    row = await db_fetch_one_async(
        test_db, "SELECT * FROM sessions WHERE session_id = ?", ("nonexistent",)
    )
...
async def test_db_fetch_all_success(test_db):
    ...
    rows = await db_fetch_all_async(
        test_db, "SELECT * FROM sessions ORDER BY session_id"
    )
...
async def test_db_execute_delete(test_db):
    ...
    # データを削除
    await db_execute_commit_async(
        test_db, "DELETE FROM sessions WHERE session_id = ?", (session_id,)
    )
...
async def test_db_error_handling(test_db):
    ...
    with pytest.raises(StateManagementError):
        await db_fetch_all_async(test_db, "SELECT * FROM nonexistent_table")
