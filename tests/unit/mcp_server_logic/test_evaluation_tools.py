...
try:
    ...
    from src.mcp_server_logic.schemas import (
        ...
        EvaluateDetectorInput, EvaluateDetectorOutput,
        RunEvaluationInput, RunEvaluationOutput,
        GridSearchInput, GridSearchOutput,
        ErrorInfo
    )
    ...
    print("Warning: Using dummy implementations for evaluation_tools.py and dependencies.")
    from dataclasses import dataclass, field
    from typing import Dict, Any as TypingAny, Optional, List

    class EvaluationToolsError(Exception): pass
    class StateManagementError(Exception): pass # Dummy if needed
    ...
    @dataclass
    class EvaluateDetectorInput: params: Dict[str, TypingAny]
    @dataclass
    class EvaluateDetectorOutput: result: Dict[str, TypingAny]
    @dataclass
    class RunEvaluationInput: params: Dict[str, TypingAny]
    @dataclass
    class RunEvaluationOutput: result: Dict[str, TypingAny]
    @dataclass
    class GridSearchInput: params: Dict[str, TypingAny]
    @dataclass
    class GridSearchOutput: result: Dict[str, TypingAny]
    @dataclass
    ...
    class DummyEvaluationRunner:
        ...
        def evaluate_detector(*args, **kwargs) -> Dict[str, TypingAny]:
             ...
             if kwargs.get("audio_path") == "fail": raise ValueError("Dummy eval failure")
        ...
        def run_evaluation(*args, **kwargs) -> Dict[str, TypingAny]:
             ...
             if kwargs.get("dataset_path") == "fail": raise ValueError("Dummy run_eval failure")
             return {"summary": {"f_measure_mean": 0.85}, "results_path": "/path/to/results.json"}
    ...
    class DummyGridSearchCore:
         ...
         def run_grid_search(*args, **kwargs) -> Dict[str, TypingAny]:
             ...
             if kwargs.get("detector_name") == "fail": raise ValueError("Dummy grid_search failure")
             return {"best_params": {"threshold": 0.6}, "best_results": {"f_measure": 0.92}, "summary_path": "/path/gs_summary.csv"}
    grid_search_core = DummyGridSearchCore()

    # --- Dummy Async Wrappers (Copied for context, real wrappers assumed to exist) ---
    async def _run_async_wrapper(func, session_info: SessionInfoResponse, tool_input, tool_output_class, event_name: str, executor_mock, session_manager_mock, config):
    ...
    async def _run_evaluate_detector_async(session_info: SessionInfoResponse, tool_input: EvaluateDetectorInput, executor_mock, session_manager_mock, config):
         return await _run_async_wrapper(evaluation_runner.evaluate_detector, session_info, tool_input, EvaluateDetectorOutput, TOOL_NAME_EVALUATE_DETECTOR, executor_mock, session_manager_mock, config)

    async def _run_run_evaluation_async(session_info: SessionInfoResponse, tool_input: RunEvaluationInput, executor_mock, session_manager_mock, config):
         return await _run_async_wrapper(evaluation_runner.run_evaluation, session_info, tool_input, RunEvaluationOutput, TOOL_NAME_RUN_EVALUATION, executor_mock, session_manager_mock, config)

    async def _run_grid_search_async(session_info: SessionInfoResponse, tool_input: GridSearchInput, executor_mock, session_manager_mock, config):
         return await _run_async_wrapper(grid_search_core.run_grid_search, session_info, tool_input, GridSearchOutput, TOOL_NAME_GRID_SEARCH, executor_mock, session_manager_mock, config)
...
def mock_evaluate_detector():
    ...
    return MagicMock(return_value={"valid": True, "metrics": {"f_measure": 0.95}, "error_message": None})

@pytest.fixture
def mock_run_evaluation():
    """Mocks evaluation_runner.run_evaluation."""
    return MagicMock(return_value={"summary": {"f_measure_mean": 0.88}, "results_path": "/results.json"})
...
async def test_run_evaluate_detector_async_success(dummy_session_info_for_eval, mock_executor, mock_session_manager_for_eval, mock_config_for_eval, mock_evaluate_detector):
     ...
     eval_runner_path = 'src.mcp_server_logic.evaluation_tools.evaluation_runner.evaluate_detector'
     try:
         with patch(eval_runner_path, mock_evaluate_detector), \
             ...
             output = await _run_evaluate_detector_async(
                 dummy_session_info_for_eval, tool_input, mock_executor,
                 mock_session_manager_for_eval, mock_config_for_eval
             )
...
async def test_run_run_evaluation_async_success(dummy_session_info_for_eval, mock_executor, mock_session_manager_for_eval, mock_config_for_eval, mock_run_evaluation):
    ...
    target_path = 'src.mcp_server_logic.evaluation_tools.evaluation_runner.run_evaluation'
    try:
        with patch(target_path, mock_run_evaluation), \
             patch('asyncio.get_running_loop') as mock_loop:
            mock_loop.return_value.run_in_executor = AsyncMock(return_value=sync_result)
...
async def test_run_grid_search_async_success(dummy_session_info_for_eval, mock_executor, mock_session_manager_for_eval, mock_config_for_eval, mock_run_grid_search):
    ...
    target_path = 'src.mcp_server_logic.evaluation_tools.grid_search_core.run_grid_search'
    try:
        with patch(target_path, mock_run_grid_search), \
             patch('asyncio.get_running_loop') as mock_loop:
            mock_loop.return_value.run_in_executor = AsyncMock(return_value=sync_result)
...
async def test_run_evaluate_detector_async_failure(dummy_session_info_for_eval, mock_executor, mock_session_manager_for_eval, mock_config_for_eval, mock_evaluate_detector):
     ...
     try:
         with patch(eval_runner_path, mock_evaluate_detector), \
              patch('asyncio.get_running_loop') as mock_loop:
             mock_loop.return_value.run_in_executor = AsyncMock(side_effect=core_exception)
             with pytest.raises(EvaluationToolsError, match=f"Task {TOOL_NAME_EVALUATE_DETECTOR} failed: {error_message}"):
