...
try:
    ...
    from pydantic import (
        BaseModel,
        ValidationError,
    )  # Use pydantic for dummy config model
    ...
    def load_config(config_path: Optional[str] = None) -> ServerConfig:
        ...
        if "MCP_DB_PATH" in os.environ:
            config_data["db_path"] = os.environ["MCP_DB_PATH"]
        if "MCP_WORKSPACE_BASE" in os.environ:
            config_data["workspace_base"] = os.environ["MCP_WORKSPACE_BASE"]
        if "MCP_OUTPUT_BASE" in os.environ:
            config_data["output_base"] = os.environ["MCP_OUTPUT_BASE"]
        if "MCP_LOG_LEVEL" in os.environ:
            config_data["log_level"] = os.environ["MCP_LOG_LEVEL"]
        if "MCP_CLEANUP_INTERVAL_MINUTES" in os.environ:
            try:
                config_data["cleanup_interval_minutes"] = int(
                    os.environ["MCP_CLEANUP_INTERVAL_MINUTES"]
                )
            except ValueError:
                pass
        if "MCP_KEEP_DURATION_MINUTES" in os.environ:
            try:
                config_data["keep_duration_minutes"] = int(
                    os.environ["MCP_KEEP_DURATION_MINUTES"]
                )
            except ValueError:
                pass
        ...
        try:
            ...
            if db_p and not Path(db_p).is_absolute():
                config_data["db_path"] = str(root / db_p)
            if ws_p and not Path(ws_p).is_absolute():
                config_data["workspace_base"] = str(root / ws_p)
            if out_p and not Path(out_p).is_absolute():
                config_data["output_base"] = str(root / out_p)

            if config_data.get("workspace_base"):
                path_utils_module.ensure_dir(Path(config_data["workspace_base"]).parent)
            if config_data.get("output_base"):
                path_utils_module.ensure_dir(Path(config_data["output_base"]).parent)
            if config_data.get("db_path"):
                path_utils_module.ensure_dir(Path(config_data["db_path"]).parent)
    ...
    async def cleanup_workspace_files(config: ServerConfig | Dict[str, Any]):
        ...
        logger = logging.getLogger(
            "src.mcp_server_logic.core"
        )  # Assume this logger name
            ...
            try:
                ...
                if item_time < cutoff_time:
                    try:
                        ...
                        logger.error(
                            f"Error deleting directory {item}: {e}", exc_info=True
                        )
                        errors.append(str(item))
            except Exception as e:
                logger.error(
                    f"Error processing {item} during cleanup: {e}", exc_info=True
                )
                errors.append(str(item))

        logger.info(
            f"Deleted {len(deleted_dirs)} directories. Encountered {len(errors)} errors."
        )
...
def mock_env(monkeypatch):
    ...
    env_keys = [
        ...
        "MCP_OUTPUT_BASE",
        "MCP_LOG_LEVEL",
        "MCP_CLEANUP_INTERVAL_MINUTES",
        "MCP_KEEP_DURATION_MINUTES",
    ]
...
def test_load_config_from_yaml(mock_env, mock_project_paths, tmp_path):
    ...
    with patch(
        "pathlib.Path.exists", side_effect=mock_exists, autospec=True
...
def test_load_config_path_resolution(mock_env, mock_project_paths):
    ...
    with patch("pathlib.Path.exists") as mock_exists, patch.object(
        path_utils_module, "ensure_dir", MagicMock()
    ):
        mock_exists.return_value = False
        config = load_config()
    ...
    expected_ws_parent = (
        mock_project_paths / Path(default_model.workspace_base)
    ).parent
...
def test_load_config_invalid_data_raises_error(mock_env, mock_project_paths):
    ...
    with patch("pathlib.Path.exists") as mock_exists, patch.object(
        ...
        assert (
            config.cleanup_interval_minutes == default_config.cleanup_interval_minutes
        )
...
async def test_cleanup_disabled(tmp_path):
    ...
    config_zero = ServerConfig(
        workspace_base=str(ws_path), cleanup_interval_minutes=0, keep_duration_minutes=1
    )
    config_neg = ServerConfig(
        workspace_base=str(ws_path),
        cleanup_interval_minutes=-10,
        keep_duration_minutes=1,
    )
...
async def test_cleanup_removes_old_items(tmp_path):
    ...
    config = {
        ...
        "cleanup": {"workspace": {"enabled": True, "retention_days": retention_days}},
    }
...
async def test_start_cleanup_task_creates_task_when_enabled(mock_create_task):
    ...
    config = {"cleanup": {"interval_seconds": 3600}}  # 1時間
    ...
    returned_task = await start_cleanup_task(
        config, mock_cleanup_sessions, mock_cleanup_jobs, mock_cleanup_workspace
    )
...
async def test_start_cleanup_task_no_task_when_disabled(mock_create_task):
    ...
    config_zero = {"cleanup": {"interval_seconds": 0}}
    ...
    task_zero = await start_cleanup_task(
        config_zero, mock_cleanup_sessions, mock_cleanup_jobs, mock_cleanup_workspace
    )
    ...
    config_neg = {"cleanup": {"interval_seconds": -1}}
    task_neg = await start_cleanup_task(
        config_neg, mock_cleanup_sessions, mock_cleanup_jobs, mock_cleanup_workspace
    )
    mock_create_task.assert_not_called()
