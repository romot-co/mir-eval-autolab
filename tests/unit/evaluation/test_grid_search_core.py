...
def mock_dependencies():
    ...
    with patch('src.evaluation.grid_search.core.yaml.safe_load') as mock_yaml_load, \
         patch('src.evaluation.grid_search.core.yaml.dump') as mock_yaml_dump, \
         patch('src.evaluation.grid_search.core.os.makedirs') as mock_makedirs, \
         patch('src.evaluation.grid_search.core.os.listdir') as mock_listdir, \
         patch('src.evaluation.grid_search.core.os.path.exists') as mock_exists, \
         patch('src.evaluation.grid_search.core.get_detector_class') as mock_get_detector, \
         patch('src.evaluation.grid_search.core.run_evaluation') as mock_run_eval, \
         patch('src.evaluation.grid_search.core.pd.DataFrame') as mock_pd_dataframe, \
         patch('src.evaluation.grid_search.core.get_logger') as mock_get_logger, \
         patch('builtins.open', new_callable=mock_open) as mock_file_open: # open もモック
        ...
        mock_run_eval.return_value = { # run_evaluation のデフォルト戻り値
            'summary': {'TestDetector':{'note': {'f_measure': 0.8}}}, # detector 名キーの下にメトリクス
            'all_results': [], # 個別結果は省略
        ...
        yield {
            'yaml_load': mock_yaml_load, 'yaml_dump': mock_yaml_dump,
            'makedirs': mock_makedirs, 'listdir': mock_listdir, 'exists': mock_exists,
            'get_detector': mock_get_detector, 'run_eval': mock_run_eval,
            'pd_dataframe': mock_pd_dataframe, 'df_instance': mock_df_instance,
            'get_logger': mock_get_logger, 'logger': mock_get_logger.return_value,
            'open': mock_file_open
        }
...
def test_create_grid_config_basic(mock_dependencies, temp_dir):
    ...
    # yaml.dump が呼ばれたか確認
    mock_dependencies['open'].assert_called_once_with(output_path, 'w', encoding='utf-8')
...
def test_save_results_to_csv_basic(mock_dependencies, temp_dir):
    ...
    results = [
        {'run_id': 'r1', 'execution_time': 1.1, 'params': {'p1': 0.1, 'p2': 'a'},
         'metrics': {'note': {'f_measure': 0.8, 'recall': 0.7}, 'onset': {'precision': 0.9}}},
        {'run_id': 'r2', 'execution_time': 1.2, 'params': {'p1': 0.2, 'p2': 'b'},
         'metrics': {'note': {'f_measure': 0.85, 'recall': 0.75}, 'onset': {'precision': 0.95}}},
        {'run_id': 'r3', 'execution_time': 1.3, 'params': {'p1': 0.3, 'p2': 'c'},
         'metrics': {'note': {'f_measure': 0.7}}} # onset がないケース
    ]
    ...
    assert 'onset.precision' not in passed_data[2] or pd.isna(passed_data[2]['onset.precision'])


    # DataFrame.to_csv が呼ばれたか確認
    mock_dependencies['df_instance'].to_csv.assert_called_once_with(output_csv_path, index=False)
...
def test_save_results_to_csv_empty(mock_dependencies, temp_dir):
    ...
    mock_dependencies['pd_dataframe'].assert_called_once_with([]) # 空リストで DataFrame が呼ばれる
    mock_dependencies['df_instance'].to_csv.assert_called_once_with(output_csv_path, index=False)
...
def test_run_grid_search_basic(mock_dependencies, temp_dir):
    ...
    # モックの設定
    config_data = {'audio_dir': str(temp_dir / 'a'), 'reference_dir': str(temp_dir / 'r'), 'evaluation': {'tolerance_onset': 0.06}}
    grid_config_data = {'detector': 'TestDetector', 'param_grid': {'p1': [1, 2], 'p2': ['x']}}
    ...
    run_eval_results = [
        {'summary': {'TestDetector':{'note': {'f_measure': 0.8}}}, 'status': 'success'}, # p1=1, p2='x'
        {'summary': {'TestDetector':{'note': {'f_measure': 0.9}}}, 'status': 'success'}  # p1=2, p2='x'
    ]
    mock_dependencies['run_eval'].side_effect = run_eval_results

    # 関数実行
    result_data = run_grid_search(config_path, grid_config_path, output_dir, best_metric)
    ...
    mock_dependencies['get_detector'].assert_called_once_with(grid_config_data['detector'])
    # run_evaluation 呼び出し
    ...
    assert calls[0][1]['evaluator_config'] == config_data['evaluation'] # 設定が渡されるか
    assert calls[1][1]['detector_params'] == {'TestDetector': {'p1': 2, 'p2': 'x'}}
    # 結果の保存
    assert mock_dependencies['open'].call_count >= 2 # JSON と CSV
    assert mock_dependencies['yaml_dump'].call_count == 0 # YAML dump は create_grid_config
    ...
    assert result_data['best_result']['params'] == {'p1': 2, 'p2': 'x'} # f_measure が高い方
    ...
    # ロガーの呼び出し確認
    mock_dependencies['logger'].info.assert_any_call("グリッドサーチを開始: 2個のパラメータ組み合わせを評価")
    mock_dependencies['logger'].info.assert_any_call("最適なパラメータが見つかりました (Metric: note.f_measure): {'p1': 2, 'p2': 'x'} (Score: 0.9000)")
...
def test_run_grid_search_config_load_error(mock_dependencies, temp_dir):
    ...
    with pytest.raises(yaml.YAMLError):
        run_grid_search(str(temp_dir / "c.yaml"), str(temp_dir / "g.yaml"), str(temp_dir / "o"))
...
def test_run_grid_search_no_audio_files_found(mock_dependencies, temp_dir):
    ...
    config_data = {'audio_dir': str(temp_dir / 'a'), 'reference_dir': str(temp_dir / 'r')}
    grid_config_data = {'detector': 'D', 'param_grid': {'p': [1]}}
    mock_dependencies['yaml_load'].side_effect = [config_data, grid_config_data]
    ...
    with pytest.raises(ValueError, match="評価対象の音声ファイルが見つかりません"):
        run_grid_search(str(temp_dir / "c.yaml"), str(temp_dir / "g.yaml"), str(temp_dir / "o"))
    mock_dependencies['logger'].warning.assert_not_called() # エラーになるので Warning は出ないはず
...
def test_run_grid_search_no_ref_files_found(mock_dependencies, temp_dir):
    ...
    mock_dependencies['logger'].warning.assert_any_call("参照ファイルが見つかりません: audio1.*")
...
def test_run_grid_search_run_evaluation_fails(mock_dependencies, temp_dir):
    ...
    mock_dependencies['run_eval'].return_value = {'status': 'error', 'error': 'Eval failed miserably'}

    result_data = run_grid_search(str(temp_dir / "c.yaml"), str(temp_dir / "g.yaml"), str(temp_dir / "o"))
    ...
    mock_dependencies['logger'].error.assert_any_call("実行 run_1 で評価エラーが発生しました: Eval failed miserably")
    mock_dependencies['logger'].warning.assert_any_call("有効な評価結果が見つからなかったため、最適なパラメータを決定できませんでした。")
    # 結果の保存は行われるはず
    assert mock_dependencies['open'].call_count >= 2 # JSON & CSV

@patch('traceback.format_exc', return_value="Traceback...") # traceback もモック
def test_run_grid_search_unexpected_exception(mock_traceback, mock_dependencies, temp_dir):
    ...
    mock_dependencies['run_eval'].side_effect = [
        {'summary': {'TestDetector':{'note': {'f_measure': 0.8}}}, 'status': 'success'},
    ...
    mock_dependencies['logger'].error.assert_any_call("実行 run_2中に予期せぬエラーが発生しました: Unexpected crash\nTraceback...")
...
def test_run_grid_search_invalid_best_metric_format(mock_dependencies, temp_dir):
    ...
    mock_dependencies['run_eval'].return_value = {'summary': {'TestDetector':{'note': {'f_measure': 0.9}}}, 'status': 'success'}
    ...
    mock_dependencies['logger'].warning.assert_any_call("不正な best_metric 形式: invalid-format。'category.metric' 形式で指定してください。デフォルトの 'note.f_measure' を使用します。")
...
def test_run_grid_search_metric_not_found_in_results(mock_dependencies, temp_dir):
    ...
    mock_dependencies['run_eval'].return_value = {'summary': {'TestDetector':{'onset': {'precision': 0.9}}}, 'status': 'success'}
...
def test_run_grid_search_save_results_error(mock_dependencies, temp_dir):
    ...
    # エラーログが出力されるはず
    mock_dependencies['logger'].error.assert_any_call("結果ファイルの保存中にエラーが発生しました: Cannot write results")
