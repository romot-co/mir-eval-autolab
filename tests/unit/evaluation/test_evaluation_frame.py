...
def mock_mir_eval_melody():
    ...
    with patch(
        "src.evaluation.evaluation_frame.mir_eval.melody", spec=True
    ) as mock_melody:
        # to_cent_voicing のモック: voicing 配列も返すように
        # (ref_voiced, ref_cents, est_voiced, est_cents) のタプルを返す
        def mock_to_cent_voicing_side_effect(
            ref_times,
            ref_freqs,
            est_times,
            est_freqs,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            None,
            7,
            [0.0, 440.0, 440.0, 0.0, 0.0, 220.0, 220.0],
        ),
...
def test_notes_to_frames(
    note_intervals,
    note_pitches,
    hop_time,
    end_time,
    expected_times_len,
    expected_freqs_pattern,
):
    """
    ...
    assert (
        len(result_times) == expected_times_len
    ), f"Expected {expected_times_len} frames, got {len(result_times)}"
    ...
    if expected_times_len > 0:
        ...
        np.testing.assert_allclose(
            result_times, expected_times, err_msg="Times array mismatch"
        )
        if end_time is not None:
            assert result_times[-1] < (
                end_time + 1e-9
            )  # 最後の時刻がend_timeを超えない
    ...
    np.testing.assert_allclose(
        ...
        rtol=1e-5,
        atol=1e-5,
        err_msg="Frequencies array mismatch",
    )
...
def test_evaluate_frame_pitches_empty_arrays(mock_mir_eval_melody, caplog):
    ...
    # ケース3: 両方が空
    metrics_both_empty = evaluate_frame_pitches(
        empty_time, empty_freq, empty_time, empty_freq
    )
...
def test_evaluate_frame_pitches_length_mismatch(mock_mir_eval_melody, caplog):
    ...
    # ケース1: refの長さ不一致
    metrics_ref_mismatch = evaluate_frame_pitches(
        ref_time_bad, ref_freq_bad, est_time_ok, est_freq_ok
    )
    ...
    # 警告ログを確認
    assert any(
        "参照データの時刻数とピッチ数が一致しません" in rec.message
        for rec in caplog.records
    )
    caplog.clear()

    # ケース2: estの長さ不一致
    metrics_est_mismatch = evaluate_frame_pitches(
        ref_time_ok, ref_freq_ok, est_time_bad, est_freq_bad
    )
    ...
    assert any(
        "推定データの時刻数とピッチ数が一致しません" in rec.message
        for rec in caplog.records
    )
    caplog.clear()

    # ケース3: 両方の長さ不一致
    metrics_both_mismatch = evaluate_frame_pitches(
        ref_time_bad, ref_freq_bad, est_time_bad, est_freq_bad
    )
...
def test_evaluate_frame_pitches_mir_eval_exception(mock_mir_eval_melody, caplog):
    ...
    mock_mir_eval_melody.to_cent_voicing.side_effect = ValueError(
        "Test Exception from mir_eval"
    )
    ...
    # エラーログが出力されていることを確認
    assert any(
        "フレーム評価中にエラーが発生しました: Test Exception from mir_eval"
        in rec.message
        for rec in caplog.records
    )
...
def test_evaluate_frame_pitches_tolerance_argument(mock_mir_eval_melody):
    ...
    args, kwargs = mock_mir_eval_melody.raw_pitch_accuracy.call_args
    assert (
        "cent_tolerance" in kwargs and kwargs["cent_tolerance"] == 50.0
    )  # デフォルト値を確認
