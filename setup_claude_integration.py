...
def get_default_claude_config_path() -> Optional[str]:
    ...
    if system == "Darwin":
        ...
        if app_data:
            ...
            logger.warning("WindowsのAPPDATA環境変数が見つかりません。config.jsonのパスを特定できませんでした。")
            return None
    elif system == "Linux":
...
def setup_claude_desktop_config(config_path_override: Optional[str] = None):
    ...
    if not config_path:
        logger.error("Claude設定ファイル(config.json)のパスを特定できませんでした。--config-pathで指定してください。")
        return False
    ...
    if not config_path_obj.exists():
        ...
        try:
         ...
         logger.error(f"設定ファイルまたはディレクトリへの書き込み権限がありません: {config_path_obj}")
         return False
    ...
    try:
        ...
        logger.warning("設定ファイルが有効なJSONではありません。新しい設定で開始します。")
    except Exception as e:
...
def update_claude_config(current_config: Dict[str, Any], mirex_path_str: str) -> Dict[str, Any]:
    ...
    setup_workflows(config["workflows"], mirex_path_str) # ワークフロー設定関数を呼び出し
...
def setup_prompt_templates(config: Dict[str, Any], mirex_path_str: str):
    """プロンプトテンプレートの設定"""
    if "promptTemplates" not in config: config["promptTemplates"] = []
    ...
    mirex_templates = [
        {"name": "MIREX開発モード", "description": "MIREXプロジェクトの開発者モード", "isDefault": True, "text": f"MIREXアルゴリズム開発プロジェクトについての質問に答えてください。\nあなたは音楽情報抽出技術の専門家です。\nMIR（Music Information Retrieval）の技術について詳しく説明してください。\n\n作業ディレクトリ: {mirex_path_str}"},
        {"name": "MIREX検出器改善", "description": "検出器アルゴリズムの改善依頼用", "text": f"以下の音楽検出器アルゴリズムを改善したいと思います。\n問題点を分析して、具体的な改善案を提案してください。\n特に、[問題点]のセクションに対処する方法を教えてください。\n\n作業ディレクトリ: {mirex_path_str}\n\n検出器名: {{detector_name}}\n\n問題点:\n{{problem_description}}\n\n現在のコード:\n```python\n{{current_code}}\n```"},
        {"name": "MIREX評価結果分析", "description": "検出器の評価結果を分析", "text": f"以下の音楽検出器の評価結果を分析し、パフォーマンスの問題点と改善案を提案してください。\n評価メトリクスの意味と、それらから読み取れる問題点を詳しく説明してください。\n\n作業ディレクトリ: {mirex_path_str}\n\n検出器名: {{detector_name}}\n\n評価結果:\n```\n{{evaluation_result}}\n```"},
        {"name": "音楽信号処理Q&A", "description": "音楽信号処理に関する質問", "text": "音楽信号処理、特に以下の質問について専門的に回答してください:\n\n質問: {{question}}\n\n具体的なコード例やアルゴリズムの説明もあると助かります。"},
        {"name": "グリッドサーチ結果分析", "description": "パラメータグリッドサーチ結果の分析", "text": f"以下の検出器パラメータのグリッドサーチ結果を分析してください。\n最適なパラメータ組み合わせとその理由、およびパラメータ変化によるパフォーマンスへの影響を説明してください。\n\n作業ディレクトリ: {mirex_path_str}\n\n検出器名: {{detector_name}}\n\nグリッドサーチ結果:\n```\n{{grid_search_result}}\n```"},
        # 他のテンプレートも同様に追加...
    ]
    new_templates = []
    processed_names = set()
    for template in mirex_templates:
        ...
        if name in existing_templates:
            ...
            if "isDefault" in template: existing["isDefault"] = template["isDefault"]
...
def setup_custom_instructions(config: Dict[str, Any], mirex_path_str: str):
    """カスタム指示の設定"""
    if "customInstructions" not in config: config["customInstructions"] = {}
...
""".strip()
    config["customInstructions"]["text"] = custom_instructions # 常に上書きする形でシンプル化
...
def setup_workflows(workflows_config: Dict[str, Any], mirex_path_str: str):
    ...
    workflows_config["mirex_improve_algorithm"] = {
        ...
        "steps": [
            {"id": "start_session", "name": "セッション開始", "description": "改善セッションを開始します", "tool": "mcp", "toolName": "start_session", "parameters": {"base_algorithm": "{detector_name}"}},
            {"id": "get_code", "name": "コード取得", "description": "検出器のコードを取得します", "tool": "mcp", "toolName": "get_code", "parameters": {"detector_name": "{detector_name}"}},
            {"id": "run_evaluation", "name": "初期評価", "description": "検出器の初期評価を実行します", "tool": "mcp", "toolName": "run_evaluation", "parameters": {"detector_name": "{detector_name}"}},
            {"id": "improve_code", "name": "コード改善", "description": "AIを使ってコードを改善します", "tool": "mcp", "toolName": "improve_code", "parameters": {"prompt": "以下の検出器を改善してください: {improvement_goal}", "session_id": "{session_id}"}},
            {"id": "evaluate_improved", "name": "改善評価", "description": "改善されたコードを評価します", "tool": "mcp", "toolName": "run_evaluation", "parameters": {"detector_name": "{detector_name}_improved"}}
        ]
    }
...
def setup_mcp_servers(config: Dict[str, Any], mirex_path_str: str):
    ...
    venv_python_path_win = Path(mirex_path_str) / "venv" / "Scripts" / "python.exe" # Windows
    ...
    if venv_python_path.exists():
        ...
        logger.warning(f"仮想環境のPythonが見つかりません ({venv_python_path} or {venv_python_path_win})。現在のPython ({sys.executable}) を使用します。")

    # デフォルトのMCPポート
    mcp_port = os.environ.get("MCP_PORT", "5002")

    # メインワークスペースパス
    main_workspace = os.environ.get("MIREX_WORKSPACE", str(MIREX_PROJECT_ROOT / "mcp_workspace")) # Pathを文字列に
    ...
    config["mcpServers"]["mirex-auto-improver"] = { # サーバー名を指定
        ...
        "env": {
            ...
            "ANTHROPIC_API_KEY": os.environ.get("ANTHROPIC_API_KEY", "") # 念のため渡す
        },
    ...
    logger.info(f"MCPサーバー設定を更新/追加しました: mirex-auto-improver (Port: {mcp_port})")
...
def open_claude_desktop():
    ...
    try:
        if platform.system() == "Darwin":
            ...
            subprocess.run(["xdg-open", "claude://"], check=True) # claude:// URI スキームを試す
        else:
        ...
        return True
         ...
         logger.error("Claude Desktopアプリケーションが見つかりません。パスが通っているか確認してください。")
         return False
    except Exception as e:
...
def main():
    ...
    parser.add_argument("--config-path", type=str, help=f"Claude設定ファイル(config.json)のパス (デフォルト: {DEFAULT_CONFIG_FILE or 'OS依存'})")
