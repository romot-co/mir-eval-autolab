...
from src.utils.exception_utils import FileError, SynthesizerError, log_exception
...
def ensure_output_dirs():
    ...
    logging.info(
        f"出力ディレクトリを作成/確認しました: {OUTPUT_AUDIO_DIR}, {OUTPUT_LABEL_DIR}"
    )
...
def generate_adsr_envelope(
    ...
    sustain_samples = max(
        0, duration_samples - attack_samples - decay_samples - release_samples
    )
    ...
    if decay_samples > 0:
        envelope[attack_samples : attack_samples + decay_samples] = np.linspace(
            1, sustain_level, decay_samples, endpoint=False
        )
    envelope[
        attack_samples
    ...
    if release_samples > 0:
        envelope[attack_samples + decay_samples + sustain_samples :] = np.linspace(
            sustain_level, 0, release_samples, endpoint=True
        )
    if total_samples < duration_samples:
        envelope = np.pad(
            envelope,
            (0, duration_samples - total_samples),
            "constant",
            constant_values=(0,),
        )
    elif total_samples > duration_samples:
        envelope = envelope[:duration_samples]
    return envelope


def generate_sine_wave(
    freq: float, duration_samples: int, amp: float = 1.0, sr: int = SR
) -> np.ndarray:
    if freq <= 0:
        logging.debug(
            f"Frequency {freq:.2f} <= 0, returning zeros for {duration_samples} samples."
        )
        return np.zeros(duration_samples)
    assert duration_samples >= 0, "Duration must be non-negative"
    t = np.linspace(0.0, duration_samples / sr, duration_samples, endpoint=False)
...
def generate_harmonic_tone(
    ...
    if base_freq <= 0:
        logging.debug(
            f"Base frequency {base_freq:.2f} <= 0, returning zeros for {duration_samples} samples."
        )
        return tone
    assert duration_samples >= 0, "Duration must be non-negative"
    ...
    for harmonic_num, harmonic_amp in harmonics:
        ...
        assert (
            harmonic_amp >= 0
        ), f"Harmonic amplitude must be non-negative: {harmonic_amp}"
        freq = base_freq * harmonic_num
        tone += generate_sine_wave(freq, duration_samples, harmonic_amp, sr)
        total_harmonic_amp += harmonic_amp
    if total_harmonic_amp > 1.0:
        tone /= total_harmonic_amp  # Normalize
    return amp * tone


def save_audio_and_label(
    filename_base: str,
    ...
    if not np.all(np.isfinite(audio_data)):
        logging.error(
            f"音声データに NaN または Inf が含まれています: {filename_base}。0に置換します。"
        )
    ...
    if peak_amp > 1e-9:
        ...
        logging.warning(
            f"オーディオ信号のピーク振幅が非常に小さいです: {filename_base}。正規化はスキップされました。"
        )
    ...
    try:
        ...
        log_exception(
            ...
            file_err,
            f"音声ファイルの保存中にIO/Permissionエラー: {audio_path}",
        ...
        log_exception(
            logging.getLogger(__name__),
            e,
            f"音声ファイルの保存中に予期せぬエラー: {audio_path}",
            log_level=logging.ERROR,
        )
    ...
    try:
        if audio_saved_successfully and os.path.exists(audio_path):
            ...
            if file_size == 0:
                logging.error(
                    f"音声ファイルは作成されましたが空です (0 バイト): {audio_path}"
                )
            else:
                logging.debug(
                    f"音声ファイル保存確認OK: {audio_path}, サイズ: {file_size} バイト"
                )
        elif audio_saved_successfully:
            logging.error(
                f"音声保存関数は成功しましたが、ファイルが存在しません: {audio_path}"
            )
    except (IOError, PermissionError) as verify_err:
        log_exception(
            logging.getLogger(__name__),
            verify_err,
            f"音声ファイルの存在/サイズ確認中にIO/Permissionエラー: {audio_path}",
            log_level=logging.ERROR,
        )
        ...
        log_exception(
            logging.getLogger(__name__),
            ve,
            f"音声ファイルの存在確認中に予期せぬエラー: {audio_path}",
            log_level=logging.ERROR,
        )
    ...
    try:
        ...
        with open(label_path, "w", newline="") as csvfile:
            ...
            if not labels:
                logging.warning(
                    f"{filename_base} にラベルがありません。CSVは空またはヘッダーのみになります。"
                )
            for onset, offset, freq_or_marker in labels:
                freq_str = (
                    f"{freq_or_marker:.3f}"
                    if isinstance(freq_or_marker, (float, np.floating))
                    else str(freq_or_marker)
                )
        ...
        log_exception(
            logging.getLogger(__name__),
            file_err,
            f"ラベルファイルの保存中にIO/Permissionエラー: {label_path}",
            log_level=logging.ERROR,
        )
        ...
        log_exception(
            logging.getLogger(__name__),
            csv_err,
            f"ラベルファイルのCSV書き込みエラー: {label_path}",
            log_level=logging.ERROR,
        )
        ...
        log_exception(
            logging.getLogger(__name__),
            e,
            f"ラベルファイルの保存中に予期せぬエラー: {label_path}",
            log_level=logging.ERROR,
        )
    ...
    try:
        if label_saved_successfully and os.path.exists(label_path):
            ...
            if file_size == 0 and labels:
                logging.error(
                    f"ラベルファイルは作成されましたが空です (0 バイト): {label_path} (ラベル数: {len(labels)})"
                )
            elif file_size == 0 and not labels:
                logging.debug(
                    f"ラベルファイルは空ですが、ラベルデータもありません: {label_path}"
                )
            else:
                logging.debug(
                    f"ラベルファイル保存確認OK: {label_path}, サイズ: {file_size} バイト"
                )
        elif label_saved_successfully:
            logging.error(
                f"ラベル保存関数は成功しましたが、ファイルが存在しません: {label_path}"
            )
    except (IOError, PermissionError) as verify_err:
        log_exception(
            logging.getLogger(__name__),
            verify_err,
            f"ラベルファイルの存在/サイズ確認中にIO/Permissionエラー: {label_path}",
            log_level=logging.ERROR,
        )
    except Exception as ve:
        log_exception(
            logging.getLogger(__name__),
            ve,
            f"ラベルファイルの存在確認中に予期せぬエラー: {label_path}",
            log_level=logging.ERROR,
        )
...
def add_white_noise(audio_data: np.ndarray, snr_db: float) -> np.ndarray:
    ...
    if snr_linear < 1e-9:  # Corresponds to SNR < -90dB, effectively infinite noise
        logging.warning(
            f"SNR {snr_db}dB is very low (linear {snr_linear:.2e}). Noise might dominate or cause issues. Returning original signal."
        )
    ...
    if (
        noise_power > 1e6 * signal_power
    ):  # Heuristic check: if noise power is million times signal power
        logging.warning(
            f"Calculated noise power ({noise_power:.2e}) is extremely high relative to signal power ({signal_power:.2e}) for SNR {snr_db}dB. Clipping noise or returning original signal might be needed."
        )
...
def generate_simple_ir(
    duration_sec: float, decay_rate: float, sr: int = SR
) -> np.ndarray:
...
def apply_reverb(
    audio_data: np.ndarray, ir: np.ndarray, mix_level: float = 0.3
) -> np.ndarray:
    """
    ...
    if len(ir) == 0 or mix_level <= 0:
        logger.debug(
            "IRが空またはミックスレベルが0以下なのでリバーブ適用をスキップします。"
        )
        return audio_data
    if mix_level > 1.0:
        logger.warning(
            f"ミックスレベル {mix_level} が1.0を超えています。1.0にクリップします。"
        )
        mix_level = 1.0
    ...
    try:
        ...
        log_exception(
            logger,
            ve,
            f"fftconvolve中にValueErrorが発生しました。IR Length: {len(ir)}, Audio Length: {len(audio_data)}",
            log_level=logging.ERROR,
        )
        ...
        log_exception(
            logger,
            me,
            f"fftconvolve中にMemoryErrorが発生しました。IR Length: {len(ir)}, Audio Length: {len(audio_data)}",
            log_level=logging.CRITICAL,
        )
        raise SynthesizerError(
            f"Convolution failed due to insufficient memory: {me}"
        ) from me
    except Exception as e:
        # その他の予期せぬエラー
        log_exception(
    ...
    if len(reverb_signal) != target_len:
        logger.warning(
            f"畳み込み結果の長さが期待値と異なります。期待値: {target_len}, 実際: {len(reverb_signal)}。調整します。"
        )
        if len(reverb_signal) < target_len:
...
def generate_kick_sound(
    ...
    t = np.linspace(0.0, duration_sec, duration_samples, endpoint=False)
...
def generate_noise_perc_sound(
    ...
    if bandpass_freqs:
        ...
        assert (
            0 < low < high < nyquist
        ), f"Invalid bandpass frequencies: {low}, {high} for sr={sr}"
        ...
        if low_norm >= high_norm:
            ...
            logging.warning(
                f"Adjusted high norm frequency to be slightly above low norm: {low_norm=}, {high_norm=}"
            )
    ...
    t = np.linspace(
        0.0, duration_sec, duration_samples, endpoint=False
    )  # Add this line
    envelope = np.exp(-decay_rate * t)
    perc = noise * envelope
    return perc * amp
...
def generate_formant_filter(
    formant_freqs: List[float], bandwidths: List[float], sr: int = SR
):
    assert len(formant_freqs) == len(
        bandwidths
    ), "Formant frequencies and bandwidths must have the same length"
...
def apply_formant_filter(
    audio_data: np.ndarray, filter_response: np.ndarray, sr: int = SR
) -> np.ndarray:
    ...
    if len(audio_fft) != len(filter_response):
        ...
        logging.warning(
            f"FFT length ({len(audio_fft)}) and filter response length ({len(filter_response)}) mismatch. Resizing filter."
        )
        ...
        if target_len > len(filter_response):
            filter_response = np.pad(
                filter_response, (0, target_len - len(filter_response))
            )
    else:
...
def generate_basic_sine_sequence(filename_base: str = "1_basic_sine", sr: int = SR):
    ...
    for midi_note, duration_sec in notes:
        ...
        if write_len > 0:
            audio[current_time_samples:write_end] += note_wave[:write_len]
...
def generate_dynamics_change(
    filename_base: str = "3_dynamics_change", midi_note: int = 67, sr: int = SR
):
    duration_sec = 6.0
    duration_samples = int(duration_sec * sr)
    freq = midi_to_hz(midi_note)
    ...
    dynamic_env = np.concatenate(
        [np.linspace(0.1, 1.0, cres_samples), np.linspace(1.0, 0.1, decres_samples)]
    )
...
def generate_pitch_modulation(
    filename_base: str = "4_pitch_mod", midi_note: int = 69, sr: int = SR
):
    duration_sec = 6.0
    duration_samples = int(duration_sec * sr)
    ...
    freq_mod_vib_segment = base_freq + vib_depth_hz * np.sin(
        2 * np.pi * vib_rate * t_vib
    )
    ...
    freq_mod_port_segment = np.linspace(
        portamento_start_freq, target_freq, port_samples
    )
    freq_mod_combined = np.concatenate((freq_mod_vib_segment, freq_mod_port_segment))
    if len(freq_mod_combined) != duration_samples:
        logging.warning(
            f"Pitch mod length mismatch: {len(freq_mod_combined)} vs {duration_samples}"
        )
        if abs(len(freq_mod_combined) - duration_samples) < 5:
            freq_mod_combined = np.resize(freq_mod_combined, duration_samples)
        else:
            logging.error("Cannot generate pitch mod")
            return
    ...
    labels: List[Label] = [
        ...
        (
            vib_samples / sr,
            final_offset_time_sec,
            (portamento_start_freq + target_freq) / 2,
        ),  # Approx pitch for portamento
    ]
    save_audio_and_label(filename_base, audio, labels, sr=sr)
...
def generate_reverb_sequence(filename_base_prefix: str = "6_reverb", sr: int = SR):
    base_audio, labels = _generate_harmonic_sequence_audio_labels(sr=sr)
    reverb_settings = {
        "short": {"duration": 0.5, "decay": 10.0, "mix": 0.2},
        "long": {"duration": 1.5, "decay": 3.0, "mix": 0.35},
    }
    for name, settings in reverb_settings.items():
        filename_base = f"{filename_base_prefix}_{name}"
        ir = generate_simple_ir(
            duration_sec=settings["duration"], decay_rate=settings["decay"], sr=sr
        )
...
def generate_chords(filename_base: str = "7_chords", sr: int = SR):
    ...
    for midi_notes, duration_sec in chord_sequence:
        ...
        perceived_release_duration_sec = release_time_sec * 0.8
        offset_time_sec = (
            current_time_samples + chord_duration_samples
        ) / sr + perceived_release_duration_sec

        for midi_note in midi_notes:
            freq = midi_to_hz(midi_note)
            tone = generate_harmonic_tone(
                freq, envelope_samples, harmonics, amp=1.0 / len(midi_notes), sr=sr
            )
            chord_wave += tone
...
def generate_percussion_only(filename_base: str = "9_10_percussion_only", sr: int = SR):
    ...
    for i in range(int(duration_sec / (beat_duration * 2))):
        ...
        if end_sample <= duration_samples:
            audio[onset_sample:end_sample] += kick_sound
            labels.append((onset_time, onset_time + 0.1, 0.0))
    ...
    for i in range(int(duration_sec / (beat_duration * 2))):
        ...
        if end_sample <= duration_samples:
            audio[onset_sample:end_sample] += snare_sound
            labels.append((onset_time, onset_time + 0.1, 0.0))
    hh_sound = generate_noise_perc_sound(
        bandpass_freqs=(5000, 15000), decay_rate=50.0, amp=0.5, sr=sr
    )
    hh_len = len(hh_sound)
    for i in range(int(duration_sec / (beat_duration / 2))):
        onset_time = i * beat_duration / 2
        ...
        if end_sample <= duration_samples:
            audio[onset_sample:end_sample] += hh_sound
            labels.append((onset_time, onset_time + 0.05, 0.0))
    labels.sort()
    save_audio_and_label(filename_base, audio, labels, sr=sr)


def generate_mixed_melody_percussion(
    filename_base: str = "11_mixed_melody_perc", sr: int = SR
):
    ...
    for i in range(int(duration_sec / (beat_duration * 2))):
        ...
        if end_sample <= duration_samples:
            audio[onset_sample:end_sample] += kick_sound
            labels.append((onset_time, onset_time + 0.1, 0.0))
    snare_sound = generate_noise_perc_sound(
        bandpass_freqs=(200, 1000), amp=0.7 * perc_amp_scale, sr=sr
    )
    snare_len = len(snare_sound)
    for i in range(int(duration_sec / (beat_duration * 2))):
        onset_time = (i * 2 + 1) * beat_duration
    ...
    for i in range(int(duration_sec / beat_duration)):
        if i % 2 == 0:  # Kick on even beats
            ...
            if end_sample <= duration_samples:
                ...
                labels.append((onset_time, onset_time + 0.1, 0.0))
    hh_sound = generate_noise_perc_sound(
        bandpass_freqs=(6000, 16000), decay_rate=60.0, amp=0.4 * perc_amp_scale, sr=sr
    )
    hh_len = len(hh_sound)
    for i in range(int(duration_sec / (beat_duration / 2))):
        onset_time = i * beat_duration / 2
...
def generate_smooth_vibrato(
    filename_base: str = "13_smooth_vibrato", midi_note: int = 69, sr: int = SR
):
    ...
    for h_num, h_amp in harmonics:
        ...
        total_amp += h_amp
    if total_amp > 0:
        harmonic_tone /= total_amp  # Normalize harmonic amplitudes
...
def generate_portamento(
    filename_base: str = "14_portamento",
    start_midi: int = 60,
    end_midi: int = 72,
    sr: int = SR,
):
    ...
    single_label: List[Label] = [
        (adsr_params["attack"], duration_sec, (start_freq + end_freq) / 2)
    ]

    save_audio_and_label(filename_base, audio, single_label, sr=sr)


def generate_slow_attack(
    filename_base: str = "15_slow_attack", midi_note: int = 65, sr: int = SR
):
    ...
    audio = audio[
        :duration_samples
    ]  # Trim to exact duration before normalization in save

    # Label onset starts at the beginning of the attack phase
    labels: List[Label] = [
        (0.0, duration_sec - adsr_params["release"], freq)
    ]  # Offset before release starts
...
def generate_staccato(filename_base: str = "16_staccato", sr: int = SR):
    ...
    audio, labels = _generate_melody_audio_labels(
        notes, harmonics, adsr_params, sr=sr, gap_sec=0.05
    )
...
def generate_pianissimo(filename_base: str = "17_pianissimo", sr: int = SR):
    ...
    audio, labels = _generate_melody_audio_labels(
        notes, harmonics, adsr_params, amp_scale=0.05, sr=sr
    )
...
def generate_octave_errors(filename_base: str = "18_octave_errors", sr: int = SR):
    ...
    audio, labels = _generate_melody_audio_labels(
        notes, harmonics, adsr_params, sr=sr, gap_sec=0.02
    )
...
def generate_inharmonicity(
    filename_base: str = "19_inharmonicity", midi_note: int = 60, sr: int = SR
):
    ...
    for freq, amp in harmonics_inharmonic:
        ...
        if freq > 0:
            ...
            total_amp += amp
    if total_amp > 0:
        tone /= total_amp  # Normalize amplitudes
...
def generate_vocal_imitation(filename_base: str = "21_vocal_imitation", sr: int = SR):
    ...
    try:  # Add try block for the main generation loop
        for i, (onset, offset, midi_note) in enumerate(notes):
            assert (
                0 <= onset < offset <= total_duration_sec
            ), f"Invalid note timing: {onset}, {offset}"
            ...
            ), f"Note duration must be positive: {note_duration_samples}"
            ...
            pitch_mod_cents = lfo_amp_cents * np.sin(
                2 * np.pi * lfo_freq_pitch * t_global + lfo_phase_offset_pitch
            )
            ...
            current_phase = instantaneous_phase[
                -1
            ]  # Store last phase for next note (smoother transitions?)
            ...
            if total_harmonic_amp < 1e-9:
                total_harmonic_amp = 1.0  # Avoid div by zero

            for harmonic_num, harmonic_amp in harmonics:
                # Use instantaneous phase for each harmonic
                note_wave += (harmonic_amp / total_harmonic_amp) * np.sin(
                    harmonic_num * instantaneous_phase
                )

            # Apply ADSR envelope
            envelope = generate_adsr_envelope(
                note_duration_samples, **adsr_params, sr=sr
            )
            note_wave *= envelope

            # Apply amplitude LFO (tremolo)
            amp_mod = 1.0 - lfo_amp_level * (
                0.5
                * (
                    1
                    + np.sin(2 * np.pi * lfo_freq_amp * t_global + lfo_phase_offset_amp)
                )
            )
            note_wave *= amp_mod
            ...
            if write_len > 0:
                if write_len == len(note_wave):
                    ...
                    logging.warning(
                        f"Note {i} wave truncated during write ({len(note_wave)} -> {write_len})"
                    )
                    audio_out[write_start:write_end] += note_wave[:write_len]
                else:  # write_len > len(note_wave) - should not happen with clip
                    logging.warning(
                        f"Note {i} wave shorter than write segment ({len(note_wave)} vs {write_len})"
                    )
                    audio_out[write_start : write_start + len(note_wave)] += note_wave
        else:
            logging.warning(
                f"Calculated zero write length for note {i} at {onset=}, {offset=}"
            )
        ...
        logging.error(
            f"Assertion failed during vocal imitation generation: {ae}", exc_info=True
        )
        raise SynthesizerError(f"Assertion failed: {ae}") from ae
    except Exception as e:
        logging.error(
            f"Error generating vocal imitation {filename_base}: {e}", exc_info=True
        )
...
def generate_clicks(filename_base: str = "22_clicks", sr: int = SR):
    ...
    base_audio, labels = _generate_harmonic_sequence_audio_labels(sr=sr)
    # Add clicks
    audio_with_clicks = add_click_noise(
        base_audio.copy(), click_prob=0.002, click_amp=0.6, sr=sr
    )
...
def generate_pitch_bend(
    filename_base: str = "14b_pitch_bend",
    midi_note: int = 60,
    bend_range_cents: float = 80.0,
    sr: int = SR,
):
    ...
    if (
        max_phase_delta > np.pi
    ):  # If instantaneous freq exceeds Nyquist, can cause aliasing/instability
        logging.warning(
            f"Maximum phase delta {max_phase_delta:.4f} exceeds pi. Potential instability/aliasing."
        )
    ...
    # Apply envelope
    envelope = generate_adsr_envelope(
        duration_samples, attack=0.1, decay=0.2, sustain_level=0.8, release=0.5, sr=sr
    )
...
def _generate_melody_audio_labels(
    ...
    labels: List[Label] = []
    current_time_samples = 0
    release_time_sec = adsr_params.get(
        "release", 0.1
    )  # Get release time for buffer calc
    ...
    for i, (midi_note, duration_sec) in enumerate(notes):
        ...
        envelope = generate_adsr_envelope(envelope_samples, **adsr_params, sr=sr)
        tone = generate_harmonic_tone(
            freq, envelope_samples, harmonics, amp=amp_scale, sr=sr
        )
        ...
        end_sample = (
            current_time_samples + note_duration_samples
        )  # Theoretical end of note sustain
        envelope_end_sample_in_context = current_time_samples + len(note_wave)
        ...
        current_time_samples += (
            note_duration_samples  # Move time based on sustain duration
        )
...
def _generate_harmonic_sequence_audio_labels(
    sr: int = SR,
) -> Tuple[np.ndarray, List[Label]]:
    notes = [
        (60, 0.3),
...
def _generate_polyphony_audio_labels(sr: int = SR) -> Tuple[np.ndarray, List[Label]]:
    ...
    audio1, labels1 = _generate_melody_audio_labels(
        voice1_notes, voice1_harmonics, voice1_adsr, amp_scale=0.6, sr=sr
    )
    audio2, labels2 = _generate_melody_audio_labels(
        voice2_notes, voice2_harmonics, voice2_adsr, amp_scale=0.7, sr=sr
    )
