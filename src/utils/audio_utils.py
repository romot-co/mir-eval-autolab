...
def load_audio_file(
    audio_path: str, logger: Optional[logging.Logger] = None
    ...
    try:
        ...
        if logger:
            logger.info(
                f"音声ファイルを読み込みました: {audio_path} (sr={sr}Hz, length={len(audio)/sr:.2f}秒)"
            )

        return audio, sr
    except Exception as e:
        error_msg = (
            f"音声ファイル '{audio_path}' の読み込み中にエラーが発生しました: {e}"
        )
        if logger:
...
def load_reference_data(ref_path: str) -> Dict[str, np.ndarray]:
    ...
    try:
        if ext == ".csv":
            ...
            if not has_expected_columns:
                try:
                    ...
                    if len(df.columns) >= 3:
                        # 位置に基づいて列名を割り当てる
                        df.columns = ["onset", "offset", "pitch"] + [
                            f"col_{i}" for i in range(3, len(df.columns))
                        ]
                        logger.warning(
                            f"列名が見つからないか不正なため、位置でデータを判断します: {ref_path}"
                        )
                    else:
                        # 列数が足りない場合はエラー
                        raise ValueError(
                            f"ヘッダーなしで読み込んだが、列数が3未満です: {len(df.columns)}"
                        )
                except Exception as e_no_header:
                    logger.error(
                        f"CSVファイルの読み込みに失敗しました (ヘッダー有無両方): {ref_path}, Error: {e_no_header}"
                    )
                    return {
            ...
            if (
                ...
                logger.warning(
                    f"列名が見つからないため、位置でデータを判断します: {df.columns.tolist()}"
                )
                ...
                logger.error(
                    f"CSVファイルのフォーマットが不正です (列数不足): {ref_path}"
                )
                return {
                    "intervals": np.array([]).reshape(0, 2),
                    "note_pitches": np.array([]),
            ...
            if (
                min_len < len(onsets)
                or min_len < len(offsets)
                or min_len < len(pitches)
            ):
                logger.warning(
                    f"データ長が一致しません (onsets: {len(onsets)}, offsets: {len(offsets)}, pitches: {len(pitches)})。最小値に合わせます: {min_len}"
                )
            ...
            return {"intervals": intervals, "note_pitches": pitches}
            ...
            if "intervals" in data:
                ...
                if "note_pitches" in data:
                    ...
                    logger.warning(
                        f"JSONファイルにピッチデータがありません: {ref_path}"
                    )
                    pitches = np.array([])
        ...
        return {"intervals": np.array([]).reshape(0, 2), "note_pitches": np.array([])}


def load_audio_and_reference(
    audio_path: str, ref_path: str, logger: Optional[logging.Logger] = None
...
def make_output_path(
    output_dir: str, audio_file: str, detector_name: str, extension: str = "json"
) -> str:
    """
    出力ファイルのパスを生成する
