...
def setup_logger(
    ...
    if (
        logger.hasHandlers()
    ):  # .handlersリストを直接チェックするよりhasHandlers()が推奨される
    ...
    if output_dir:
        try:
            ...
            log_file = (
                output_path
                / f'{name}_{datetime.now().strftime("%Y%m%d_%H%M%S_%f")}.log'
            )
            ...
            logger.error(
                f"Failed to set up file handler for logger '{name}': {e}", exc_info=True
            )
