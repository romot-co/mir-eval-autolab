...
class DetectionResult:
    ...
    frame_frequencies: np.ndarray = field(
        default_factory=lambda: np.empty(0, dtype=np.float64)
    )
    ...
    def __post_init__(self):
        ...
        if self.intervals.size > 0:
            ...
            if np.any(self.intervals < 0):
                logger.error(
                    "インターバル時間に負の値が含まれています: %s", self.intervals
                )
                raise ValueError("インターバル時間に負の値が含まれています")
            # 開始時刻 <= 終了時刻 チェック
            if np.any(self.intervals[:, 0] > self.intervals[:, 1]):
                invalid_indices = np.where(self.intervals[:, 0] > self.intervals[:, 1])[
                    0
                ]
                logger.error(
                    "インターバルの開始時刻が終了時刻より後になっています (indices: %s): %s",
                    invalid_indices,
                    self.intervals[invalid_indices],
                )
        ...
        if self.note_pitches.size > 0:
            ...
            if np.any(self.note_pitches < 0):
                ...
                logger.error(
                    "note_pitches に負の値が含まれています: %s", self.note_pitches
                )
        ...
        if self.frame_times.size > 0:
            ...
            if np.any(self.frame_times < 0):
                logger.error(
                    "frame_times に負の値が含まれています: %s", self.frame_times
                )
            ...
            if np.any(diffs < 0):
                ...
                logger.error(
                    "frame_times が単調増加ではありません (indices: %s): %s",
                    invalid_indices,
                    self.frame_times,
                )
        ...
        if self.frame_frequencies.size > 0:
            ...
            if np.any(self.frame_frequencies < 0):
                logger.error(
                    "frame_frequencies に負の値が含まれています: %s",
        ...
        if len(self.intervals) != len(self.note_pitches):
            logger.error(
                f"intervals ({len(self.intervals)}) と note_pitches ({len(self.note_pitches)}) の長さが一致しません。"
            )
        ...
        if len(self.frame_times) != len(self.frame_frequencies):
            ...
            logger.warning(
                f"frame_times ({len(self.frame_times)}) と frame_frequencies ({len(self.frame_frequencies)}) の長さが一致しません。"
            )
    ...
    def from_dict(cls, detection_dict: Dict[str, Any]) -> "DetectionResult":
            ...
            if key in detection_dict:
                ...
                if key in [
                    ...
                    try:
                        ...
                        logger.warning(
                            f"フィールド '{key}' の NumPy 配列(float64)への変換中にエラー: {e}。そのまま渡します。"
                        )
                        init_args[key] = (
                            value  # そのまま渡し、__post_init__での検証に任せる
                        )
                else:
                    init_args[key] = value  # 他の既知フィールドはそのままコピー

        # additional_data を処理 (入力辞書に直接指定されている場合も考慮)
        if "additional_data" in detection_dict and isinstance(
            detection_dict["additional_data"], dict
        ):
        ...
        try:
            ...
            logger.error(
                f"DetectionResult の初期化中に TypeError: {e}. 引数キー: {list(init_args.keys())}"
            )
            ...
            raise ValueError(
                f"DetectionResult の初期化中に予期せぬエラーが発生しました: {e}"
            ) from e
