...
class ECHOEDetector(BaseDetector):
    ...
    def detect(
        ...
        results = {
            ...
            "note_intervals": (
                np.column_stack((notes["onset_times"], notes["offset_times"]))
                if len(notes["onset_times"]) > 0
                else np.array([])
            ),
        }
    ...
    def detect_onsets(self, audio: np.ndarray, sr: int) -> np.ndarray:
        ...
        # オンセット検出のためのパラメータ
        min_frames_between_onsets = int(
            0.05 * sr / self.hop_length
        )  # 最小50msのギャップ
        ...
        for i in range(1, len(self.state_buffer)):
            # 条件1: 非調和状態(N)への遷移
            is_non_harmonic_transition = (self.state_buffer[i] == 2) and (
                self.state_buffer[i - 1] != 2
            )
            ...
            if (is_non_harmonic_transition or is_cd_falling or high_change) and (
                i - last_onset >= min_frames_between_onsets
            ):
    ...
    def detect_offsets(self, audio: np.ndarray, sr: int) -> np.ndarray:
        ...
        while i < len(self.hmm_posterior) - self.offset_min_frames:
            # 条件1: カオス(C)が優勢かどうか
            chaos_dominant = np.all(
                self.hmm_posterior[i : i + self.offset_min_frames, 1]
                > self.offset_chaos_threshold
            )

            # 条件2: 安定(S)と非調和(N)の区別が困難になっているか
            # |π_S - π_N| が小さければ区別困難（埋没状態）
            stability_diff = np.all(
                np.abs(
                    self.hmm_posterior[i : i + self.offset_min_frames, 0]
                    - self.hmm_posterior[i : i + self.offset_min_frames, 2]
                )
                < self.offset_stability_diff
            )
    ...
    def _compute_features(self, audio: np.ndarray, sr: int):
        ...
        for i in range(n_frames):
            ...
            if np.max(acf) < 1e-5:
                ...
                min_lag = max(
                    1, int(sr / self.frame_length / 20)
                )  # 例: 20Hz以上の周期性を探索
                if len(acf) > min_lag + 1:
                    # より低周波の周期性を優先するため、ある程度の範囲でピークを探す
                    max_lag = min(
                        len(acf) - 1, int(sr / self.frame_length / 0.5)
                    )  # 0.5Hz以上
                    ...
                    if peak_idx > 0 and peak_idx < len(acf) - 1:
                        peak_prominence = harmony_score - 0.5 * (
                            acf[peak_idx - 1] + acf[peak_idx + 1]
                        )
                        harmony_score = max(
                            0.1, min(1.0, harmony_score * (1 + peak_prominence))
                        )
                else:
                    harmony_score = 0.2
            ...
            if i > 0:
                ...
                if i >= 1:
                    prev_frame = audio[
                        (i - 1) * self.hop_length : (i - 1) * self.hop_length
                        + self.frame_length
                    ]
                    spec1 = np.abs(np.fft.rfft(frame))
                    spec2 = np.abs(np.fft.rfft(prev_frame))
                    if len(spec1) > 0 and len(spec2) > 0:
                        spec_diff = np.sum(np.abs(spec1 - spec2)) / (
                            np.sum(spec1 + spec2) + 1e-5
                        )
                        # スペクトル変化も急変度に加味
                ...
                if (
                    relative_change > 0.2
                    and envelope_energy > np.mean(prev_energies[:-1]) * 1.5
                ):
        ...
        # デバッグ出力（実際の値の分布を確認）
        print(
            f"協和度(C) 範囲: {min(c_values):.3f}～{max(c_values):.3f}, 平均: {np.mean(c_values):.3f}"
        )
        print(
            f"カオス度(K) 範囲: {min(k_values):.3f}～{max(k_values):.3f}, 平均: {np.mean(k_values):.3f}"
        )
        print(
            f"急変度(D) 範囲: {min(d_values):.3f}～{max(d_values):.3f}, 平均: {np.mean(d_values):.3f}"
        )
    ...
    def _estimate_states(self):
        ...
        if n_frames > 0:
            init_obs_prob = self._compute_observation_probability(
                    ...
                    for j in range(n_states):
                        if j != i:
                            transition_matrix[i, j] += (
                                diff * transition_matrix[i, j] / remaining
                            )
        ...
        for t in range(n_frames):
            ...
            if t == 0:
                ...
                for j in range(n_states):
                    for i in range(n_states):
                        alpha[j] += (
                            self.hmm_posterior[t - 1, i] * transition_matrix[i, j]
                        )
                    alpha[j] *= b[j]
        ...
        print(
            f"状態分布 - S:{states_count[0]}, C:{states_count[1]}, N:{states_count[2]}"
        )
    ...
    def _estimate_pitch_for_segment(
        self, audio: np.ndarray, sr: int, start_frame: int, end_frame: int
        ...
        for frame_idx in range(start_frame, end_frame, step):
            ...
            if n_fft > 32:  # 最低限のFFTサイズチェック
                ...
                if len(frame_segment) >= n_fft:
                    ...
                    # ピッチの範囲内のスペクトルピークを検出
                    in_range_idx = np.where(
                        (freqs >= self.pitch_min_freq) & (freqs <= self.pitch_max_freq)
                    )[0]
                    if len(in_range_idx) > 1:
                        ...
                        peak_idx = signal.find_peaks(
                            in_range_spect,
                            height=np.max(in_range_spect) * 0.3,
                            distance=3,
                        )[0]
                        if len(peak_idx) > 0:
                            ...
                            for idx in sorted_idx:
                                spectral_peaks.append(
                                    (in_range_freqs[peak_idx[idx]], peak_values[idx])
                                )
                        ...
                        if len(acf) > 1:
                            ...
                            if max_lag > min_lag:
                                try:
                                    ...
                                    nearby_vals = acf[
                                        max(0, peak_idx - 3) : min(
                                            len(acf), peak_idx + 4
                                        )
                                    ]
                                    nearby_vals = np.delete(
                                        nearby_vals, 3
                                    )  # ピーク自体を除く
                    ...
                    if stability_scores[idx] > 0.2:
                        ...
                        if peak_lags[idx] > 0:
                            ...
                            combined_candidates.append(
                                (estimated_freq, stability_scores[idx], "acf")
                            )
                        else:
                            # ラグがない場合はバンドの中心周波数を使用
                            center_freq = np.sqrt(bands[idx] * bands[idx + 1])
                            combined_candidates.append(
                                (center_freq, stability_scores[idx] * 0.5, "band")
                            )
            ...
            for freq, magnitude in spectral_peaks:
                ...
                norm_magnitude = magnitude / (
                    np.max([p[1] for p in spectral_peaks]) + 1e-10
                )
            ...
            if combined_candidates:
                ...
                for freq, conf, source in combined_candidates:
                    ...
                    is_new = True
                    for i, (existing_freq, existing_conf, _) in enumerate(
                        merged_candidates
                    ):
                        ...
                        if freq_ratio < 1.05:
                            ...
                            weight2 = conf
                            new_freq = (existing_freq * weight1 + freq * weight2) / (
                                weight1 + weight2
                            )
    ...
    def _match_notes(
        self, audio: np.ndarray, sr: int, onsets: np.ndarray, offsets: np.ndarray
    ) -> Dict[str, np.ndarray]:
        """
        ...
        if len(onsets) == 0:
            ...
            for i in range(1, len(energy_diff)):
                if energy_diff[i] > 1.5 and audio_frames[  # 50%以上のエネルギー増加
                    i
                ] > 0.05 * max(
                    audio_frames
                ):  # 最大の5%以上のエネルギー
        ...
        print(
            f"マッチング前のオンセット数: {len(onsets)}, オフセット数: {len(offsets)}"
        )
        ...
        for onset_idx, onset_time in enumerate(onsets):
            ...
            if offset_idx < len(offsets):
                ...
                if (
                    onset_idx + 1 < len(onsets)
                    and onsets[onset_idx + 1] < offsets[offset_idx]
                ):
                ...
                if end_frame - start_frame >= min_frames:
                    ...
                    if len(pitches) > 0:
                        ...
                        avg_pitch = (
                            np.sum(weighted_pitches) / np.sum(confidences)
    ...
    def _estimate_states_viterbi(self):
        ...
        for t in range(n_frames):
            if self.use_gaussian_obs:
                b[t] = self._compute_observation_probability_gaussian(
                    self.feature_buffer[t]
                )
            else:
        ...
        states_count = np.bincount(self.state_buffer, minlength=3)
        print(
            f"Viterbi状態分布 - S:{states_count[0]}, C:{states_count[1]}, N:{states_count[2]}"
        )

    def _compute_observation_probability_gaussian(
