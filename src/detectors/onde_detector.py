...
class ONDEDetector(BaseDetector):
    ...
    def __init__(self, 
        ...
        # CQTパラメータ
        self.bins_per_octave = bins_per_octave if bins_per_octave is not None else cqt_bins_per_octave
        self.n_bins = cqt_n_bins
        self.f_min = f_min if f_min is not None else (cqt_fmin if cqt_fmin is not None else 65.4)
        self.f_max = f_max if f_max is not None else (cqt_fmax if cqt_fmax is not None else 8000.0)
    ...
    def _detect_pitch(self, audio: np.ndarray, sr: int, 
                     event_results: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        """
        オンセット/オフセットからのピッチ検出
    ...
    def detect(self, audio_data: np.ndarray, sr: int) -> Dict[str, np.ndarray]:
        ...
        # イベント検出（オンセット/オフセット）
        logging.info(f"イベント検出を開始します (audio.shape={audio_data.shape}, sr={sr}Hz)")
        ...
        if self.enable_pitch_detection:
            if event_results['onset_frames'].size > 0:
                logging.info(f"{len(event_results['onset_frames'])}個のイベントに対してピッチ検出を開始します")
                ...
                if len(event_results['onset_frames']) != len(pitches):
                    logging.warning(f"検出結果の長さが一致しません: onsets({len(event_results['onset_frames'])}) vs pitches({len(pitches)})")
                    # 長さを揃える（小さい方に合わせる）
                    min_len = min(len(event_results['onset_frames']), len(pitches))
                    event_results['onset_frames'] = event_results['onset_frames'][:min_len]
                    event_results['offset_frames'] = event_results['offset_frames'][:min_len]
                    event_results['percussive_flags'] = event_results['percussive_flags'][:min_len]
        ...
        intervals = np.column_stack((onsets, offsets)) if len(onsets) > 0 else np.array([]).reshape(0, 2)
        ...
        # 検出完了ログ
        logging.info(f"音符検出が完了しました: {len(onsets)}個の音符を検出 (処理時間: {detection_time:.3f}秒)")
        ...
        return {
            ...
            'frame_frequencies': pitch_values if pitch_values is not None else np.array([]),
    ...
    def detect_pitch_from_events(self, audio: np.ndarray, sr: int, onset_frames: np.ndarray, offset_frames: np.ndarray, percussive_flags: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        既知のイベント位置からピッチを検出する
    ...
    def detect_notes(self, audio: Optional[np.ndarray] = None, sr: Optional[int] = None, audio_path: Optional[str] = None) -> Dict[str, np.ndarray]:
        """
        ...
        try:
            ...
            if audio_path is not None:
                ...
                try:
                    ...
                    logging.info(f"音声ファイルを読み込みました: {audio_path} (sr={sr}Hz, length={len(audio)}samples)")
                except Exception as e:
                    raise RuntimeError(f"音声ファイルの読み込みに失敗しました: {str(e)}")
            elif audio is None or sr is None:
                raise ValueError("audio_pathまたはaudioとsrの両方を指定する必要があります")
            ...
            if not isinstance(results, dict):
                raise RuntimeError(f"detectメソッドが不正な型を返しました: {type(results)}")

            # 検出完了ログ
            logging.info(f"音符検出が完了しました: {len(results.get('intervals', []))}個の音符を検出 (処理時間: {time.time() - start_time:.3f}秒)")
    ...
    # パラメータ感度分析用メソッド
    def analyze_parameter_sensitivity(self, audio_data: np.ndarray, sr: int, 
                                     param_name: str, values: List[float]) -> Dict[str, List]:
        """
        ...
        try:
            for value in values:
                ...
                logging.info(f"パラメータ {param_name}={value} の場合: {event_count}個のイベントを検出 (処理時間: {detection_result['detection_time']:.3f}秒)")
    ...
    def evaluate_with_multiple_parameters(self, audio_data: np.ndarray, sr: int) -> Dict[str, Any]:
        """
        ...
        try:
            for params in parameter_sets:
                ...
                logging.info(f"パラメータ: Theta={params['Theta']}, c_T={params['c_T']}, c_min={params['c_min']} で {result['event_count']}個のイベントを検出 (処理時間: {detection_time:.3f}秒)")
