...
class FAZEDetector(BaseDetector):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.logger.info(
            f"理論上の周波数範囲: {f_min} - {f_max_theory} Hz (bands={bands}, beta={beta})"
        )
    ...
    def _design_filterbank(self, sr: int) -> None:
        ...
        if sr < 8000 or sr > 192000:
            self.logger.warning(
                f"不適切なサンプリングレート: {sr} Hz (通常は8k-192kHz)"
            )
        ...
        self.logger.debug(
            f"C8(4186Hz)カバー: {'OK' if coverage_high else 'NG - 最高周波数が足りません'}"
        )
        ...
        for freq in important_freqs:
            ...
            error_cents = (
                1200 * np.log2(freq / closest_freq)
                if closest_freq > 0
                else float("inf")
            )
            self.logger.debug(
                f"周波数 {freq:.2f} Hz の最近接バンド: {closest_band}, 中心周波数: {closest_freq:.2f} Hz, 誤差: {abs(error_cents):.2f} cents"
            )
        ...
        for b in range(self.bands):
            ...
            if rho > 0.9999:
                self.logger.warning(
                    f"バンド {b}, 中心周波数 {self.center_freqs[b]:.2f} Hz: rho値が1に非常に近い ({rho:.8f}), 数値不安定の可能性あり"
                )
            ...
            if (
                ...
                self.logger.debug(
                    f"バンド {b}, 中心周波数 {self.center_freqs[b]:.2f} Hz のフィルタ係数:"
                )
                self.logger.debug(
                    f"  減衰率: {alpha:.6f}, rho: {rho:.6f}, theta: {theta:.6f}"
                )
    ...
    def _apply_complex_filters(
        ...
        if n_samples < 3:
            self.logger.warning(
                f"音声データのサンプル数が少なすぎます: {n_samples} (最低3サンプル必要)"
            )
            # 空の結果を返す（処理を続行できない）
            z_bands = [
                np.zeros(n_samples, dtype=np.complex128) for _ in range(self.bands)
            ]
        ...
        for b in range(self.bands):
            try:
                ...
                if self.debug:
                    ...
                    max_amp = np.max(r)
                    self.logger.debug(
                        f"バンド {b}, 中心周波数 {center_freq:.2f} Hz: "
    ...
    def _calculate_curvature(
        self, z_bands: List[np.ndarray], r_bands: List[np.ndarray]
    ) -> List[np.ndarray]:
        """
        各バンドの複素包絡から曲率を計算する
        ...
        for b in range(min(self.bands, len(z_bands))):
            try:
                ...
                if self.debug:
                    ...
                    self.logger.debug(
                        f"バンド {b}: 平均曲率 {mean_kappa:.6f}, 最大曲率 {max_kappa:.6f}"
                    )
    ...
    def _calculate_entropy(
        self, kappa_bands: List[np.ndarray], r_bands: List[np.ndarray]
    ...
    def _process_frames(
        ...
        if n_samples < self.hop_length:
            self.logger.warning(
                f"サンプル数がホップ長より少ないため、フレーム処理を省略します: サンプル数={n_samples}, ホップ長={self.hop_length}"
            )
            # 単一フレームとみなす
    ...
    def _detect_onsets_offsets(
        self, xi_real_frames: np.ndarray, frame_times: np.ndarray, sr: int
    ) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        if n_frames < 3:
            self.logger.warning(
                f"フレーム数が少なすぎます: {n_frames}. 少なくとも3フレーム必要です。空の結果を返します。"
            )
        ...
        for m in range(n_frames):
            ...
            if m > 0 and m < n_frames - 1:
                ...
                is_local_peak = (xi_real_frames[m] > xi_real_frames[m - 1]) and (
                    xi_real_frames[m] > xi_real_frames[m + 1]
                )
        ...
        self.logger.debug(
            f"検出結果: {len(onsets)}個のオンセット, {len(offsets)}個のオフセット"
        )
    ...
    def _estimate_pitches(
        ...
        # 入力配列のサイズ確認
        min_size = min(
            len(xi_imag_frames), len(frame_energies), len(np_indices), len(frame_times)
        )
        if min_size < n_frames:
            self.logger.warning(
                f"入力配列のサイズが不一致です: xi_imag_frames={len(xi_imag_frames)}, frame_energies={len(frame_energies)}, np_indices={len(np_indices)}, frame_times={len(frame_times)}"
            )
            n_frames = min_size
        ...
        for m in range(n_frames):
            ...
            if (
                ...
                or m >= len(np_indices)
                or m >= len(xi_imag_frames)
            ):
                self.logger.warning(
                    f"インデックス {m} が範囲外です: frame_energies={len(frame_energies)}, np_indices={len(np_indices)}, xi_imag_frames={len(xi_imag_frames)}"
                )
                continue
        ...
        if np.sum(pitch_freqs > 0) > 0:
            ...
            if np.sum(valid_mask) > 2:  # 十分なデータポイントがある場合
                ...
                if window_size >= 3:  # 最小窓サイズ
                    smoothed_freqs = signal.medfilt(
                        valid_freqs, kernel_size=window_size
                    )
        ...
        if len(valid_indices) > 0 and len(frame_times) > 0:
            ...
            if len(safe_indices) > 0:
                ...
                self.logger.warning(
                    "有効なピッチポイントがないか、全てのインデックスがframe_timesの範囲外です"
                )
    ...
    def detect(self, audio_data: np.ndarray, sr: int) -> Dict[str, Any]:
        ...
        if len(audio_data) < 1024:
            self.logger.warning(
                f"音声データのサンプル数が少なすぎます: {len(audio_data)}サンプル"
            )
            # 空の結果を返す
            return {
                "intervals": np.array([]).reshape(0, 2),
        ...
        try:
            # パラメータと音声情報のログ出力
            self.logger.debug(
                f"音声データ: 長さ={len(audio_data)/sr:.2f}秒, サンプリングレート={sr}Hz, サンプル数={len(audio_data)}"
            )
            self.logger.debug(
                f"検出パラメータ: bands={self.bands}, f_min={self.f_min}, hop_length={self.hop_length}, frame_length={self.frame_length}"
            )
            ...
            self.logger.debug(
                f"z_bandsの長さ: {len(z_bands)}, 各バンドのサンプル数: {[len(z) for z in z_bands[:3]]}..."
            )
            ...
            self.logger.debug(
                f"kappa_bandsの長さ: {len(kappa_bands)}, 各バンドのサンプル数: {[len(k) for k in kappa_bands[:3]]}..."
            )
            ...
            self.logger.debug(
                f"np_indicesの長さ: {len(np_indices)}, frame_energiesの長さ: {len(frame_energies)}"
            )
            ...
            self.logger.debug(
                f"フレーム処理結果: xi_real_framesの長さ: {len(xi_real_frames)}, xi_imag_framesの長さ: {len(xi_imag_frames)}, frame_timesの長さ: {len(frame_times)}"
            )

            self.logger.debug("オンセット・オフセットを検出します")
            # オンセット・オフセット検出
            onsets, offsets = self._detect_onsets_offsets(
                xi_real_frames, frame_times, sr
            )

            self.logger.debug(
                f"検出されたオンセット数: {len(onsets)}, オフセット数: {len(offsets)}"
            )
            ...
            self.logger.debug(
                f"検出結果: オンセット数={len(onsets)}, オフセット数={len(offsets)}, ピッチポイント数={len(pitch_times)}"
            )
            ...
            if len(onsets) > 0 and len(offsets) > 0:
                ...
                for i, onset in enumerate(onsets):
                    ...
                    if np.any(mask):
                        ...
                        # 診断ログ
                        self.logger.debug(
                            f"音符#{i+1}: 時間=[{onset:.3f}, {offset:.3f}], ピッチ={median_freq:.1f}Hz, データポイント数={np.sum(mask)}"
                        )
                    else:
                        # ピッチが見つからない場合は0とする
                        note_pitches.append(0.0)
                        self.logger.warning(
                            f"音符#{i+1}: 時間=[{onset:.3f}, {offset:.3f}], 区間内にピッチデータがありません"
                        )
