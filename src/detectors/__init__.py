...
class DetectorMetadata:
    ...
    def __init__(self, name: str, description: str = "", version: str = "1.0", 
                 params: Dict[str, Any] = None, file_path: str = None):
        """
        検出器メタデータの初期化
...
def register_detector(cls=None, *, name: str = None, description: str = None, 
                      version: str = None, params: Dict[str, Any] = None):
    """
...
def load_detector_from_file(file_path: str, detector_name: str = None) -> Type[BaseDetector]:
    """
    ...
    try:
        ...
        if detector_name is not None:
            ...
            raise ValueError(f"検出器クラス {detector_name} が見つかりません: {file_path}")
        ...
        if not found_detectors:
            raise ValueError(f"BaseDetectorを継承するクラスが見つかりません: {file_path}")
        
        # 複数のクラスが見つかった場合は警告
        if len(found_detectors) > 1:
            logger.warning(f"複数の検出器クラスが見つかりました。最初のクラスを使用します: {[cls.__name__ for cls in found_detectors]}")
...
def get_detector_class(detector_name: str) -> Type[BaseDetector]:
    ...
    try:
        ...
        project_roots = [p for p in sys_path_items if (p / 'src' / 'detectors').exists()]
    ...
    for search_path in search_paths:
        ...
        if detector_file.exists():
            try:
                ...
                logger.warning(f"検出器ファイルのロードに失敗しました: {detector_file}: {e}")
...
# 既存の検出器モジュールをロード
detector_files = sorted([f for f in Path(__file__).resolve().parent.glob("*.py") 
                        if f.name not in ["__init__.py", "base_detector.py", "numba_helpers.py", "sihr_hypothesis.py"]])
