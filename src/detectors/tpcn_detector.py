...
def _parabolic_interpolation_numba(y_values: np.ndarray, peak_index: int) -> Tuple[float, float]:
    ...
    interpolated_value = max(0.0, min(interpolated_value, y_0 * 1.1)) # Ensure plausible
...
def _calculate_harmonic_frequencies_numba(
    ...
    if abs(inharmonicity_factor) < 1e-9:
        ...
        inharmonicity_term_sq = np.maximum(0.0, 1.0 + inharmonicity_factor * harmonic_numbers**2)
...
def _calculate_single_valley_spod_metric_numba(
    ...
    h_nums, h_freqs = _calculate_harmonic_frequencies_numba(
        f0_hz, h_max + 1, inharmonicity_factor, max_f_analysis # Need h+1 for valley calculation
    )
    num_valid_harmonics = len(h_freqs)
        ...
        if min_valley_bin <= max_valley_bin: # Ensure range is valid
            for k in range(min_valley_bin, max_valley_bin + 1):
                ...
                if np.isfinite(spod_val) and np.isfinite(amp_val) and amp_val > 0:
                    ...
                    if not np.isfinite(weight) or weight < 0: weight = 0.0
    ...
    avg_s_valleys = weighted_spod_sum_valleys / total_valley_weight if total_valley_weight > 0 else 0.0
    # Clip to [0, 1]
...
def _calculate_single_hme_numba(
    ...
    for i in range(num_valid_harmonics):
        ...
        if np.isfinite(interp_amp) and h > 0:
            ...
            weighted_amp_sum += weight * max(0.0, interp_amp) # Ensure amplitude is non-negative
...
# [+] Update return type hint -> No change needed here
    ...
    if np.any(valid_f0_mask):
        F_cand_cents[valid_f0_mask] = 1200 * np.log2(F_cand_hz[valid_f0_mask] / 10.0) # Ref 10 Hz
    ...
    for i in numba.prange(n_f0_cands):
        ...
        for n in range(n_frames):
            ...
            S_peaks, _ = _calculate_single_spod_metric_numba(
                f0, SPOD_frame, A_frame, # Pass SPOD_frame and A_frame
                sr, n_fft, h_max, inharmonicity_factor, freq_resolution_hz
            ...
            S_valleys, _ = _calculate_single_valley_spod_metric_numba(
                f0, f0_cents, SPOD_frame, A_frame, # Pass SPOD_frame for S_valleys, A_frame for weighting
                sr, n_fft, h_max, inharmonicity_factor, freq_resolution_hz,
                valley_window_cents, valley_weight_exponent
            )

            # [+] Calculate HME using the updated dedicated function (removed weight params)
            hme_value = _calculate_single_hme_numba(
                 f0, A_frame, # Use A_frame for HME calculation
                 sr, n_fft, h_max, inharmonicity_factor, freq_resolution_hz,
                 # Removed harmonic weight params
            )
...
class TPCNDetector(BaseDetector):
    ...
    DEFAULT_TPCN_SALIENCE_THRESHOLD = 0.08 # Min PS_lambda value (Increased: 0.05 -> 0.08)
    DEFAULT_TPCN_ONSET_SCORE_THRESHOLD = 0.1 # Min OS_TPCN value (Increased: 0.05 -> 0.1)
    DEFAULT_TPCN_PC_DIFF_THRESHOLD = 0.1 # Min PC_diff value (Increased: 0.05 -> 0.1)
    DEFAULT_TPCN_VALLEY_WINDOW_CENTS = 50.0 # Search window size around valley centers (cents)
    DEFAULT_TPCN_VALLEY_WEIGHT_EXPONENT = 0.5 # Weight exponent for valley SPOD averaging (Reverted: 0.0 -> 0.5)
    ...
    DEFAULT_TPCN_NOTE_MERGE_TOLERANCE_CENTS = 50.0 # Max pitch diff to merge adjacent notes
    DEFAULT_TPCN_MIN_SILENCE_DURATION_SEC = 0.04 # Max silence duration to merge adjacent notes
    # [+] Add NMS parameters
    DEFAULT_TPCN_NMS_WINDOW_CENTS = 50.0 # Window size for Non-Maximum Suppression on F0 candidates
    # [+] Add Map Smoothing Parameters
    DEFAULT_TPCN_MAP_SMOOTH_FRAMES = 3     # Num frames for time smoothing (use odd numbers)
    ...
    def __init__(self,
        ...
        self.tpcn_pc_diff_threshold = max(-np.inf, tpcn_pc_diff_threshold) # Allow negative threshold if needed? Or max(0,...)
        ...
        self.tpcn_pitch_continuity_tolerance_cents = max(1.0, tpcn_pitch_continuity_tolerance_cents)
    ...
    def _initialize_run(self, sr: int):
        ...
        if sr <= 0: raise ValueError(f"Invalid sample rate: {sr}")
        self._sr = sr
        ...
        logger.debug(f"Run initialized for SR={sr}. Freq resolution: {self._freq_resolution_hz:.3f} Hz. {len(self._f0_candidates_hz)} F0 candidates.")

    def _generate_f0_candidates(self) -> np.ndarray:
        """Generates F0 candidates in Hz on a cents scale."""
        if self._freqs is None or self._freq_resolution_hz is None: return np.array([])
        min_f = self.min_freq_hz
        max_f = min(self.max_freq_hz, self._freqs[-1])
        if min_f >= max_f: return np.array([])
        min_cents_arr = hz_to_cents_numba(np.array([min_f]))
        max_cents_arr = hz_to_cents_numba(np.array([max_f]))
        if not (np.isfinite(min_cents_arr[0]) and np.isfinite(max_cents_arr[0])): return np.array([])
        min_cents, max_cents = min_cents_arr[0], max_cents_arr[0]
        num_steps = int(np.ceil((max_cents - min_cents) / self.f0_candidate_resolution_cents)) + 1
        if num_steps <= 1:
        ...
        return f0_candidates_hz[(f0_candidates_hz >= min_f) & (f0_candidates_hz <= max_f)]

    def _calculate_stft(self, audio_data: np.ndarray) -> bool:
        """Calculates STFT and linear amplitude. Returns True on success."""
        if self._sr is None: return False
        try:
            S_complex = librosa.stft(audio_data, n_fft=self.n_fft, hop_length=self.hop_length, window=self.window, center=True)
            self._A_lin = np.abs(S_complex)
            self._times = librosa.frames_to_time(np.arange(S_complex.shape[1]), sr=self._sr, hop_length=self.hop_length)
            return True
    ...
    def _calculate_spod(self, audio_data: np.ndarray) -> bool:
        ...
        try:
            ...
            spod_window_frames = max(1, int(round(self.spod_window_sec * self._sr / self.hop_length)))
            ...
            inst_freq_sq_mean = uniform_filter1d(inst_freq**2, size=spod_window_frames, axis=1, mode='nearest')
            inst_freq_mean = uniform_filter1d(inst_freq, size=spod_window_frames, axis=1, mode='nearest')
    ...
    def _calculate_tpcn_maps(self) -> bool:
        ...
        """
        if self._A_lin is None or self._spod_map is None or \
           self._f0_candidates_hz is None or self._sr is None or \
           self._freq_resolution_hz is None:
        ...
        try:
            ...
            logger.debug(f"TPCN maps calculated. Shape: {ps_map.shape}. Time: {calc_time:.4f}s")
            return True
    ...
    def _estimate_f0s_from_salience(self) -> List[List[float]]: # Return type changed
        ...
        """
        if self._PS_lambda_map is None or self._PC_diff_map is None or \
           self._f0_candidates_hz is None or self._times is None:
            logger.error("Required maps for F0 estimation are missing.")
            return [[] for _ in range(len(self._times) if self._times is not None else 0)]
        ...
        for t in range(n_frames):
            ...
            for i in range(len(sorted_indices)):
                ...
                if 0 < current_idx < n_f0_cands - 1: # Check if index allows interpolation
                    ...
                    if len(y_slice) == 3 and np.isfinite(y_slice).all():
                         ...
                         delta_index, interp_ps_val = _parabolic_interpolation_numba(y_slice, 1)
                         ...
                         if abs(delta_index) <= 0.5: # Plausible refinement range
                              ...
                              if delta_index < 0: # Interpolate between f_low and f_mid
                                   refined_hz = f_mid + delta_index * (f_mid - f_low) # delta_index is negative
                              ...
                              refined_hz = max(f_low * 0.99, min(refined_hz, f_high * 1.01))
                ...
                for j in range(i + 1, len(sorted_indices)):
                    if not suppressed[j]:
                        neighbor_cents = sorted_f0s_cents[j] # Use original cents for comparison
                        # Check if neighbor is within the window (absolute difference)
                        if abs(neighbor_cents - suppress_center_cents) < nms_window_cents / 2.0:
                             suppressed[j] = True
        ...
        logger.debug(f"F0 estimation (PC+PS) complete. Found F0s in {sum(1 for f0s in multi_f0_output if f0s)} / {n_frames} frames.")
        return multi_f0_output


    def _frames_to_notes_tpcn(self,
                              multi_f0_per_frame: List[List[float]]) -> List[Dict[str, Any]]:
        """
        ...
        """
        if self._times is None or self._sr is None or \
           self._PS_lambda_map is None or self._OS_TPCN_map is None or \
           self._f0_candidates_hz is None:
        ...
        if smooth_frames > 1 or smooth_cents > 0:
            logger.debug(f"Applying map smoothing: frames={smooth_frames}, cents={smooth_cents}...")
            ...
            if smooth_frames > 1:
                ...
                ps_map_smoothed = uniform_filter1d(ps_map_orig, size=time_window, axis=1, mode='nearest')
                os_map_smoothed = uniform_filter1d(os_map_orig, size=time_window, axis=1, mode='nearest')
            else: # Need a copy if only frequency smoothing is applied
                ps_map_smoothed = ps_map_orig.copy() if smooth_cents > 0 else ps_map_orig
                os_map_smoothed = os_map_orig.copy() if smooth_cents > 0 else os_map_orig
            ...
            if smooth_cents > 0 and n_f0_cands > 1:
                 ...
                 ps_map_smoothed = uniform_filter1d(ps_map_smoothed, size=freq_window_bins, axis=0, mode='nearest')
                 os_map_smoothed = uniform_filter1d(os_map_smoothed, size=freq_window_bins, axis=0, mode='nearest')
        ...
        if len(frame_times) != num_frames:
             logger.warning(f"Mismatch frame times/F0 results: {len(frame_times)} vs {num_frames}. Truncating.")
        ...
        max_pitch_gap_frames = max(1, int(np.ceil(self.tpcn_max_pitch_gap_sec / hop_time)))
        ...
        for frame_idx in range(num_frames):
            ...
            if num_active_tracks > 0:
                 for r, track_id in enumerate(active_track_ids):
                     ...
                     active_track_last_pitches_cents[r] = pitch_hist_cents[-1] if pitch_hist_cents else -np.inf
                     # [+] Get last salience
                     salience_hist = active_notes[track_id].get('saliences', [])
                     active_track_last_saliences[r] = salience_hist[-1] if salience_hist else 0.0
            ...
            if num_active_tracks > 0 and num_current_f0s > 0:
                 for r in range(num_active_tracks):
                     ...
                     if not np.isfinite(last_pitch_cents): continue # Skip inactive tracks

                     for c in range(num_current_f0s):
                         curr_pitch_cents = current_f0s_cents_np[c]
                         if not np.isfinite(curr_pitch_cents): continue # Skip invalid F0s

                         # --- Calculate Pitch Cost ---
                         pitch_diff = abs(curr_pitch_cents - last_pitch_cents)
                         pitch_cost = pitch_diff if pitch_diff < self.tpcn_pitch_continuity_tolerance_cents else COST_INF

                         # --- Calculate Salience Cost ---
                         salience_cost = COST_INF
                         if pitch_cost < COST_INF: # Only calculate salience cost if pitch is plausible
                             # Find current salience value (needs f0_map_idx)
                             curr_f0_hz = current_f0s_hz[c]
                             diffs_hz = np.abs(f0_cand_list - curr_f0_hz)
                             curr_f0_map_idx = np.argmin(diffs_hz) if len(diffs_hz) > 0 else -1

                             current_salience = 0.0
                             if 0 <= curr_f0_map_idx < n_f0_cands:
                                  current_salience = ps_map_smoothed[curr_f0_map_idx, frame_idx]
                         ...
                         if pitch_cost < COST_INF and salience_cost < COST_INF:
                             combined_cost = W_PITCH * pitch_cost + W_SALIENCE * salience_cost
            ...
            for r, track_id in enumerate(active_track_ids):
                if r not in matched_track_indices:
                    ...
                    if last_pitch_hz > 0:
                         ...
                         if len(diffs) > 0: f0_map_idx = np.argmin(diffs)
                    ...
                    if should_end_note:
                        ...
                        if timed_out: stats["offset_timeout"] += 1
                        if ps_lambda_low: stats["offset_salience_low"] += 1

                        # Finalize Note Data
                        start_time = note_info['start_time']
                        last_update_idx = note_info['last_update_frame']
                        offset_time = frame_times[last_update_idx] + hop_time if 0 <= last_update_idx < len(frame_times) else current_time
                        ...
                        if duration >= self.tpcn_min_note_duration_sec and pitches_hz:
                             ...
                             notes_list.append({'start_time': start_time, 'offset_time': offset_time,
                                                'pitch_hz': pitch_median, 'duration': duration,
                                                'pitches_hz': pitches_hz})

            # Remove ended tracks
            for track_id in ended_track_ids:
                 if track_id in active_notes: del active_notes[track_id]
            ...
            for c, f0_hz in enumerate(current_f0s_hz):
                if c not in matched_f0_indices:
                    ...
                    if 0 <= f0_map_idx < n_f0_cands:
                        ...
                        onset_score_ok = current_os_tpcn >= self.tpcn_onset_score_threshold
                    ...
                    if 0 <= f0_map_idx < n_f0_cands:
                         ...
                         salience_ok = True # Assumed True if f0_hz is in multi_f0_per_frame
                    ...
                    if onset_score_ok and salience_ok:
                        ...
                        track_id_counter += 1
                         ...
                         if not onset_score_ok: stats["onset_suppressed_os_low"] += 1
        ...
        if frame_times is not None:
             ...
             for track_id, note_info in active_notes.items():
                  start_time = note_info['start_time']
                  last_update_idx = note_info['last_update_frame']
                  offset_time = frame_times[last_update_idx] + hop_time if 0 <= last_update_idx < num_frames_final else frame_times[-1] + hop_time
                  duration = offset_time - start_time
                  pitches_hz = note_info['pitches_hz']
                  if duration >= self.tpcn_min_note_duration_sec and pitches_hz:
                      pitch_median = np.median(pitches_hz)
        ...
        logger.info(f"Note Tracking Stats: Onsets Started:{stats['onset_started']} | Onsets Suppressed [OS Low:{stats['onset_suppressed_os_low']}] | Offsets Advanced [Timeout:{stats['offset_timeout']}, Salience Low:{stats['offset_salience_low']}]")
        return notes_list
    ...
    def _postprocess_notes(self, notes_list: List[Dict[str, Any]]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        logger.debug(f"[_postprocess_notes_tpcn] Start: Received {len(notes_list)} raw notes.")

        # --- Initial Filtering & Data Structure ---
        notes = []
        for n in notes_list:
            if n.get('pitch_hz', 0.0) > 0 and n.get('duration', 0.0) > 1e-6:
                 notes.append({k: v for k, v in n.items() if k in ['start_time', 'offset_time', 'pitch_hz', 'duration', 'pitches_hz']})
                 notes[-1]['onset'] = notes[-1].pop('start_time') # Rename for consistency
                 notes[-1]['offset'] = notes[-1].pop('offset_time')
                 notes[-1]['pitch'] = notes[-1].pop('pitch_hz')

        logger.debug(f"[_postprocess_notes_tpcn] After initial filter: {len(notes)} notes.")
        ...
        if notes:
            ...
            current_note_pitch_cents_arr = hz_to_cents_numba(np.array([current_note['pitch']]))
            current_note_pitch_cents = current_note_pitch_cents_arr[0] if len(current_note_pitch_cents_arr)>0 else -np.inf

            for i in range(1, len(notes)):
                next_note = notes[i]
                gap = next_note['onset'] - current_note['offset']
                next_note_pitch_cents_arr = hz_to_cents_numba(np.array([next_note['pitch']]))
                next_note_pitch_cents = next_note_pitch_cents_arr[0] if len(next_note_pitch_cents_arr)>0 else -np.inf
                pitch_diff_cents = abs(current_note_pitch_cents - next_note_pitch_cents) if np.isfinite(current_note_pitch_cents) and np.isfinite(next_note_pitch_cents) else np.inf

                # Merge Condition: Small gap AND similar pitch
                if 0 <= gap <= self.tpcn_min_silence_duration_sec and pitch_diff_cents <= self.tpcn_note_merge_tolerance_cents:
                    duration1 = current_note['duration']
                    duration2 = next_note['duration']
                    total_duration = duration1 + duration2
                    if total_duration > 1e-9: current_note['pitch'] = (current_note['pitch'] * duration1 + next_note['pitch'] * duration2) / total_duration
                    else: current_note['pitch'] = (current_note['pitch'] + next_note['pitch']) / 2.0
                    current_note['offset'] = next_note['offset']
                    current_note['duration'] = current_note['offset'] - current_note['onset']
                    if 'pitches_hz' not in current_note: current_note['pitches_hz'] = []
            ...
            merged_notes.append(current_note)
            notes = merged_notes
            logger.debug(f"[_postprocess_notes_tpcn] After merging consecutive: {len(notes)} notes.")
        ...
        if notes:
             ...
             while i < len(notes):
                 ...
                 while j < len(notes) and notes[j]['onset'] < current_note['offset']:
                     other_note = notes[j]
                     pitch1_cents_arr = hz_to_cents_numba(np.array([current_note['pitch']]))
                     pitch1_cents = pitch1_cents_arr[0] if len(pitch1_cents_arr)>0 else -np.inf
                     pitch2_cents_arr = hz_to_cents_numba(np.array([other_note['pitch']]))
                     pitch2_cents = pitch2_cents_arr[0] if len(pitch2_cents_arr)>0 else -np.inf
                     ...
                     if pitch_diff_cents <= self.tpcn_note_merge_tolerance_cents:
                         ...
                         total_dur = dur1 + dur2
                         if total_dur > 1e-9: current_note['pitch'] = (current_note['pitch'] * dur1 + other_note['pitch'] * dur2) / total_dur
                         else: current_note['pitch'] = (current_note['pitch'] + other_note['pitch']) / 2.0
                         current_note['offset'] = max(current_note['offset'], other_note['offset'])
                         current_note['duration'] = current_note['offset'] - current_note['onset']
                         if 'pitches_hz' not in current_note: current_note['pitches_hz'] = []
                         current_note['pitches_hz'].extend(other_note.get('pitches_hz', []))
                         notes.pop(j) # Remove merged note
                         continue # Re-check overlaps with the expanded current_note
                     else:
                         j += 1 # Different pitch overlap, move to next potential overlap
                 processed_notes_overlap.append(current_note)
                 i += 1
             notes = processed_notes_overlap
             logger.debug(f"[_postprocess_notes_tpcn] After overlap resolution: {len(notes)} notes.")
        ...
        for note in notes:
            ...
            if len(pitches_hz) >= 2:
                ...
                if len(valid_cents) >= 2:
                    ...
                    else: removed_unstable_count += 1
                else: stable_notes.append(note)
            else: stable_notes.append(note)
        if removed_unstable_count > 0: logger.debug(f"[_postprocess_notes_tpcn] Removed {removed_unstable_count} unstable notes.")
        notes = stable_notes
        logger.debug(f"[_postprocess_notes_tpcn] After pitch stability filter: {len(notes)} notes.")

        # --- Stage 4: Final Duration Filter ---
        final_notes: List[Dict[str, Any]] = []
        removed_short_count = 0
        for note in notes:
            note['duration'] = note['offset'] - note['onset'] # Recalculate duration after merging
            if note['duration'] >= self.tpcn_min_note_duration_sec:
                 pitches_hz_final = np.array(note.get('pitches_hz', []), dtype=np.float64)
                 if len(pitches_hz_final) > 0:
                      note['final_pitch_hz'] = np.median(pitches_hz_final)
                      final_notes.append(note)
                 else: removed_short_count +=1
            else: removed_short_count += 1
        if removed_short_count > 0: logger.debug(f"[_postprocess_notes_tpcn] Removed {removed_short_count} short notes in final filter.")
        logger.debug(f"[_postprocess_notes_tpcn] Final notes count: {len(final_notes)}.")
    ...
    def detect(self, audio_data: np.ndarray, sr: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self._calculate_stft(audio_data) or self._A_lin is None or self._times is None:
            ...
            if not self._calculate_spod(audio_data) or self._spod_map is None:
                 ...
                 logger.warning("SPOD map calculation failed. TPCN results might be inaccurate.")
            ...
            multi_f0_per_frame = self._estimate_f0s_from_salience() # Rewritten function call
            ...
            logger.info(f"Detection complete in {detection_time:.4f} seconds. Found {final_note_count} notes.")
    ...
    def _empty_result(self, error_message: str = "Detection failed") -> Dict[str, Any]:
        ...
        return {
                'intervals': np.array([]).reshape(0, 2), 'note_pitches': np.array([]),
                'frame_times': np.array([]), 'frame_frequencies': np.array([]),
                'detector_name': self.__class__.__name__ + "_Error", 'detection_time': 0.0,
