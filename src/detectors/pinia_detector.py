...
class PinnaDetector(BaseDetector):
    ...
    def _half_wave_rectify_compress(self, signals: List[np.ndarray]) -> List[np.ndarray]:
        """
        半波整流と信号圧縮を適用します
    ...
    def _calculate_spike_rates(self, spikes: List[List[int]], signal_length: int, sr: int) -> np.ndarray:
        """
        スパイク発火率を計算します
        ...
        for b, band_spikes in enumerate(spikes):
            for i in range(n_frames):
                ...
                # ウィンドウ内のスパイク数をカウント
                count = sum(1 for spike in band_spikes if window_start <= spike < window_end)
    ...
    def _calculate_onset_offset_functions(self, spike_rates: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def _dynamic_thresholding(self, onset_function: np.ndarray, offset_function: np.ndarray, 
                             energy_function: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        for i in range(1, n_frames):
            ...
            z[i] = ((1 - self.alpha) * z[i-1] + 
                   self.alpha * np.tanh(input_term))
        ...
        for i in range(window_size, n_frames):
            ...
            onset_threshold[i] = (
                self.onset_threshold * np.mean(z_diff[i-window_size:i][z_diff[i-window_size:i] > 0])
                if np.any(z_diff[i-window_size:i] > 0) else self.onset_threshold * 0.01
            )
            
            # オフセット閾値は過去の陰性値の平均x係数
            offset_threshold[i] = (
                self.offset_threshold * np.mean(np.abs(z_diff[i-window_size:i][z_diff[i-window_size:i] < 0]))
                if np.any(z_diff[i-window_size:i] < 0) else self.offset_threshold * 0.01
            )
        ...
        for i in range(1, n_frames - 1):
            ...
            if (z_diff[i] > onset_threshold[i] and 
                z_diff[i] > z_diff[i-1] and 
                z_diff[i] >= z_diff[i+1]):
                onset_peaks.append(i)
                
            # オフセットピーク: z_diffが閾値より小さく、前後のフレームより小さい
            if (z_diff[i] < -offset_threshold[i] and 
                z_diff[i] < z_diff[i-1] and 
                z_diff[i] <= z_diff[i+1]):
    ...
    def _analyze_interspike_intervals(self, spikes: List[List[int]], 
        ...
        for i in range(len(onsets)):
            ...
            if offset_sample is not None:
                ...
                analysis_start = onset_sample + min(int(duration * 0.1), 1000)  # 10%後から
            ...
            for b in pitch_band_indices:
                ...
                # 周期範囲内のISIのみを考慮
                valid_intervals = intervals[(intervals >= min_period) & (intervals <= max_period)]
    ...
    def detect_notes(self, audio: np.ndarray, sr: int) -> Dict[str, np.ndarray]:
        ...
        onset_function, offset_function = self._calculate_onset_offset_functions(spike_rates)
        ...
        pitches = self._analyze_interspike_intervals(spikes, onset_frames, offset_frames, sr)
