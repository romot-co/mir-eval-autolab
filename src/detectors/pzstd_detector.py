...
@numba.jit(nopython=True, cache=True)
def _parabolic_interpolation_numba(y_values: np.ndarray, peak_index: int) -> Tuple[float, float]:
    ...
    if abs(denominator) < 1e-9: return 0.0, y_0
...
@numba.jit(nopython=True, cache=True)
def _calculate_harmonic_freqs_numba(f0: float, max_harmonics: int, inharmonicity_factor: float, max_f_analysis: float, tolerance_cents: float) -> np.ndarray:
    ...
    if inharmonicity_factor == 0.0:
        ...
        inharmonicity_term = np.maximum(0.0, inharmonicity_term) # Ensure non-negative before sqrt
...
def _score_f0_candidate_numba(
    ...
    expected_harmonics_hz = _calculate_harmonic_freqs_numba(
        f0_cand_hz, max_harmonics, inharmonicity_factor, max_f_analysis, harmonic_match_tolerance_cents
    )
    ...
    cent_diffs = np.abs(expected_harmonics_cents.reshape(-1, 1) - peak_freqs_cents.reshape(1, -1))
    ...
    for h_idx in range(len(expected_harmonics_cents)):
        ...
        if len(match_indices_h) > 0:
            ...
            closest_idx_in_available = match_indices_h[np.argmin(harmonic_cent_diffs[match_indices_h])]

            # Peak properties for scoring
            matched_peak_spod = peak_spods[closest_idx_in_available]
            matched_peak_amp = peak_amps[closest_idx_in_available] # Could use amplitude?
            min_diff_cents = harmonic_cent_diffs[closest_idx_in_available]

            # Scoring components
            # 動的パラメータを使用した重み付け計算
            harmonic_weight = 1.0 / (h + harmonic_weight_offset)**harmonic_weight_exponent
            peak_salience = matched_peak_spod
            # Avoid division by zero if tolerance is zero (though constructor ensures > 0)
            proximity_score = max(0.0, 1.0 - (min_diff_cents / harmonic_match_tolerance_cents)) if harmonic_match_tolerance_cents > 0 else (1.0 if min_diff_cents == 0 else 0.0)
...
def _calculate_cost_matrix_numba(
    ...
    cost_matrix = np.full((num_active_tracks, num_current_f0s), cost_inf, dtype=np.float64)
...
def _interpolate_map_numba(map_data_frame: np.ndarray, freq_bin_float: float, n_bins: int) -> float:
    """
    ...
    if not (np.isfinite(val_low) and np.isfinite(val_high)):
        ...
        if np.isfinite(val_low): return val_low
        if np.isfinite(val_high): return val_high
...
def _calculate_harmonic_metrics_numba(
    ...
    # Calculate averages
    avg_coherence = weighted_spod_sum / total_spod_weight if total_spod_weight > 0 else 0.0
...
def _calculate_hcf_maps_numba(
    ...
    harmonic_weight_offset: float = 0.5,
    harmonic_weight_exponent: float = 1.0,
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]: # <<< 戻り値にAvgHarmonicSPOD_map追加
    """
    ...
    AvgHarmonicSPOD_map = np.zeros((n_f0_cands, n_frames), dtype=np.float64) # <<< AvgHarmonicSPOD_map初期化
    ...
    for i in numba.prange(n_f0_cands):
        ...
        for n in range(n_frames):
            ...
            C_map[i, n], AmpSum_map[i, n] = _calculate_harmonic_metrics_numba(
                f0, n, spod_frame, A_frame, sr, n_fft, h_max,
                inharmonicity_factor, freq_resolution_hz, n_bins,
                harmonic_weight_offset, harmonic_weight_exponent
            )
            # Calculate AvgHarmonicSPOD
            AvgHarmonicSPOD_map[i, n] = _calculate_avg_harmonic_spod_numba( # <<< 計算呼び出し追加
                 f0, n, spod_frame, sr, n_fft, h_max, # Use same h_max as HCF for now
                 inharmonicity_factor, freq_resolution_hz, n_bins,
                 harmonic_weight_offset, harmonic_weight_exponent
             )
    ...
    if n_frames > delta_n:
        ...
        for i in numba.prange(n_f0_cands):
            ...
            Phi_comb_map[i, :] = flux_weight_c * np.maximum(0.0, Phi_C_map[i, :]) + \
    ...
    return C_map, AmpSum_map, Phi_C_map, Phi_A_map, Phi_comb_map, O_vec, AvgHarmonicSPOD_map # <<< 戻り値に追加
...
class PZSTDDetector(BaseDetector):
    ...
    DEFAULT_HCF_NOTE_START_FLUX_THRESH = 0.1  # グリッドサーチで最適化された値（0.06→0.1）
    ...
    def __init__(self,
                 ...
                 **kwargs): # Keep kwargs for BaseDetector compatibility
        """
        ...
        self.harmonic_match_tolerance_cents = max(1.0, harmonic_match_tolerance_cents) # Ensure positive
        self.pitch_continuity_tolerance_cents = max(1.0, pitch_continuity_tolerance_cents) # Ensure positive
    ...
    def _initialize_run(self, sr: int):
        ...
        self._sr = sr
        self._freqs = librosa.fft_frequencies(sr=self._sr, n_fft=self.n_fft) # Use instance var
        self._freq_resolution_hz = self._freqs[1] - self._freqs[0] if len(self._freqs) > 1 else sr / self.n_fft # Use instance var
    ...
    def _freq_to_bin_approx(self, freq_hz: float) -> Optional[float]:
        ...
        if self._freqs is None or self._freq_resolution_hz is None or self._freq_resolution_hz <= 0: return None
        if freq_hz < self._freqs[0] or freq_hz > self._freqs[-1]: return None
    ...
    def _bin_to_freq(self, bin_index: float) -> Optional[float]:
        ...
        if self._freqs is None or self._freq_resolution_hz is None: return None
        return np.clip(bin_index * self._freq_resolution_hz, self._freqs[0], self._freqs[-1])

    def _detect_peaks_per_frame(self, A_lin: np.ndarray, S_db: np.ndarray, spod_map: np.ndarray) -> List[List[Tuple[float, float, float]]]:
        """
        ...
        """
        if self._freqs is None: return [[] for _ in range(A_lin.shape[1])]
        n_bins, n_frames = A_lin.shape
        peaks_per_frame: List[List[Tuple[float, float, float]]] = [[] for _ in range(n_frames)]
        min_bin_idx = np.searchsorted(self._freqs, self.min_freq_hz, side='left') # Use instance var
        ...
        min_height_amp = librosa.db_to_amplitude(self.energy_db_threshold, ref=1.0) # Use instance var
        for t in range(n_frames):
            ...
            if amp_t_slice.size == 0: continue
            ...
            if np.any(valid_mask):
                 ...
                 amp_range = np.max(valid_amps) - np.min(valid_amps) if len(valid_amps) > 1 else np.max(valid_amps)
                 prominence_threshold = max(1e-9, amp_range * self.peak_prominence_rel) # Use instance var
            else:
                 continue
            try:
                ...
                if len(peak_indices_abs) > self.max_peaks_per_frame: # Use instance var
                    ...
                    if len(prominences) >= self.max_peaks_per_frame: # Use instance var
                        partition_idx = np.argpartition(prominences, -self.max_peaks_per_frame)[-self.max_peaks_per_frame:] # Use instance var
                        top_k_indices = partition_idx[np.argsort(prominences[partition_idx])[::-1]]
                ...
                for p_idx_abs in peak_indices_abs:
                    ...
                    if interp_freq is not None and self.min_freq_hz <= interp_freq <= self.max_freq_hz: # Use instance vars
                        frame_peaks_data.append((interp_freq, amp_at_peak, spod_at_peak))
                frame_peaks_data.sort(key=lambda x: x[0])
                peaks_per_frame[t] = frame_peaks_data
            except Exception as e:
                logger.error(f"Frame {t}: Error during peak detection/interpolation: {e}", exc_info=False)
        return peaks_per_frame

    def _generate_f0_candidates(self) -> np.ndarray:
        """Generates F0 candidates using fixed parameters."""
        if self._freqs is None: return np.array([])
        min_f = self.min_freq_hz # Use instance var
        max_f = self._max_f_analysis
        if min_f >= max_f:
            logger.warning(f"Min F0 freq ({min_f} Hz) >= Max F0 freq ({max_f} Hz). No candidates generated.")
            return np.array([])
        ...
        if not np.isfinite(min_cents) or not np.isfinite(max_cents):
             ...
             return np.array([])
        num_steps = int(np.ceil((max_cents - min_cents) / self.f0_candidate_resolution_cents)) + 1 # Use instance var
        if num_steps <= 1:
             f0_candidates_hz = np.array([min_f, max_f]) if min_f != max_f else np.array([min_f])
        else:
             f0_candidates_cents = np.linspace(min_cents, max_cents, num_steps)
             f0_candidates_hz = cents_to_hz_numba(f0_candidates_cents)
        logger.debug(f"Generated {len(f0_candidates_hz)} F0 candidates ({min_f:.1f}Hz-{max_f:.1f}Hz, {self.f0_candidate_resolution_cents} cents res).") # Use instance var
    ...
    def _estimate_f0s_from_peaks(self,
        ...
        for t in range(n_frames):
            ...
            if detected_f0s_t:
                previous_f0s_cents = hz_to_cents_numba(np.array(detected_f0s_t, dtype=np.float64))
            else:
                previous_f0s_cents = np.array([], dtype=np.float64)

        logger.debug(f"F0 estimation found F0s in {sum(1 for f0s in multi_f0_per_frame if f0s)} / {n_frames} frames.")
        return multi_f0_per_frame
    ...
    def _frames_to_notes_multipitch(self,
        ...
        # --- 初期化、状態チェック --- (略)
        # ...
        if self._times is None or self._sr is None or self._freqs is None or self._freq_resolution_hz is None:
        ...
        # --- F0候補マッピング (既存のまま) ---
        # ...
        f0_cand_hz_list = self._f0_candidates_hz if self._f0_candidates_hz is not None else []
        ...
        if hop_time <= 0: hop_time = 1024 / 44100 # Safe fallback
        max_pitch_gap_frames = max(1, int(np.ceil(self.max_pitch_gap_sec / hop_time))) if hop_time > 0 else 2 # Use instance var
        COST_INF = 1e6
        ...
        for frame_idx in range(num_frames):
            ...
            current_f0s_cents_np = hz_to_cents_numba(np.array(current_f0s_hz, dtype=np.float64))
            ...
            active_track_last_pitches_cents = np.zeros(num_active_tracks, dtype=np.float64)
            active_track_stabilities_cents = np.zeros(num_active_tracks, dtype=np.float64)
            if num_active_tracks > 0:
                for r, track_id in enumerate(active_track_ids):
                    note_info = active_notes[track_id]
                    pitch_history_cents = np.array(note_info['pitches_cents'], dtype=np.float64)
                    if len(pitch_history_cents) > 0:
                         active_track_last_pitches_cents[r] = pitch_history_cents[-1]
                         history_len = min(len(pitch_history_cents), 5) # Look at last 5 frames
                         active_track_stabilities_cents[r] = np.std(pitch_history_cents[-history_len:]) if history_len >= 2 else 0.0
                    else: # Handle empty history case
                         active_track_last_pitches_cents[r] = -np.inf # Or some invalid value
            ...
            if num_active_tracks > 0 and num_current_f0s > 0 and np.any(cost_matrix < COST_INF):
                try:
                    ...
                    if timed_out or coherence_too_low:
                        ...
                        if not timed_out: # どの条件で早期終了したかログに残す
                            if coherence_too_low:
                                ...
                                logger.debug(f"Frame {frame_idx}, Track {track_id}: Coherence low ({current_coherence:.3f} < {self.hcf_coherence_offset_thresh:.3f}) -> Advance offset.")
                            # SPOD関連のログは削除
                            if coherence_too_low:
                        ...
                        # ...
                        last_frame = min(note_info['last_update_frame'], len(frame_times) - 1)
                        if last_frame < 0:
                             ended_track_ids.append(track_id)
                             continue
                        precise_offset_time = frame_times[last_frame] + hop_time

                        if not timed_out and coherence_too_low and hcf_available and 0 <= closest_f0_cand_idx_unmatched < C_map.shape[0] and frame_idx > 0:
                             try:
                                f0_map_idx = closest_f0_cand_idx_unmatched
                                prev_coherence = C_map[f0_map_idx, frame_idx - 1]
                                curr_coherence = C_map[f0_map_idx, frame_idx]
                                if prev_coherence >= self.hcf_coherence_offset_thresh > curr_coherence:
                                     coh_diff = curr_coherence - prev_coherence
                                     if abs(coh_diff) > 1e-9:
                                        frame_offset_ratio = (self.hcf_coherence_offset_thresh - prev_coherence) / coh_diff
                                        time_offset = frame_offset_ratio * hop_time
                                        precise_offset_time = frame_times[frame_idx - 1] + time_offset
                             except Exception as interp_e:
                                logger.warning(f"Frame {frame_idx}, Track {track_id}: Error during precise offset time interpolation: {interp_e}", exc_info=False)
                        ...
                        if duration >= self.min_note_duration_sec and pitches_hz:
                             ...
                             notes_list.append({
                                'start_time': start_time, 'offset_time': precise_offset_time,
                                'pitch_hz': pitch_median_hz, 'duration': duration,
            ...
            for c, f0_hz in enumerate(current_f0s_hz):
                if c not in matched_f0_indices:
                    ...
                    if is_onset:
                        ...
                        if hcf_available and 0 <= closest_f0_cand_idx_new < Phi_comb_map.shape[0] and frame_idx > 0 and frame_idx < Phi_comb_map.shape[1] - 1:
                            try:
                                flux_values = Phi_comb_map[closest_f0_cand_idx_new, frame_idx-1 : frame_idx+2]
                                if len(flux_values) == 3:
                                    delta_frame, _ = _parabolic_interpolation_numba(flux_values, 1)
                                    time_offset = delta_frame * hop_time
                                    precise_onset_time = current_time + time_offset
                            except Exception as interp_e:
                              logger.warning(f"Frame {frame_idx}, Track {track_id_counter}: Error during precise onset time interpolation: {interp_e}", exc_info=False)
        ...
        if self._times is not None and self._sr is not None:
             ...
             if hop_time <= 0: hop_time = 1024 / 44100
             ...
             for track_id, note_info in active_notes.items():
                  ...
                  if duration >= self.min_note_duration_sec and pitches_hz:
                     pitch_median_hz = np.median(np.array(pitches_hz))
                     notes_list.append({
                        'start_time': start_time, 'offset_time': offset_time,
                        'pitch_hz': pitch_median_hz, 'duration': duration,
                        'pitches_hz': pitches_hz
                     })
    ...
    def _postprocess_notes(self, notes_list: List[Dict[str, Any]]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if not notes_list: return np.array([]).reshape(0, 2), np.array([])

        logger.debug(f"[_postprocess_notes] Start: Received {len(notes_list)} raw notes.")
        ...
        notes = [{
         ...
         } for n in notes_list if n['pitch_hz'] > 0 and n['duration'] > 1e-6]
        logger.debug(f"[_postprocess_notes] After initial pitch/duration filter: {len(notes)} notes.")
        if not notes: return np.array([]).reshape(0, 2), np.array([])
        ...
        if notes:
            ...
            current_note_pitch_cents = hz_to_cents_numba(np.array([current_note['pitch']]))[0]

            for i in range(1, len(notes)):
                next_note = notes[i]
                gap = next_note['onset'] - current_note['offset']
                pitch2_cents = hz_to_cents_numba(np.array([next_note['pitch']]))[0]
                pitch_diff_cents = abs(current_note_pitch_cents - pitch2_cents) if np.isfinite(current_note_pitch_cents) and np.isfinite(pitch2_cents) else np.inf

                if 0 <= gap <= self.min_silence_duration_sec and pitch_diff_cents <= self.note_merge_tolerance_cents:
                    ...
                    if total_weight_duration > 1e-9:
                        current_note['pitch'] = (current_note['pitch'] * weight_duration1 + next_note['pitch'] * weight_duration2) / total_weight_duration
                    else:
                         current_note['pitch'] = (current_note['pitch'] + next_note['pitch']) / 2.0
                    current_note['offset'] = next_note['offset']
                    current_note['duration'] = current_note['offset'] - current_note['onset']
            ...
            merged_notes.append(current_note)
            notes = merged_notes
            logger.debug(f"[_postprocess_notes] After merging consecutive: {len(notes)} notes.")

        # --- Handle Overlaps (uses self.note_merge_tolerance_cents) ---
        logger.debug(f"[_postprocess_notes] Before overlap resolution: {len(notes)} notes.")
        ...
        if notes:
             ...
             while i < len(notes):
                 ...
                 while j < len(notes) and notes[j]['onset'] < current_note['offset']:
                     ...
                     pitch1_cents = hz_to_cents_numba(np.array([current_note['pitch']]))[0]
                     pitch2_cents = hz_to_cents_numba(np.array([other_note['pitch']]))[0]
                     pitch_diff_cents = abs(pitch1_cents - pitch2_cents) if np.isfinite(pitch1_cents) and np.isfinite(pitch2_cents) else np.inf

                     if pitch_diff_cents <= self.note_merge_tolerance_cents:
                         duration1 = current_note['duration']
                         duration2 = other_note['duration']
                         weight1 = duration1; weight2 = duration2
                         total_weight = weight1 + weight2
                         if total_weight > 1e-9:
                              current_note['pitch'] = (current_note['pitch'] * weight1 + other_note['pitch'] * weight2) / total_weight
                         else:
                              current_note['pitch'] = (current_note['pitch'] + other_note['pitch']) / 2.0
                         current_note['offset'] = max(current_note['offset'], other_note['offset'])
                         current_note['duration'] = current_note['offset'] - current_note['onset']
             ...
             logger.debug(f"[_postprocess_notes] After overlap resolution: {len(notes)} notes.")

        # --- Pitch Stability Filter (uses self.max_pitch_std_dev_cents) ---
        logger.debug(f"[_postprocess_notes] Before pitch stability filter (max_std_dev={self.max_pitch_std_dev_cents:.1f} cents): {len(notes)} notes.")
        ...
        if removed_unstable_count > 0:
            logger.debug(f"    Removed {removed_unstable_count} notes due to pitch instability.")
        notes = stable_notes
        logger.debug(f"[_postprocess_notes] After pitch stability filter: {len(notes)} notes.")

        # --- Final Duration Filter (uses self.min_note_duration_sec) ---
        logger.debug(f"[_postprocess_notes] Before final duration filter ({self.min_note_duration_sec:.3f}s): {len(notes)} notes.")
        final_notes_intermediate = [n for n in notes if n.get('duration', 0.0) >= self.min_note_duration_sec]
        logger.debug(f"[_postprocess_notes] Final notes after duration filter: {len(final_notes_intermediate)} notes.")

        if not final_notes_intermediate: return np.array([]).reshape(0, 2), np.array([])
        ...
        for note in notes: # 元の 'notes' (安定性フィルタ後) を使用
             if note.get('duration', 0.0) >= self.min_note_duration_sec:
                  ...
                  if len(pitches_hz) > 0:
                       # 元のシンプルな中央値計算に戻す
                       note['pitch_hz'] = np.median(pitches_hz) # 'pitch' キーではなく 'pitch_hz' に統一
                       final_notes.append(note)
                  # else: ピッチ履歴がないノートは含めない（元の挙動に依存）

        logger.debug(f"[_postprocess_notes] Final notes after duration filter: {len(final_notes)} notes.") # ログも修正


        if not final_notes: return np.array([]).reshape(0, 2), np.array([])

        intervals = np.array([[n['onset'], n['offset']] for n in final_notes]) # final_notes を使用
        pitches = np.array([n['pitch_hz'] for n in final_notes]) # 'pitch_hz' を使用

        logger.debug(f"[_postprocess_notes] Returning {len(pitches)} notes (using original median pitch calculation).") # ログも修正
    ...
    def detect(self, audio_data: np.ndarray, sr: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._freq_resolution_hz is None or self._freq_resolution_hz <= 0:
                 raise RuntimeError("Initialization failed, frequency resolution not set or invalid.")

            # --- Core Processing Steps ---
            logger.debug("1. Calculating STFT...")
            A_lin, S_db, phases = self._calculate_stft(audio_data)
            if A_lin.size == 0 or self._times is None: raise ValueError("STFT failed or produced empty results.")
            if self._freq_resolution_hz is None or self._freq_resolution_hz <= 0:
            ...
            if O_vec is not None:
                 ...
                 logger.warning("HCF O_vec calculation failed, cannot detect HCF onsets.")
            ...
            # --- ハイブリッドオンセットへの分岐ロジックを削除 ---
                 ...
                 logger.warning("HCF onset detection failed. Using empty set for allowed onsets.")
                 allowed_onset_frames = set()
            ...
            logger.info(f"Detection completed in {detection_time:.4f} seconds. Found {len(note_pitches)} notes.")

            # --- Format Results ---
            frame_frequencies = np.zeros_like(self._times, dtype=float) if self._times is not None else np.array([])
            ...
            result = {
                ...
                'additional_data': {
                    ...
                    'hcf_onset_count': len(allowed_onset_frames) if allowed_onset_frames is not None else 0
                }
            }
            return result
    ...
    def _empty_result(self, error_message: str = "Detection failed or invalid input") -> Dict[str, Any]:
        """Returns a standardized empty result dictionary."""
        params = {}
        try:
            params = self.get_params()
        except Exception:
             logger.warning("Could not retrieve parameters for empty result.", exc_info=False)

        return {
                'intervals': np.array([]).reshape(0, 2),
                'note_pitches': np.array([]),
                'frame_times': self._times if hasattr(self, '_times') and self._times is not None else np.array([]),
                'frame_frequencies': np.array([]),
                'detector_name': self.__class__.__name__ + "_HCF_Enhanced_Error",
                'detection_time': 0.0,
                'additional_data': {'error': error_message, 'note_count': 0, 'parameters': params}
        }

    # --- HCF Helper Methods (New) ---

    def _calculate_harmonic_metrics(self, frame_idx: int, f0_hz: float, spod_map: np.ndarray, A_lin: np.ndarray) -> Tuple[float, float]:
        """
        ...
        if self._sr is None or self._freq_resolution_hz is None or self._freqs is None:
            logger.error("Detector not properly initialized for harmonic metrics calculation.")
            return 0.0, 0.0
    ...
    def _calculate_hcf_maps(self, spod_map: np.ndarray, A_lin: np.ndarray) -> Tuple[Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray]]: # <<< 戻り値の型ヒント更新
        ...
        try:
            ...
            if self.hcf_onset_smoothing_sec > 0: # Use HCF param
                 smoothing_frames = max(1, int(round(self.hcf_onset_smoothing_sec * self._sr / self.hop_length))) # Use instance vars & HCF param
                 if smoothing_frames > 1 and len(O_vec) >= smoothing_frames:
                     logger.debug(f"Smoothing HCF onset function O_vec with window {smoothing_frames}")
                     O_vec_writable = np.require(O_vec, requirements=['W'])
                     uniform_filter1d(O_vec_writable, size=smoothing_frames, mode='nearest', output=O_vec_writable)
