...
class UnpredictabilityBasedDetector(BaseDetector):
    ...
    PARTIAL_MAX_INACTIVE_FRAMES = 3 # Max frames a partial can be missing before termination
    PARTIAL_MATCH_MAX_COST = 50.0 # Max cost (e.g., cents difference) for matching peaks to partials
    ...
    def __init__(self, beta: float = DEFAULT_BETA, **kwargs):
        ...
        # --- State for Tracking ---
        self._active_partials: Dict[int, Dict[str, Any]] = {} # track_id -> partial info
        self._partial_id_counter: int = 0

        logger.info(f"UnpredictabilityBasedDetector initialized with beta={self.beta:.3f}")
        logger.info(f"  -> Onset Sensitivity: {self.onset_sensitivity_level}")
        logger.info(f"  -> Smoothing Frames: {self.smoothing_frames}")
        logger.info(f"  -> Relative Onset Threshold: {self.relative_onset_threshold:.3f}")
    ...
    def _calculate_internal_params(self):
        ...
        self.smoothing_frames = max(1, int(np.ceil(smoothing_sec * sr_approx / hop_approx)))
        ...
        min_thresh_rel = 0.05
        max_thresh_rel = 0.30
        self.relative_onset_threshold = max_thresh_rel - (max_thresh_rel - min_thresh_rel) * self.beta

        # 3. Onset Sensitivity Level (descriptive)
        if self.beta > 0.75: self.onset_sensitivity_level = "High"
        elif self.beta > 0.4: self.onset_sensitivity_level = "Medium"
        else: self.onset_sensitivity_level = "Low"
        ...
        self.partial_match_cost_threshold = self.PARTIAL_MATCH_MAX_COST * (1.5 - self.beta)
    ...
    def _initialize_run(self, sr: int):
        ...
        self._freq_resolution_hz = self._freqs[1] - self._freqs[0] if len(self._freqs) > 1 else sr / self.N_FFT
    ...
    def _calculate_stft(self, audio_data: np.ndarray) -> Tuple[Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray]]:
        ...
        try:
            ...
            S_db = librosa.amplitude_to_db(S_amp, ref=ref_amp) # Use calculated max as ref
    ...
    def _calculate_spod(self, phases: np.ndarray) -> Optional[np.ndarray]:
        ...
        if n_frames <= 1 or self._sr is None or self._hop_time is None or self._hop_time <= 0:
             logger.warning("Cannot calculate SPOD: insufficient frames or invalid timebase.")
             return np.zeros_like(phases) if phases is not None else None

        spod_window_frames = max(1, int(np.ceil(SPOD_WINDOW_SEC_FIXED / self._hop_time)))
    ...
    def _detect_peaks_per_frame(self, A_lin: np.ndarray, S_db: np.ndarray) -> List[List[Tuple[float, float, float]]]:
        """
        ...
        max_bin_idx = np.searchsorted(self._freqs, self.PARTIAL_MAX_FREQ_HZ, side='right') - 1 # Use partial max freq

        if min_bin_idx >= max_bin_idx: # Use >= to handle edge case
             logger.warning(f"Invalid frequency range for peak detection: min_bin={min_bin_idx}, max_bin={max_bin_idx}")
        ...
        for t in range(n_frames):
            ...
            if safe_min_bin > safe_max_bin: continue # Skip frame if range is invalid after safety check

            db_t_slice = S_db[safe_min_bin : safe_max_bin + 1, t]

            if db_t_slice.size == 0: continue
            ...
            try:
                ...
                peak_indices_abs = safe_min_bin + peak_indices_rel # Convert back to absolute indices

                frame_peaks_data: List[Tuple[float, float, float]] = []
                for p_idx_abs in peak_indices_abs:
                    # Ensure index is valid for interpolation
                    if p_idx_abs <= 0 or p_idx_abs >= n_bins -1: continue

                    # --- Parabolic Interpolation on Amplitude Spectrum ---
                    delta_bin, interp_amp = _parabolic_interpolation_numba(amp_t_full, p_idx_abs)
                    interp_bin = p_idx_abs + delta_bin
                    interp_freq = self._bin_to_freq(interp_bin)

                    if interp_freq is not None and self.MIN_FREQ_HZ <= interp_freq <= self.PARTIAL_MAX_FREQ_HZ:
                ...
                logger.error(f"Frame {t}: Error during peak detection: {e}", exc_info=False)
    ...
    def _track_partials(self,
        ...
        for frame_idx in range(num_frames):
            ...
            cost_matrix = np.full((num_active_tracks, num_current_peaks), COST_INF, dtype=np.float64)
            ...
            if num_active_tracks > 0 and num_current_peaks > 0:
                ...
                active_last_freqs_hz = np.array([self._active_partials[tid]['last_freq'] for tid in active_track_ids])
                # Handle cases where last_freq might be invalid
                valid_last_freqs = active_last_freqs_hz > 0
                if not np.all(valid_last_freqs):
                     logger.warning(f"Frame {frame_idx}: Found invalid last frequencies in active partials.")
                     # Handle or filter - for now, proceed but costs might be INF
                     active_last_freqs_hz = np.where(valid_last_freqs, active_last_freqs_hz, 1.0) # Replace invalid with 1Hz to avoid log(0)
                ...
                if not np.all(valid_peaks):
                    logger.warning(f"Frame {frame_idx}: Found invalid peak frequencies.")
                    current_peak_freqs_hz = np.where(valid_peaks, current_peak_freqs_hz, 1.0)
                ...
                valid_cents_mask = np.isfinite(active_last_freqs_cents[:, np.newaxis]) & np.isfinite(current_peak_freqs_cents[np.newaxis, :])
                cent_diffs = np.full((num_active_tracks, num_current_peaks), COST_INF, dtype=np.float64)
                if np.any(valid_cents_mask):
                     diffs = np.abs(active_last_freqs_cents[:, np.newaxis] - current_peak_freqs_cents[np.newaxis, :])
            ...
            if num_active_tracks > 0 and num_current_peaks > 0 and np.any(cost_matrix < COST_INF):
                try:
                    ...
                    assigned_cols = col_ind[valid_assignment_mask]
                     ...
                     logger.error(f"Error in linear_sum_assignment at frame {frame_idx}: {e}")
                ...
                for r, c in zip(assigned_rows.astype(int), assigned_cols.astype(int)):
                    ...
                    peak_bin_idx = int(round(peak_bin_approx)) if peak_bin_approx is not None else -1

                    current_phase = 0.0
                    current_spod = 0.0
                    # Safely access phase/spod arrays
                    if phases_per_frame is not None and spod_map_per_frame is not None and \
                       peak_bin_idx >= 0 and peak_bin_idx < phases_per_frame.shape[0] and \
                       frame_idx < phases_per_frame.shape[1]:
                        try:
                           current_phase = phases_per_frame[peak_bin_idx, frame_idx]
                           current_spod = spod_map_per_frame[peak_bin_idx, frame_idx]
                        except IndexError:
                             logger.warning(f"Index error accessing phase/spod at frame {frame_idx}, bin {peak_bin_idx}")
            ...
            for c, peak_data in enumerate(current_peaks):
                if c not in matched_peak_indices:
                    ...
                    if peak_freq <= 0: continue # Skip invalid peaks
                       ...
                       frame_idx < phases_per_frame.shape[1]:
                        try:
                           ...
                           current_spod = spod_map_per_frame[peak_bin_idx, frame_idx]
                             ...
                             logger.warning(f"Index error accessing phase/spod for new partial at frame {frame_idx}, bin {peak_bin_idx}")
        ...
        logger.info(f"Partial tracking completed. Max partial ID: {self._partial_id_counter}")
    ...
    def _calculate_unpredictability(self, partial_states_per_frame: List[Dict[int, Dict[str, Any]]]) -> np.ndarray:
        """
    ...
    def _detect_onsets_from_unpredictability(self, unpredictability: np.ndarray) -> Set[int]:
        """
        ...
        Placeholder implementation.
        """
        if unpredictability.size == 0: return set()


        # 1. Smooth unpredictability using self.smoothing_frames
        if self.smoothing_frames > 1:
             smoothed_U = uniform_filter1d(unpredictability, size=self.smoothing_frames, mode='nearest')
        else:
        ...
        if max_U > 1e-9:
            ...
            norm_smoothed_U = smoothed_U # Avoid division by zero if signal is flat zero
        ...
        peak_threshold = self.relative_onset_threshold # Absolute threshold on 0-1 normalized signal
        ...
        try:
             ...
             logger.error(f"Error detecting onset peaks from unpredictability: {e}", exc_info=True)
             return set()


    def _segment_notes(self, partial_states_per_frame: List[Dict[int, Dict[str, Any]]], onset_frames: Set[int]) -> List[Dict[str, Any]]:
        """
            ...
            for frame_idx in sorted(list(onset_frames)):
                 if frame_idx < len(self._times):
                     ...
                     notes_list.append({
                         ...
                         'offset_time': start_time + 0.1, # Fixed 100ms duration placeholder
                         'pitch_hz': pitch_hz,
                         'duration': 0.1
                     })
        return notes_list
    ...
    def _postprocess_notes(self, notes_list: List[Dict[str, Any]]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        min_duration = 0.05 + 0.05 * (1 - self.beta) # Example: 50ms (high beta) to 100ms (low beta)

        processed_notes = [n for n in notes_list if n.get('duration', 0) >= min_duration]
        logger.info(f"Postprocessing: Kept {len(processed_notes)} notes after min duration filter ({min_duration:.3f}s).")


        # Placeholder: Return notes after basic duration filter
        try:
            if not processed_notes: return np.array([]).reshape(0, 2), np.array([])
            intervals = np.array([[n['start_time'], n['offset_time']] for n in processed_notes])
    ...
    def detect(self, audio_data: np.ndarray, sr: int) -> Dict[str, Any]:
        ...
        logger.info(f"Starting detection ({self.__class__.__name__} beta={self.beta:.3f}) with SR={sr}.")
        ...
        try:
            ...
            if spod_map is None:
                logger.warning("SPOD calculation failed. Proceeding without SPOD info for tracking.")
                spod_map_for_tracking = None
            elif spod_map.shape != S_amp.shape:
                 logger.warning(f"SPOD map shape {spod_map.shape} mismatch with STFT {S_amp.shape}. Proceeding without SPOD info.")
                 spod_map_for_tracking = None
            else:
            ...
            partial_states_per_frame = self._track_partials(peaks_per_frame, phases, spod_map_for_tracking)

            # --- 5. Unpredictability Calculation (Placeholder) ---
            logger.debug("Calculating frame-wise unpredictability...")
            unpredictability = self._calculate_unpredictability(partial_states_per_frame)
    ...
    def _empty_result(self, error_message: str = "Detection failed or invalid input") -> Dict[str, Any]:
        ...
        return {
                ...
                'detector_name': self.__class__.__name__,
                'detection_time': 0.0,
                'additional_data': {'error': error_message, 'note_count': 0, 'parameters': self.get_params()}
        }
