...
try:
    from src.detectors import (  # __init__.py から register_detector をインポート
        register_detector,
    )
    ...
    logging.error(
        f"Failed to import BaseDetector or register_detector: {e}. Ensure 'src' is in PYTHONPATH."
    )
...
class CriteriaDetector(BaseDetector):
    ...
    def __init__(self, **kwargs: Any):
        ...
        **kwargs : Any
        ...
        super().__init__(
            **kwargs
        )  # BaseDetectorの__init__を呼び出し、self.params を設定

        # crepeが利用可能かチェック (インポート時に確認済みだが念のため)
        if crepe is None:
            raise ImportError(
                "CREPE library is not available. Cannot initialize CriteriaDetector."
            )
        ...
        logger.info(
            f"{self.__class__.__name__} (v{self.version}) initialized with effective parameters:"
        )
    ...
    def detect(self, audio_data: np.ndarray, sr: int) -> Dict[str, Any]:
        ...
        logger.info(
            f"Starting detection on audio with duration={len(audio_data)/sr:.2f}s, sr={sr}Hz..."
        )
        ...
        try:
            logger.debug(
                f"Running CREPE pitch detection (model: {self.crepe_model_capacity}, step: {self.crepe_step_size}ms)..."
            )
            ...
            if np.any(invalid_pitch_mask):
                logger.debug(
                    f"Found {np.sum(invalid_pitch_mask)} invalid pitch values (<=0 or NaN) from CREPE, setting to NaN."
                )
            ...
            frame_times, frame_frequencies, frame_confidences = (
                np.array([]),
                np.array([]),
                np.array([]),
            )
        ...
        if len(offsets) == 0:  # 通常は onsets と同じ長さのはずだが念のため
            logger.warning(
                "Offset detection resulted in an empty array. Using default durations."
            )
        ...
        if len(onsets) > 0:
            ...
            for i in range(num_notes_to_pair):
                ...
                next_onset_boundary = (
                    onsets[i + 1] - 1e-3 if i + 1 < len(onsets) else total_duration
                )
                ...
                offset_time = min(
                    candidate_offset_time, next_onset_boundary, max_offset_boundary
                )
                ...
                if (
                    ...
                    and (offset_time - onset_time) >= self.min_note_duration
                ):
                    valid_intervals.append([onset_time, offset_time])
                    valid_onsets_for_pitch.append(onset_time)  # 対応するオンセット時刻
                else:
                    logger.debug(
                        f"Note {i+1} (onset {onset_time:.3f}s) discarded due to duration constraints "
                        f"(candidate offset: {candidate_offset_time:.3f}, final offset: {offset_time:.3f})."
                    )

        intervals = (
            np.array(valid_intervals) if valid_intervals else np.array([]).reshape(0, 2)
        )
        logger.info(
            f"Filtered notes based on duration and overlap. Kept {len(intervals)} notes."
        )
        ...
        if intervals.shape[0] > 0 and frame_times.shape[0] > 0:
            ...
            note_pitches = self._extract_note_pitches(
                intervals, frame_times, frame_frequencies, frame_confidences
            )
            logger.debug("Pitch extraction complete.")
        elif intervals.shape[0] > 0:
            logger.warning(
                "Valid note intervals found, but no frame data available for pitch extraction. Returning 0Hz pitches."
            )
            note_pitches = np.zeros(
                intervals.shape[0]
            )  # フレーム情報がない場合は 0 Hz とする
    ...
    def _detect_onsets(self, audio: np.ndarray, sr: int) -> np.ndarray:
        ...
        try:
            ...
            onset_times = librosa.frames_to_time(
                onset_frames, sr=sr, hop_length=self.onset_hop_length
            )
    ...
    def _detect_offsets(
        self, audio: np.ndarray, sr: int, onsets: np.ndarray
    ) -> np.ndarray:
        """
        ...
        try:
            ...
            rms = librosa.feature.rms(
                ...
                0
            ]  # 結果は (1, num_frames) なので [0] で1次元配列に

            # 2. RMSフレームに対応する時刻を計算
            rms_times = librosa.frames_to_time(
                np.arange(len(rms)), sr=sr, hop_length=self.offset_hop_length
            )
            ...
            for i, onset_time in enumerate(onsets):
                ...
                next_onset_time = (
                    onsets[i + 1] if i + 1 < len(onsets) else total_duration
                )
                ...
                if (
                    start_rms_idx < end_rms_idx
                ):  # 区間内にRMSフレームが1つ以上存在する場合
                    # 区間内のRMS値と時刻を取得
                ...
                if len(rms_segment) > 0:
                    ...
                    decay_indices = np.where(
                        rms_segment[peak_idx_in_segment:] < offset_rms_threshold
                    )[0]
                    ...
                    if len(decay_indices) > 0:
                        ...
                        offsets[i] = min(
                            offset_time, next_onset_time - 1e-6
                        )  # 微小値を引いて境界一致を防ぐ
                ...
                if not offset_found_for_this_note:
                    ...
                    offsets[i] = min(
                        default_offset_candidate, next_onset_time - 1e-6
                    )  # 微小値を引いて境界一致を防ぐ
                    logger.debug(
                        f"Offset not found via RMS decay for note {i+1} (onset {onset_time:.3f}s). Using calculated default offset: {offsets[i]:.3f}s"
                    )

            # 念のため、オフセット時刻が対応するオンセット時刻より前にならないようにクリップ
            offsets = np.maximum(
                onsets + 1e-6, offsets
            )  # 非常に小さい値を加えて同値にならないように
            ...
            logger.warning(
                "Offset detection error. Falling back to default durations limited by next onset."
            )
            ...
            for i, onset_time in enumerate(onsets):
                ...
                fallback_offsets[i] = min(
                    onset_time + self.default_note_duration, next_onset_time - 1e-6
                )
            return np.maximum(onsets + 1e-6, fallback_offsets)

    def _extract_note_pitches(
        self,
        ...
        for i, (onset, offset) in enumerate(intervals):
            ...
            #    - ピッチが NaN でない
            #    - 信頼度が閾値以上
            valid_mask = ~np.isnan(interval_freqs) & (
                interval_confs >= self.pitch_confidence_threshold
            )
            ...
            if np.any(valid_mask):
                ...
                if sum_confs > 1e-6:  # 小さな値より大きいか確認
                    ...
                    if np.isfinite(weighted_pitch) and weighted_pitch > 0:
                        ...
                        logger.warning(
                            f"Weighted pitch calculation resulted in invalid value ({weighted_pitch}) for note {i+1}. Using median."
                        )
                        # フォールバックとして単純な中央値を使う
                    ...
                    # 単純な中央値（または平均値）を使用
                    logger.debug(
                        f"Sum of confidences is very low ({sum_confs}) for note {i+1}. Using median pitch."
                    )
                    note_pitches[i] = np.nanmedian(valid_pitches)
                ...
                logger.debug(
                    f"No frames with sufficient confidence found for note {i+1} ({onset:.3f}-{offset:.3f}s). Pitch set to 0 Hz."
                )
        ...
        logger.debug(
            f"Extracted {len(note_pitches)} note pitches (0 Hz indicates no reliable pitch)."
        )
        return note_pitches
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "audio_file", type=str, help="Path to the input audio file (e.g., WAV, MP3)."
    )
    ...
    for param, default_value in CriteriaDetector.default_params.items():
        ...
        if param_type == bool:
            ...
            param_group.add_argument(
                arg_name, type=param_type, default=default_value, help=f"{param}"
            )
    ...
    params_from_args = {
        ...
        for key in CriteriaDetector.default_params
        if hasattr(args, key)
    }
    ...
    try:
        ...
        print("\n--- Detection Results ---")
        print(
            f"Detector: {results.get('detector_name', 'N/A')} (v{getattr(detector, 'version', 'N/A')})"
        )
        ...
        if num_detected_notes > 0:
            ...
            for i in range(min(num_detected_notes, max_notes_to_display)):
                ...
                print(
                    f"{i+1:>3} | {onset:9.3f} | {offset:9.3f} | {duration:10.3f} | {pitch_str}"
                )
            print("----------------------------------------------------------")
            if num_detected_notes > max_notes_to_display:
                print(
                    f"  ... (displaying first {max_notes_to_display} notes of {num_detected_notes})"
                )
        else:
        ...
        logger.error(
            f"Import error: {e}. Please ensure all dependencies (crepe, librosa, numpy) are installed."
        )
        print(
