...
from src.detectors.modules.filterbank import CQTFilterBank, FilterBank, STFTFilterBank
...
class ONDEFIVEDetector(BaseDetector):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.time_domain_detector = TimeDomainPitchDetector(
            min_f0=min_f0, max_f0=max_f0, sr=sr
        )
        ...
        self.min_harmonics_for_offset = (
            min_harmonics_for_offset  # オフセット判定に必要な最小倍音数
        )
        if "harmonic_decay_frames" in kwargs:
            ...
            self.harmonic_decay_frames = max(
                2, int(1.0 + 0.2 * theta)
            )  # set_thetaと同じ計算式
        self.harmonic_decay_alpha_factor = (
            harmonic_decay_alpha_factor  # 減衰中の背景モデル更新速度係数
        )
        ...
        if self.enable_debug:
            logging.debug(
                f"ONDEFIVEDetector: 初期化完了 theta={theta}, feature_dim={feature_dim}"
            )
    ...
    def process_frame(self, audio_buffer, spec_frame=None, freq_bins=None):
        ...
        frame_data = {
            ...
            "spec_frame": (
                spec_frame.copy() if self.enable_debug else None
            ),  # デバッグモード時のみスペクトルも保存
            "z_score": z_score,
            "is_onset": is_onset,
            "is_offset": is_offset,
    ...
    def _compute_spectrogram(self, audio_buffer):
        ...
        if not hasattr(self, "_prev_spec_energy"):
            ...
            self._prev_spec_energy = np.append(
                self._prev_spec_energy, np.sum(magnitude_spec)
            )
    ...
    def _calculate_harmonic_score(self, spec_frame, freq_bins, f_time):
        ...
        if self.adaptive_harmonics:
            ...
            if np.sum(spec_frame) > self.eps:
                ...
                self.used_harmonics = min(
                    self.max_harmonics, max(3, int(spectral_centroid / f_time / 2))
                )
            else:
            ...
            if bin_idx < len(spec_frame):
                ...
                if h_key not in self.harmonic_models:
                    self.harmonic_models[h_key] = GaussianBackgroundModel(
                        alpha_base=self.alpha_base, initial_mean=0.1, initial_var=0.01
                    )
                ...
                if abs(h_energy - self.harmonic_models[h_key].mean) <= 1.0 * np.sqrt(
                    self.harmonic_models[h_key].var
                ):
        ...
        # 倍音減衰度を計算
        self._calculate_harmonic_decay_metric(
            harmonic_energies, harmonic_bg_means, harmonic_bg_stds
        )

        # 倍音構造のスコアを計算
        harmonic_score = total_energy * np.sqrt(
            1 + total_deviation
        )  # エネルギーと逸脱の関数
    ...
    def _prune_harmonic_models(self):
        ...
        if self.enable_debug:
            logging.debug(
                f"ハーモニクスモデル整理: {len(expired_keys)}個削除, 残り{len(self.harmonic_models)}個"
            )
    ...
    def _calculate_harmonic_decay_metric(self, energies, bg_means, bg_stds):
        ...
        for energy, mean, std in zip(energies, bg_means, bg_stds):
            ...
            # 1.0 以下なら背景レベルに近いと判断
            decay_score = (
                1.0 if rel_distance <= 1.0 else max(0.0, 2.0 - rel_distance) / 1.0
            )
        ...
        if self.enable_debug and decay_metric >= self.harmonic_decay_threshold:
            logging.debug(
                f"倍音減衰検出: {decay_metric:.2f} (閾値: {self.harmonic_decay_threshold:.2f}), "
    ...
    def _check_pitch_decay_event(self, pitch_freq):
        ...
        # 基本的なバリデーション
        if (
            pitch_freq < self.min_f0
            or self._current_freq_bins is None
            or self._current_spec_frame is None
            or len(self._current_freq_bins) == 0
            or len(self._current_spec_frame) == 0
        ):
            return False, 0.0
        ...
        if window_end > window_start:
            surrounding_energy = np.mean(
                self._current_spec_frame[window_start : window_end + 1]
            )
        ...
        decay_score = (
            0.3 * main_decay_score
            + 0.3 * surrounding_energy_decay_score
            + 0.4 * self._harmonic_decay_metric
        )

        # 最終的な減衰判定
        # 1. 基本的な減衰条件が満たされている
        # 2. かつ倍音減衰度が十分高い
        decay_detected = basic_decay and (
            harmonic_decay
            or self._harmonic_decay_metric >= self.harmonic_decay_threshold
        )

        if self.enable_debug and decay_detected:
            logging.debug(
                f"ピッチ減衰検出: スコア={decay_score:.2f}, "
    ...
    def _detect_events(self, p_event):
        ...
        if not self.in_event:
            ...
            if p_event > self.onset_prob:
                ...
                if self.enable_debug:
                    logging.debug(
                        f"オンセット検出: p_event={p_event:.2f}, onset_prob={self.onset_prob:.2f}"
                    )
        else:
            # イベント状態での処理
            self.event_frames += 1
            ...
            # 2. かつピッチ減衰が検出された
            # 3. かつハーモニクス減衰度が閾値以上
            harmonic_decay_condition = (
                self._harmonic_decay_count >= self.harmonic_decay_frames
            )

            # より厳しいAND条件（すべての条件を満たす必要がある）
            # 条件を意味のある変数に分解
            low_event_probability = p_event < self.offset_prob
            sufficient_harmonic_decay = (
                harmonic_decay_condition
                or self._harmonic_decay_metric >= self.harmonic_decay_threshold
            )
            offset_condition_met = (
                low_event_probability and pitch_decayed and sufficient_harmonic_decay
            )
            ...
            if offset_condition_met:
                ...
                if self.enable_debug and self.low_event_count == 1:
                    logging.debug(
                        f"オフセット条件検出開始: p_event={p_event:.2f}, "
            ...
            if self.low_event_count >= self.offset_wait_frames:
                ...
                if self.event_frames >= min_event_frames:
                    ...
                    if self.enable_debug:
                        logging.debug(
                            f"オフセット検出確定: p_event={p_event:.2f}, "
            ...
            if self.event_frames > max_event_frames and p_event < self.onset_prob:
                ...
                if self.enable_debug:
                    logging.debug(
                        f"長期イベント終了: p_event={p_event:.2f}, event_frames={self.event_frames} > max={max_event_frames}"
                    )
    ...
    def set_theta(self, new_theta):
        ...
        self.harmonic_decay_threshold = max(
            0.4, min(0.8, self.harmonic_decay_threshold)
        )

        # 背景モデル更新速度係数 - thetaが大きいほど抑制的に
        self.harmonic_decay_alpha_factor = 1.2 + 0.05 * (new_theta - 1.0)
        self.harmonic_decay_alpha_factor = max(
            1.0, min(1.5, self.harmonic_decay_alpha_factor)
        )
        ...
        if self.enable_debug:
            logging.debug(
                f"メタパラメータを更新: theta={new_theta:.2f}, alpha_base={self.alpha_base:.4f}, "
    ...
    def _compute_spectrum_deviation(self, spec_frame, freq_bins):
        ...
        for bin_idx in np.where(valid_indices)[0]:
            ...
            # 平均の更新
            self.freq_bg_models[bin_key]["mean"] = (
                alpha * bg_mean + (1 - alpha) * energy
            )
        ...
        normalized_deviation = (
            total_deviation / np.sum(valid_indices)
        ...
        for idx in peak_indices:
            ...
            for j in peak_indices:
                ...
                if abs(round(ratio) - ratio) < 0.05 and round(ratio) > 1:
                    ...
                    pitch_candidates[-1] = (
                        freq,
                        amplitude + spec_frame[j] / round(ratio),
                    )
    ...
    def _optimize_offset_time(self, frame_time, buffer_size, sr, max_lookback=5):
        ...
        if hasattr(self, "_offset_decay_info"):
            ...
            if (
                decay_info["harmonic_decay_metric"] > 0.7
                and decay_info["decaying_harmonics_count"]
                >= self.min_harmonics_for_offset
            ):
                # より早期のオフセットを優先
            ...
            if len(self.recent_frame_data) > i:
                ...
                if harmonic_decay_adjusted:
                    ...
                    if (
                        ...
                        and energy_ratio < min_energy_ratio
                    ):
        ...
        optimized_offset_time = max(
            0.0, frame_time - (offset_frames_back * hop_time) + (buffer_size / (2 * sr))
        )
        ...
        if self.enable_debug:
            ...
            logging.debug(
                f"オフセット時間最適化: 現在時間={frame_time:.3f}s → 最適時間={optimized_offset_time:.3f}s "
    ...
    def detect(self, audio_data: np.ndarray, sr: int) -> Dict[str, Any]:
        ...
        if np.isnan(audio_data).any() or np.isinf(audio_data).any():
            logging.warning(
                "オーディオデータにNaNまたは無限大の値が含まれています。これらの値を0に置き換えます。"
            )
        ...
        if total_frames > 10000:  # 約3分以上の音声
            ...
            logging.info(
                f"長時間音声データを検出: {total_frames}フレーム、メモリ最適化を適用"
            )
            # 定期的にメモリを解放するための処理
        ...
        if self.enable_debug:
            logging.debug(
                f"検出開始: 音声長={len(audio_data)/sr:.2f}秒, 総フレーム数={total_frames}"
            )
        ...
        for i in range(0, len(audio_data) - buffer_size, hop_length):
            ...
            if self.enable_debug and frame_idx % progress_step == 0:
                ...
                logging.debug(
                    f"検出進捗: {progress:.1f}% ({frame_idx}/{total_frames}フレーム)"
                )
            ...
            # エネルギー計算（スペクトラムの合計）
            if (
                hasattr(self, "_current_spec_frame")
                and self._current_spec_frame is not None
            ):
            ...
            if frame_result["is_offset"] and current_onset is not None:
                ...
                offset_time = self._optimize_offset_time(
                    frame_time, buffer_size, sr, max_lookback=5
                )

                # デバッグ情報を保存
                if self.enable_debug:
                    time_adjustment = offset_time - (
                        frame_time + (buffer_size / (2 * sr))
                    )
                ...
                if avg_pitch >= self.min_f0 and avg_pitch <= self.max_f0:
                    ...
                    if self.enable_debug:
                        ...
                        logging.debug(
                            f"ノート検出: {current_onset:.3f}s → {offset_time:.3f}s (長さ: {note_duration:.3f}s, ピッチ: {avg_pitch:.1f}Hz)"
                        )
        ...
        if current_onset is not None:
            ...
            if avg_pitch >= self.min_f0 and avg_pitch <= self.max_f0:
                ...
                if self.enable_debug:
                    note_duration = offset_time - current_onset
                    logging.debug(
                        f"最終ノート: {current_onset:.3f}s → {offset_time:.3f}s (長さ: {note_duration:.3f}s, ピッチ: {avg_pitch:.1f}Hz)"
                    )
        ...
        if len(onsets) > 0:
            ...
            ) = self._post_process_notes(onsets, offsets, pitches, confidences, sr)
    ...
    def _post_process_notes(self, onsets, offsets, pitches, confidences, sr):
        ...
        for i in range(1, len(onsets)):
            ...
            if gap_duration < self.min_gap_duration:
                ...
                # レガート判定：エネルギーが連続している場合
                    ...
                    if is_musical and interval_type in [
                        "semitone",
                        "wholetone",
                        "third",
                    ]:
            ...
            if should_merge:
                ...
                if prev_duration + curr_duration > 0:
                    ...
                    if "レガート" in merge_reason:
                        ...
                        prev_pitch = (
                            prev_pitch  # 変更なし、以降のロジックで必要に応じて変更
                        )
                    else:
                        # 通常のノート統合（加重平均）
                        prev_pitch = (
                            prev_pitch * prev_duration + curr_pitch * curr_duration
                ...
                if prev_offset - prev_onset >= self.min_note_duration:
                    ...
                    if self.enable_debug and merge_reasons:
                        logging.debug(
                            f"ノート統合: {prev_onset:.3f}-{prev_offset:.3f}秒 ({prev_pitch:.1f}Hz), 理由: {', '.join(merge_reasons)}"
                        )
                        merge_reasons = []
        ...
        if self.enable_debug and len(new_onsets) < len(onsets):
            logging.debug(
                f"ノート後処理: {len(onsets)}個 → {len(new_onsets)}個 ({len(onsets) - len(new_onsets)}個削減)"
            )
    ...
    def _is_musical_interval(self, pitch_ratio, tolerance=0.03):
        ...
        # 長3度・短3度（比率 ≈ 1.25-1.2）
        if (
            abs(pitch_ratio - 5 / 4) < tolerance * 1.5
            or abs(pitch_ratio - 6 / 5) < tolerance * 1.5
        ):
            return True, "third"
    ...
    def _analyze_energy_transition(
        self, curr_onset, prev_offset, frame_times, frame_energies
    ):
        """
        ノート間のエネルギー遷移を分析
        ...
        if (
            gap_start_idx < 0
            or gap_end_idx >= len(frame_times)
            or gap_end_idx <= gap_start_idx
        ):
            return {
        ...
        post_energies = frame_energies[
            gap_end_idx : min(len(frame_energies), gap_end_idx + 3)
        ]
        ...
        energy_avg = (
            (pre_energy + post_energy) / 2
            if pre_energy > 0 and post_energy > 0
            else max(pre_energy, post_energy)
        )
        ...
        is_continuous = (
            energy_drop_ratio > 0.7
        )  # 70%以上のエネルギーが維持されていれば連続と判断
