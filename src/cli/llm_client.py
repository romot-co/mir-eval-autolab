...
class AnthropicClient:
    ...
    def __init__(
        ...
        if not api_key:
            ...
            raise LLMClientError(
                "API key is required for Anthropic client",
    ...
    async def generate(
        ...
        """
        self.logger.info(
            f"Sending prompt to {self.model} (length: {len(prompt)} chars)"
        )
        ...
        try:
            ...
            self.logger.info(
                f"Received response from {self.model} in {elapsed_time:.2f}s"
            )
                ...
                try:
                    ...
                    if match_json_block:
                        ...
                        if (
                            ...
                            self.logger.debug(
                                "Extracted JSON string by finding first '{' and last '}'."
                            )
                        else:
                            self.logger.warning(
                                "Could not find JSON object structure ('{'...'}' or ```json ... ```) in the response."
                            )
                    ...
                    self.logger.error(
                        f"Unexpected error during JSON processing: {e}", exc_info=True
                    )
            ...
            raise LLMClientError(
                error_msg, error_type="APIError", original_error=api_err
            )
        except asyncio.TimeoutError as timeout_err:  # asyncio.TimeoutError をキャッチ
            error_msg = (
                f"Timeout error when calling Anthropic API (timeout: {self.timeout}s)"
            )
            self.logger.error(error_msg)
            raise LLMClientError(
                error_msg, error_type="TimeoutError", original_error=timeout_err
            )
        except Exception as e:
            error_msg = f"Unexpected error when calling Anthropic API: {e}"
            self.logger.error(error_msg, exc_info=True)
            raise LLMClientError(
                error_msg, error_type="UnexpectedError", original_error=e
            )
