...
from typing import Dict, Any, Callable, Coroutine, Awaitable, Optional
from contextlib import asynccontextmanager # lifespan用

from mcp.server.fastmcp import FastMCP, Context # Contextもインポートしておく
...
try:
    from src.mcp_server_logic import core, schemas, db_utils, session_manager, job_manager, \
        evaluation_tools, code_tools, llm_tools
...
async def lifespan_manager(server: FastMCP):
    ...
    if app_config is None or app_db_path is None:
         logger.error("Lifespan startup: Config or DB path is None. This should not happen.")
         raise RuntimeError("Configuration or DB path not initialized before lifespan startup.")

    # バックグラウンドタスクの開始
    try:
        num_workers = app_config.get('resource_limits', {}).get('max_concurrent_jobs', 2)
        logger.info(f"Starting {num_workers} job workers...")
        job_worker_task = asyncio.create_task(job_manager.start_job_workers(num_workers, app_config, app_db_path))
        ...
        cleanup_task = asyncio.create_task(core.start_cleanup_task(
            app_config,
        ...
        logger.critical(f"Failed to start background tasks during lifespan startup: {e}", exc_info=True)
    ...
    try:
        ...
        if cancelled_tasks:
            try:
                ...
                logger.warning("Gather operation was cancelled during background task shutdown.")
            except Exception as e:
                logger.error(f"Error during background task cleanup (gather): {e}", exc_info=True)
        else:
...
async def setup_server(args: argparse.Namespace) -> Optional[FastMCP]:
    ...
    try:
        ...
        logger.info(f"Created FastMCP server instance '{mcp.name}' with lifespan manager.")
    except Exception as e:
    ...
    start_async_job_func = lambda task_coro_func, tool_name, session_id=None, *args, **kwargs: \
        job_manager.start_async_job(app_config, app_db_path, task_coro_func, tool_name, session_id, *args, **kwargs)

    add_history_async_func: Callable[..., Awaitable[None]] = \
        lambda session_id, event_type, event_data, cycle_state_update=None: \
            session_manager.add_session_history(app_config, app_db_path, session_id, event_type, event_data, cycle_state_update)

    # --- Register Tools from Modules --- #
    logger.info("Registering MCP tools...")
    try:
        evaluation_tools.register_evaluation_tools(mcp, app_config, start_async_job_func, add_history_async_func)
        code_tools.register_code_tools(mcp, app_config, start_async_job_func, app_db_path)
        session_manager.register_session_tools(mcp, app_config, app_db_path)
        llm_tools.register_llm_tools(mcp, app_config, start_async_job_func, add_history_async_func)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--port", type=int, default=5002, help="Server port (Note: may not be used by mcp.run())")
    parser.add_argument("--host", default="0.0.0.0", help="Server host (Note: may not be used by mcp.run())")
    parser.add_argument("--log-level", default="info",
                        choices=["debug", "info", "warning", "error", "critical"], help="Application log level")
    ...
    try:
        ...
        logger.critical(f"サーバー実行中に予期せぬエラーが発生 (main level): {e}\n{traceback.format_exc()}")
