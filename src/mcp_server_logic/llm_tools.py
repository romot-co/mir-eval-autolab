...
try:
    jinja_env = Environment(
        ...
        trim_blocks=True,
        lstrip_blocks=True,  # Template readability improvements
    )
    logger.info(f"Jinja2 environment loaded from: {prompt_template_dir}")
except Exception as e:
    logger.critical(
        f"Failed to initialize Jinja2 environment from {prompt_template_dir}: {e}",
        exc_info=True,
    )
...
async def _render_prompt(
    ...
    if not jinja_env:
        raise MirexError(
            "Jinja2 environment is not initialized.", error_type="ConfigurationError"
        )  # LLMErrorからMirexErrorに変更

    session_history_summary = "No session history available."
    if session_id and config and db_path:
        try:
            session_history_summary = (
                await session_manager.get_session_summary_for_prompt(
                    config=config, db_path=db_path, session_id=session_id
                )
            )
        except Exception as e:
            logger.error(
                f"Error fetching session summary for prompt context: {e}", exc_info=True
            )
    ...
    try:
        ...
        logger.error(
            f"Prompt template not found: {template_name}.j2 in {prompt_template_dir}"
        )
        raise MirexError(
            f"Prompt template '{template_name}.j2' not found.",
            error_type="ConfigurationError",
        )
    except Exception as e:
        logger.error(
            f"Error rendering prompt template '{template_name}': {e}", exc_info=True
        )
        raise MirexError(
            f"Error rendering prompt template '{template_name}': {e}",
            error_type="PromptRenderingError",
        ) from e
...
async def _generate_prompt_task_base(
    ...
    **kwargs,  # Specific start event data
    ...
    """プロンプト生成タスクの共通処理"""
    logger.info(
        f"[Job {job_id}] Starting prompt generation ({template_name})... Session: {session_id}"
    )
    ...
    try:
        ...
        raise MirexError(
            f"Failed to validate DB path: {e}", error_type="PathValidationError"
        ) from e
    ...
    try:
        ...
        await add_history_func(
            session_id,
            f"{history_event_prefix}_prompt_generation_started",
            start_event_data.model_dump(),
        )
    except Exception as hist_e:
        logger.warning(
            f"[Job {job_id}] Failed to add {history_event_prefix}_prompt_generation_started history: {hist_e}"
        )

    generated_prompt = ""
    try:
        generated_prompt = await _render_prompt(
            template_name, context, session_id, config, db_path
        )

        elapsed_time = time.monotonic() - start_time
        logger.info(
            f"[Job {job_id}] Prompt generation ({template_name}) successful ({elapsed_time:.2f}s)."
        )
        ...
        # 完了履歴
        try:
            complete_event_data = complete_event_schema(
                job_id=job_id, prompt=generated_prompt
            )
            await add_history_func(
                session_id,
                f"{history_event_prefix}_prompt_generation_complete",
                complete_event_data.model_dump(),
            )
        except Exception as hist_e:
            logger.warning(
                f"[Job {job_id}] Failed to add {history_event_prefix}_prompt_generation_complete history: {hist_e}"
            )
        ...
        MirexError
    ) as mirex_err:  # PromptRenderingError や ConfigurationError をキャッチ
        elapsed_time = time.monotonic() - start_time
        logger.error(
            f"[Job {job_id}] MirexError during prompt generation ({template_name}) ({elapsed_time:.2f}s): {mirex_err}",
            exc_info=False,
        )
        try:
            fail_event_data = fail_event_schema(
                job_id=job_id,
                error=str(mirex_err),
                error_type=mirex_err.error_type or "PromptGenerationError",
                context_used=context,  # Context を含める
            )
            await add_history_func(
                session_id,
                f"{history_event_prefix}_prompt_generation_failed",
                fail_event_data.model_dump(),
            )
        except Exception as hist_e:
            logger.warning(
                f"[Job {job_id}] Failed to add {history_event_prefix}_prompt_generation_failed history (MirexError): {hist_e}"
            )
        ...
        logger.error(
            f"[Job {job_id}] Unexpected {error_type} during prompt generation ({template_name}) ({elapsed_time:.2f}s): {e}",
            exc_info=True,
        )
        try:
            fail_event_data = fail_event_schema(
            ...
            logger.warning(
                f"[Job {job_id}] Failed to add {history_event_prefix}_prompt_generation_failed history (Unexpected): {hist_e}"
            )
        # 予期せぬエラーは MirexError でラップして再raiseする
        raise MirexError(
            f"Unexpected error during prompt generation: {e}",
            error_type="UnexpectedPromptGenerationError",
        ) from e
...
async def _run_get_suggest_exploration_strategy_prompt_async(
    ...
    try:
        ...
        logger.error(
            f"[Job {job_id}] Failed to validate DB path for strategy prompt: {e}"
        )
        raise MirexError(
            f"Failed to validate DB path for strategy prompt: {e}",
            error_type="PathValidationError",
        ) from e

    try:
        session_info = await session_manager.get_session_info(
            config, db_path, session_id
        )
        ...
        context = {
            ...
            "base_algorithm": session_info.base_algorithm,
            "user_goal": session_info.improvement_goal,
            "cycle_state": cycle_state_obj.model_dump(),
            "current_metrics": session_info.current_metrics,
        }
    except Exception as e:
        logger.error(
            f"Failed to get session info for strategy prompt context (session: {session_id}): {e}",
            exc_info=True,
        )
        raise MirexError(
            f"Could not retrieve session info for strategy prompt: {e}"
        ) from e
...
async def _run_get_generate_hypotheses_prompt_async(
    ...
    try:
        ...
        logger.error(
            f"[Job {job_id}] Failed to validate DB path for hypotheses prompt: {e}"
        )
        raise MirexError(
            f"Failed to validate DB path for hypotheses prompt: {e}",
            error_type="PathValidationError",
        ) from e
    ...
    try:
        ...
        logger.warning(
            f"Could not fetch user goal for hypotheses prompt (session: {session_id}): {e}"
        )
...
async def _run_get_assess_improvement_prompt_async(
    ...
    """改善評価プロンプトを生成する"""
    context = {
        "original_detector_code": original_detector_code,
        "improved_detector_code": improved_detector_code,
...
def register_llm_tools(
    ...
    async def _start_prompt_gen_job(
        ...
        **kwargs,  # Specific args for the task factory
        ...
        job_description = f"Prompt Generation job ({tool_name})" + (
    ...
    async def get_improve_code_prompt_tool(
        ...
        return await _start_prompt_gen_job(
            _run_get_improve_code_prompt_async,
    ...
    async def get_suggest_parameters_prompt_tool(
        session_id: str,
        detector_code: str,
        analysis_results: Optional[Dict[str, Any]] = None,
        current_metrics: Optional[Dict[str, Any]] = None,
    ...
    async def get_analyze_evaluation_prompt_tool(
        session_id: str, evaluation_results: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def get_suggest_exploration_strategy_prompt_tool(
        session_id: str,
    ) -> Dict[str, Any]:
    ...
    async def get_generate_hypotheses_prompt_tool(
        session_id: str,
    ...
    async def get_assess_improvement_prompt_tool(
        session_id: str,
        original_detector_code: str,
        improved_detector_code: str,
        evaluation_results_before: Dict[str, Any],
    ...
    async def analyze_evaluation_tool(
        ...
        try:
            ...
            job_response = await _start_prompt_gen_job(
                task_factory, "get_analyze_evaluation_prompt", session_id=session_id
            )
            return job_response
            ...
            return {
                "job_id": None,
                "status": schemas.JobStatus.FAILED.value,
                "error": f"Failed to run analyze_evaluation tool: {e}",
            }
    ...
    async def generate_hypotheses_tool(
        ...
        try:
            logger.info(
                f"仮説生成ツール実行: セッション {session_id}, 仮説数 {num_hypotheses}"
            )
            ...
            # プロンプト生成ジョブを開始
            job_response = await _start_prompt_gen_job(
                task_factory, "get_generate_hypotheses_prompt", session_id=session_id
            )
            return job_response
            ...
            return {
                "job_id": None,
                "status": schemas.JobStatus.FAILED.value,
                "error": f"Failed to run generate_hypotheses tool: {e}",
            }
    ...
    async def suggest_exploration_strategy_tool(session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "job_id": None,
                "status": schemas.JobStatus.FAILED.value,
                "error": f"Failed to run suggest_exploration_strategy tool: {e}",
            }
    ...
    async def assess_improvement_tool(
        ...
        try:
            ...
            # プロンプト生成ジョブを開始
            job_response = await _start_prompt_gen_job(
                task_factory, "get_assess_improvement_prompt", session_id=session_id
            )
            return job_response
            ...
            return {
                "job_id": None,
                "status": schemas.JobStatus.FAILED.value,
                "error": f"Failed to run assess_improvement tool: {e}",
            }
