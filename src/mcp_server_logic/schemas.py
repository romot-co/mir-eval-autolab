...
class ParameterInfo(BaseModel):
    ...
    type: str = Field(
        ..., description="Data type (e.g., 'float', 'int', 'str', 'bool')"
    )
    current_value: Optional[Any] = Field(None, description="Current value if known")
    suggested_range: Optional[List[Union[float, int]]] = Field(
        None, description="Suggested range [min, max] for numerical parameters"
    )
    suggested_values: Optional[List[Any]] = Field(
        None,
        description="Suggested discrete values for categorical or boolean parameters",
    )
    rationale: Optional[str] = Field(
        None, description="Reasoning behind the suggestion"
    )
    ...
    def check_range_length(cls, v):
        if v is not None and len(v) != 2:
            raise ValueError(
                "suggested_range must have exactly two elements [min, max]"
            )
        return v
...
class ParameterSuggestion(BaseModel):
    parameters: List[ParameterInfo] = Field(
        ..., description="List of suggested parameter adjustments"
    )
    overall_rationale: Optional[str] = Field(
        None, description="Overall reasoning for the parameter suggestions"
    )
...
class EvaluationFinding(BaseModel):
    metric: str = Field(
        ...,
        description="The metric being discussed (e.g., 'note.f_measure', 'onset_error_mean')",
    )
    observation: str = Field(
        ..., description="Observation about the metric's value or trend"
    )
    interpretation: Optional[str] = Field(
        None, description="Interpretation of the observation (e.g., potential cause)"
    )
    strength_or_weakness: str = Field(
        ...,
        description="Whether this is considered a strength or weakness ('strength', 'weakness', 'neutral')",
    )
...
class EvaluationAnalysis(BaseModel):
    ...
    weaknesses: List[str] = Field(
        default_factory=list, description="List of identified weaknesses"
    )
    detailed_findings: List[EvaluationFinding] = Field(
        default_factory=list, description="Detailed observations about specific metrics"
    )
    potential_next_steps: List[str] = Field(
        default_factory=list,
        description="Suggested next steps for improvement based on the analysis",
    )
...
class EvaluationResultData(BaseModel):
    ...
    summary: Optional[Dict[str, Any]] = Field(
        None, description="Summarized evaluation results"
    )
    metrics: Optional[Dict[str, Any]] = Field(
        None, description="Detailed metrics dictionary (e.g., from mir_eval)"
    )
    output_dir: Optional[str] = Field(
        None, description="Path to the directory where results were saved on the server"
    )
    results_json_path: Optional[str] = Field(
        None, description="Path to the saved JSON results file on the server"
    )
    plot_path: Optional[str] = Field(
        None, description="Path to the saved plot file on the server"
    )
...
class GridSearchResultData(BaseModel):
    ...
    summary: Optional[str] = Field(
        None, description="Summary of the grid search (e.g., best score and params)"
    )
    best_params: Optional[Dict[str, Any]] = Field(
        None, description="Best parameter set found"
    )
    best_score: Optional[float] = Field(
        None, description="Score achieved with the best parameters"
    )
    best_params_path: Optional[str] = Field(
        None,
        description="Path to the file containing the best parameters on the server",
    )
    results_csv_path: Optional[str] = Field(
        None,
        description="Path to the CSV file containing detailed grid search results on the server",
    )
    output_dir: Optional[str] = Field(
        None,
        description="Path to the directory where grid search results were saved on the server",
    )
    code_version: Optional[str] = None
...
class CodeSaveResultData(BaseModel):
    ...
    version: str = Field(
        ..., description="The generated version tag/hash for the saved code"
    )
    file_path: str = Field(
        ..., description="The path where the code was saved on the server"
    )
...
class ErrorDetails(BaseModel):
    ...
    type: str = Field(
        default="UnknownError", description="エラーの型名 (例: ValueError, LLMError)"
    )
    traceback: Optional[str] = Field(
        None, description="エラー発生時のスタックトレース (存在する場合)"
    )
...
class JobInfo(BaseModel):
    ...
    result: Optional[
        ...
        None,
        description="Job result (decoded JSON), specific schema depends on tool_name",
    )
    error_details: Optional[ErrorDetails] = Field(
        None, description="Job failure details"
    )
...
class SessionInfoResponse(BaseModel):
    ...
    last_error: Optional[ErrorDetails] = Field(
        None,
        description="The error details from the last failed job associated with this session, if any.",
    )


class HypothesisList(BaseModel):
    hypotheses: List[str] = Field(
        ..., description="List of generated improvement hypotheses"
    )
...
class ExplorationStrategySuggestion(BaseModel):
    ...
    action: ExplorationStrategyAction = Field(
        ..., description="提案された次のアクション"
    )
    reasoning: str = Field(..., description="このアクションが提案された理由の説明")
    parameters: Optional[Dict[str, Any]] = Field(
        default_factory=dict,
        description="提案されたアクションに必要なパラメータ (例: improve_code の suggestion)",
    )
    confidence: Optional[float] = Field(
        None, description="提案の信頼度スコア (0.0 から 1.0)", ge=0.0, le=1.0
    )
...
class SessionErrorData(HistoryEventBaseData):
    ...
    triggering_event: Optional[str] = Field(
        None, description="エラーを引き起こした可能性のあるイベントタイプ"
    )
...
class RunEvaluationInput(BaseModel):
    ...
    save_plots: bool = Field(
        default=False, description="Whether to save plots on the server"
    )
    save_results_json: bool = Field(
        default=True, description="Whether to save results JSON on the server"
    )


class RunGridSearchInput(BaseModel):
    grid_config: Dict[str, Any] = Field(
        ..., description="Grid search configuration dictionary"
    )
    skip_existing: bool = Field(
        default=False, description="Whether to skip existing results on the server"
    )
    best_metric: Optional[str] = Field(
        default="note.f_measure", description="Metric to optimize"
    )
    code_version: Optional[str] = None
    session_id: Optional[str] = None
...
class StartSessionInput(BaseModel):
    base_algorithm: str = Field(
        default="Unknown", description="ベースとなるアルゴリズム名"
    )
...
class AddSessionHistoryInput(BaseModel):
    ...
    cycle_state_update: Optional[Dict[str, Any]] = Field(
        None, description="Cycle state を更新するフィールドの辞書"
    )
...
class OptimizeParametersInput(BaseModel):
    ...
    code_version: Optional[str] = Field(
        None, description="最適化対象のコードバージョン (指定なければ最新)"
    )
    audio_dir: Optional[str] = Field(
        None,
        description="評価に使用する音声ファイルディレクトリ (データセット指定のため)",
    )
    reference_dir: Optional[str] = Field(
        None,
        description="評価に使用する参照ラベルディレクトリ (データセット指定のため)",
    )
...
class HypothesisGeneratedData(BaseModel):
    ...
    id: str = Field(
        default_factory=lambda: f"hyp_{generate_id()}", description="仮説の一意なID"
    )  # utils.generate_id が必要
    description: str = Field(..., description="仮説の内容")
    priority: Optional[float] = Field(
        None, description="仮説の優先度 (例: 0.0-1.0)", ge=0.0, le=1.0
    )
    status: str = Field(
        default="pending",
        description="仮説の状態 (pending, testing, verified, rejected)",
    )
...
class SessionCycleState(BaseModel):
    ...
    last_analysis: Optional[Dict[str, Any]] = (
        None  # 最新の分析結果 (EvaluationAnalysis.model_dump())
    )
    last_hypotheses: Optional[List[Dict[str, Any]]] = (
        None  # 最新の仮説リスト (List[HypothesisGeneratedData.model_dump()])
    )
    last_assessment: Optional[Dict[str, Any]] = (
        None  # 最新の改善評価結果 (AssessmentResult.model_dump())
    )
...
class GetImproveCodePromptInput(BaseModel):
    ...
    original_code_version_hash: Optional[str] = Field(
        None, description="元のコードバージョンハッシュ"
    )
...
class GetAssessImprovementPromptInput(BaseModel):
    ...
    evaluation_results_before: Dict[str, Any] = Field(
        ..., description="改善前の評価結果"
    )
    evaluation_results_after: Dict[str, Any] = Field(
        ..., description="改善後の評価結果"
    )
...
class GetDetectorInfoInput(BaseModel):
    ...
    detector_id: Optional[str] = Field(
        None,
        description="検出器ID（指定しない場合は現在のセッションの検出器が使用されます）",
    )
...
class AnalyzeEvaluationInput(BaseModel):
    ...
    user_goal: Optional[str] = Field(
        None, description="ユーザーの目標または課題（オプション）"
    )
