...
from src.utils.misc_utils import (  # format_timestampを追加
    format_timestamp,
    generate_id,
    get_timestamp,
)
from src.utils.path_utils import (  # パス検証用に追加
    get_workspace_dir,
...
async def start_session(
    config: Dict[str, Any], db_path: Path, base_algorithm: str = "Unknown"
...
async def get_session_info(
    config: Dict[str, Any], db_path: Path, session_id: str
) -> schemas.SessionInfoResponse:
    ...
    try:
        ...
        logger.error(
            f"セッション情報取得 DBエラー ({session_id}): {sme}", exc_info=False
        )
        raise sme
    except Exception as e:
        logger.error(
            f"セッション情報取得中に予期せぬエラー ({session_id}): {e}", exc_info=True
        )
        raise StateManagementError(
            f"Unexpected error fetching session info: {e}"
        ) from e
    ...
    if row:
        try:
            ...
            parsed_data["created_at"] = (
                format_timestamp(start_time) if start_time else "N/A"
            )
            parsed_data["updated_at"] = (
                format_timestamp(last_update) if last_update else "N/A"
            )
            ...
            for key in [
                ...
                if json_str:
                    try:
                        ...
                        logger.warning(
                            f"セッション {session_id} の {key} JSON解析失敗: {e}。デフォルト値を使用します。"
                        )
            ...
            parsed_data["current_cycle"] = parsed_data.get("cycle_state", {}).get(
                "cycle_count", 0
            )
            ...
            try:
                ...
                error_row = await db_utils.db_fetch_one_async(
                    db_path, error_sql, (session_id, schemas.JobStatus.FAILED.value)
                )
                if error_row and error_row["error_details"]:
                    try:
                        ...
                        logger.warning(
                            f"Failed to parse last error details for session {session_id}: {err_parse}"
                        )
            except Exception as db_err:
                logger.error(
                    f"Failed to fetch last error for session {session_id}: {db_err}",
                    exc_info=True,
                )
            ...
            if parse_errors:
                logger.warning(
                    f"セッション {session_id} のデータパース中にエラーが発生しました: {parse_errors}"
                )
            ...
            logger.error(
                f"セッション情報処理/パース中に予期せぬエラー ({session_id}): {e}",
                exc_info=True,
            )
            raise StateManagementError(
                f"Unexpected error processing/parsing session info: {e}"
            ) from e
    else:
...
async def add_session_history(
    ...
    try:
        ...
        if schema_class:
            ...
            validated_event_data_dict = validated_event_data_model.model_dump(
                mode="json"
            )  # DB保存用にJSON互換形式でダンプ
            logger.debug(
                f"Validated event data for '{event_type}' using {schema_class.__name__}"
            )
        else:
            # スキーマが見つからない場合、警告ログを出力し、辞書としてそのまま使用 (検証なし)
            logger.warning(
                f"No specific schema found for event_type '{event_type}'. Using raw data dict."
            )
        ...
        if cycle_state_update is not None:
            ...
            for key, value in cycle_state_update.items():
                if key in cycle_state_fields:
                    ...
                    logger.warning(
                        f"Ignoring invalid key '{key}' in cycle_state_update for session {session_id}"
                    )
            if not validated_update_dict:
                validated_update_dict = None  # 有効なキーがなければ None に戻す
            else:
                logger.debug(
                    f"Validated cycle state update keys: {list(validated_update_dict.keys())}"
                )

    except ValidationError as ve:
        logger.error(
            f"History event data validation failed for event '{event_type}' (Session: {session_id}): {ve}",
            exc_info=True,
        )
        ...
        logger.error(
            f"Unexpected error during history data validation (Session: {session_id}): {e_val}",
            exc_info=True,
        )
    ...
    new_event = {
        ...
        "type": event_type,
        "data": validated_event_data_dict,
    }
    ...
    logger.debug(
        f"[add_history] Attempting DB lock for {session_id} (Event: {event_type})..."
    )
    async with db_utils.db_lock:  # ★ DB操作全体をロックで囲む
        ...
        try:
            ...
            async with conn.execute("BEGIN"):  # 手動トランザクション開始
                ...
                if not row:
                    logger.warning(
                        f"[add_history] Session not found inside lock: {session_id}"
                    )
                    raise ValueError(f"セッション {session_id} が見つかりません")
                ...
                try:
                    ...
                    current_cycle_state = schemas.SessionCycleState.model_validate_json(
                        row["cycle_state"] or "{}"
                    )
                    ...
                    current_best_metrics_dict: Optional[Dict] = json.loads(
                        row["best_metrics"] or "null"
                    )
                    if (
                        not isinstance(current_best_metrics_dict, dict)
                        and current_best_metrics_dict is not None
                    ):
                    ...
                    if not isinstance(session_config_dict, dict):
                        session_config_dict = {}
                    improvement_threshold = session_config_dict.get(
                        "improvement_threshold", DEFAULT_IMPROVEMENT_THRESHOLD
                    )
                    # ★ 追加: 履歴最大数を設定から読み込み (デフォルト値も設定)
                    max_history_events = config.get("limits", {}).get(
                        "max_history_events_per_session", 1000
                    )
                    ...
                    logger.error(
                        f"セッション {session_id} の状態ロード中にエラー: {load_err}",
                        exc_info=True,
                    )
                    raise StateManagementError(
                        f"Error loading session state: {load_err}"
                    ) from load_err
                ...
                current_cycle_state_dict = (
                    current_cycle_state.model_dump()
                )  # 更新用に辞書に戻す

                # Determine the primary metric for comparison
                primary_metric_path = config.get("evaluation", {}).get(
                    "primary_metric", "overall.f_measure"
                )
                primary_metric_keys = primary_metric_path.split(
                    "."
                )  # e.g., ['overall', 'f_measure']
                ...
                if event_type == "evaluation_complete":
                    ...
                    new_primary_metric_value = get_metric_value(
                        new_metrics, primary_metric_keys
                    )
                    ...
                    if new_primary_metric_value is not None:
                        current_best_f = get_metric_value(
                            current_best_metrics_dict or {}, primary_metric_keys
                        )
                        logger.info(
                            f"Session {session_id}: New {primary_metric_path}={new_primary_metric_value:.4f}, Best={current_best_f or 'N/A'}"
                        )

                        if current_best_f is None or new_primary_metric_value > (
                            current_best_f + improvement_threshold
                        ):
                            logger.info(
                                f"Session {session_id}: Primary metric improved! Updating best metrics and resetting stagnation."
                            )
                            updated_best_metrics_dict = validated_event_data_dict.get(
                                "metrics", {}
                            )
                            updated_best_metrics_json = json.dumps(
                                updated_best_metrics_dict, cls=NumpyEncoder
                            )
                            ...
                            state_updated = True
                            log_msg_suffix += (
                                f", ベストスコア更新 ({primary_metric_path})"
                            )
                        else:
                            logger.info(
                                f"Session {session_id}: Primary metric did not improve significantly. Incrementing stagnation."
                            )
                            current_cycle_state.stagnation_count += 1
                            state_updated = True
                    else:
                        logger.warning(
                            f"Session {session_id}: Evaluation complete event missing primary metric '{primary_metric_path}' in results. Cannot update stagnation."
                        )
                ...
                # ★ 失敗イベントでの停滞カウント更新を有効化 ★
                    ...
                    in [
                        ...
                        "cycle_failed",
                        "improve_code_failed",
                        "analyze_evaluation_failed",
                        "parameter_suggestion_failed",
                    ]
                    or event_type.endswith("_failed")
                    or event_type == "cycle_error"
                ):
                    logger.warning(
                        f"Session {session_id}: Event '{event_type}' occurred. Incrementing stagnation count."
                    )
                    current_cycle_state.stagnation_count += 1
                    state_updated = True
                ...
                if validated_update_dict:
                    logger.debug(
                        f"Applying cycle_state_update: {validated_update_dict}"
                    )
                    try:
                        # ★ update_forward_refs は Pydantic v2 では不要 or 別の方法?
                        #    update 辞書を使ってモデルを更新する (フィールドが存在するかは検証済み)
                        updated_state = current_cycle_state.model_copy(
                            update=validated_update_dict
                        )
                        ...
                        if (
                            "stagnation_count" in validated_update_dict
                            and best_result_updated
                        ):
                            logger.debug(
                                "Stagnation reset by both evaluation improvement and external update. Using external update."
                            )
                            # The model_copy already applied the external value.

                        current_cycle_state = updated_state  # 更新されたモデルを代入
                        state_updated = True
                        if "cycle_state 更新" not in log_msg_suffix:
                            log_msg_suffix += ", cycle_state 更新"
                    except Exception as update_err:
                        # 通常は発生しないはず (キー検証済みのため)
                        logger.warning(
                            f"Failed to apply cycle_state_update {validated_update_dict} to model: {update_err}"
                        )
                ...
                if len(current_history) > max_history_events:
                    ...
                    logger.warning(
                        f"Session {session_id} history exceeds limit ({max_history_events}). Removing {num_to_remove} oldest events."
                    )
                ...
                updated_cycle_state_json = (
                    current_cycle_state.model_dump_json()
                    if state_updated
                    else row["cycle_state"]
                )
            ...
            if conn:
                await conn.rollback()
            logger.warning(
                f"[add_history] Rolling back transaction for {session_id} due to: {known_err}"
            )
            raise known_err
        except (
            aiosqlite.Error
        ) as db_err:  # aiosqlite.Error をキャッチ (OperationalError を含む)
            # DBロックなどの可能性
            if conn:
                await conn.rollback()
            logger.error(
                f"Database error updating history for {session_id}: {db_err}",
                exc_info=True,
            )
            ...
            if conn:
                await conn.rollback()
            logger.error(
                f"Unexpected database error updating history for {session_id}: {e_db}",
                exc_info=True,
            )
            raise StateManagementError(
                f"Unexpected DB error during history update: {e_db}"
            ) from e_db
        finally:
            if conn:
                await conn.close()
...
async def cleanup_old_sessions(config: Dict[str, Any]):
    ...
    session_timeout_seconds = config.get("cleanup", {}).get(
    ...
    if session_timeout_seconds <= 0 and max_sessions_count <= 0:
        logger.info(
            "セッションのタイムアウトと最大数が無効なため、クリーンアップをスキップします。"
        )
        return

    logger.info(
        f"古いセッションのクリーンアップを開始 (タイムアウト: {session_timeout_seconds}秒, 最大: {max_sessions_count}件)"
    )
    ...
    try:
        ...
        if session_timeout_seconds > 0:
            ...
            # 非同期関数を使用
            deleted_timeout_count_result = await db_utils.db_execute_commit_async(
                db_path,
            ...
            logger.info(
                f"タイムアウトしたセッション削除試行完了 (last_update < {timeout_threshold})。"
            )  # 削除件数は正確には取れない

        # 最大数を超えたセッションを削除
        if max_sessions_count > 0:
            # 非同期関数を使用
            row = await db_utils.db_fetch_one_async(
                db_path, "SELECT COUNT(*) FROM sessions"
            )
            ...
            if current_session_count > max_sessions_count:
                ...
                logger.info(
                    f"セッション数が最大保持数 {max_sessions_count} を超過 ({current_session_count} 件)。古い {sessions_to_delete_count} 件を削除します。"
                )
                # 非同期関数を使用
                # execute_commit_async は削除件数を返さないので、ログのみ
                await db_utils.db_execute_commit_async(
                    db_path,
                    "DELETE FROM sessions WHERE session_id IN (SELECT session_id FROM sessions ORDER BY last_update ASC LIMIT ?)",
                    (sessions_to_delete_count,),
                )
                logger.info(
                    f"{sessions_to_delete_count} 件の超過セッション削除試行完了。"
                )
            else:
                logger.info(
                    f"現在のセッション数: {current_session_count} (最大: {max_sessions_count}) - 超過削除は不要。"
                )
...
async def get_session_summary_for_prompt(
    ...
    try:
        ...
        logger.debug(
            f"Generated history summary for session {session_id} (length: {len(summary)})"
        )
        return summary

    except Exception as e:
        logger.error(
            f"Error summarizing history for session {session_id}: {e}", exc_info=True
        )
...
async def update_session_status_internal(
    ...
    try:
        ...
        logger.info(
            f"セッション {session_id}: ステータスを '{new_status}' に更新しました"
        )
        ...
        logger.error(
            f"Unexpected error updating session status for {session_id}: {e}",
            exc_info=True,
        )
        raise StateManagementError(
            f"Unexpected error updating session status: {e}"
        ) from e
