...
def _run_evaluate(
    ...
    **kwargs
    ...
    logger.info(f"[Job {job_id}] CORE: Running evaluation for '{detector_name}'... Session: {session_id}")
    ...
    if 'output_dir' in kwargs and kwargs['output_dir']:
        ...
        logger.warning(f"[Job {job_id}] CORE: Output directory not provided, using default within evaluation_results_dir.")
    ...
    if output_dir_path:
        try:
            ...
            logger.error(f"[Job {job_id}] CORE: Failed to ensure output directory {output_dir_path}: {e}", exc_info=True)
            raise EvaluationError(f"Failed to ensure output directory: {e}") from e
    else:
         logger.error(f"[Job {job_id}] CORE: Output directory is invalid or could not be determined.")
    ...
    try:
        ...
        logger.error(f"[Job {job_id}] CORE: Error during evaluation for '{detector_name}': {e}", exc_info=True)
...
def _execute_grid_search(
    ...
    **kwargs
    ...
    session_id = kwargs.get('session_id')
    detector_name = kwargs.get('grid_config',{}).get('detector_name', 'unknown') # grid_configから取得
    logger.info(f"[Job {job_id}] CORE: Running grid search for '{detector_name}'... Session: {session_id}")
    ...
    if 'output_dir' in kwargs and kwargs['output_dir']:
        ...
        logger.warning(f"[Job {job_id}] CORE: Grid search output directory not provided, using default within grid_search_results_dir.")
    ...
    if output_dir_path:
        try:
             ...
             logger.error(f"[Job {job_id}] CORE: Failed to ensure grid search output directory {output_dir_path}: {e}", exc_info=True)
             raise GridSearchError(f"Failed to ensure output directory: {e}") from e
    else:
        logger.error(f"[Job {job_id}] CORE: Grid search output directory is invalid or could not be determined.")
        raise GridSearchError("Grid search output directory is invalid or could not be determined.")

    # 履歴追加は削除

    try:
        results = run_grid_search_core(**kwargs)
        logger.info(f"[Job {job_id}] CORE: Grid search finished. Output: {kwargs['output_dir']}")
        ...
        logger.error(f"[Job {job_id}] CORE: Error during grid search: {e}", exc_info=True)
...
async def _run_evaluate_async(
        ...
        if dataset_name:
            try:
                ...
                allowed_dirs.extend([audio_d, label_d])
                 ...
                 logger.warning(f"[Job {job_id}] Could not get dataset paths ('{dataset_name}') for validation: {e}")
        # audio_path/ref_path が指定されている場合は検証
        if audio_path:
            verified_audio_path = str(validate_path_within_allowed_dirs(audio_path, allowed_dirs, check_existence=True, check_is_file=True))
        if ref_path:
            verified_ref_path = str(validate_path_within_allowed_dirs(ref_path, allowed_dirs, check_existence=True, check_is_file=True))
    ...
    if session_id:
        try:
             ...
             await add_history_async_func(session_id, "evaluation_started", start_event_data.model_dump())
    ...
    try:
        ...
        core_kwargs = {
            ...
            "detector_params": {detector_name: detector_params} if detector_params and detector_name else None,
        ...
        if session_id:
            try:
                ...
                await add_history_async_func(session_id, "evaluation_complete", complete_event_data.model_dump())
            except Exception as hist_e:
                 logger.warning(f"[Job {job_id}] Failed to add evaluation completed history: {hist_e}")
        ...
        job_result = EvaluationResultData(
            ...
            results_json_path=results_dict.get('results_json_path'), # コア関数が返す想定
            plot_path=results_dict.get('plot_path') # コア関数が返す想定
        )
        return job_result

    except Exception as e_core:
        logger.error(f"[Job {job_id}] Error executing evaluation core function: {e_core}", exc_info=True)
        ...
        if error_occurred and session_id:
            try:
                ...
                await add_history_async_func(session_id, "evaluation_failed", fail_event_data.model_dump())
            except Exception as hist_e:
...
async def _execute_grid_search_async(
    ...
    logger.info(f"[Job {job_id}] ASYNC WRAPPER: Running grid search for '{detector_name}'... Session: {session_id}")
    ...
    try:
        if user_output_dir:
            logger.info(f"[Job {job_id}] User specified grid search output directory: {user_output_dir}")
            final_output_dir = validate_path_within_allowed_dirs(
                user_output_dir,
                allowed_base_dirs=[output_base_dir],
                check_existence=False,
            ...
            logger.info(f"[Job {job_id}] Using validated user-specified grid search output directory: {final_output_dir}")
        else:
            logger.info(f"[Job {job_id}] No grid search output directory specified, generating automatically.")
            unique_suffix = f"gridsearch_{session_id or 'nosession'}_{job_id}"
            final_output_dir = get_output_dir(output_base_dir, unique_suffix=unique_suffix)
            logger.info(f"[Job {job_id}] Generated grid search output directory: {final_output_dir}")
        # ディレクトリ作成
        ensure_dir(final_output_dir)
        final_output_dir_str = str(final_output_dir)
    except (ValueError, FileError, TypeError, ConfigError) as e:
    ...
    if session_id:
        try:
            ...
            await add_history_async_func(session_id, "grid_search_started", start_event_data.model_dump())
        except Exception as hist_e:
    ...
    try:
        ...
        if session_id:
            try:
                ...
                await add_history_async_func(session_id, "grid_search_complete", complete_event_data.model_dump())
            except Exception as hist_e:
                 logger.warning(f"[Job {job_id}] Failed to add grid search completed history: {hist_e}")
        ...
        logger.error(f"[Job {job_id}] Error executing grid search core function: {e_core}", exc_info=True)
        ...
        if error_occurred and session_id:
            try:
                ...
                await add_history_async_func(session_id, "grid_search_failed", fail_event_data.model_dump())
            except Exception as hist_e:
...
def register_evaluation_tools(
    mcp: FastMCP,
    config: Dict[str, Any],
    start_async_job_func: Callable[..., Coroutine[Any, Any, Dict[str, str]]], # 戻り値修正: JobStartResponse辞書
    ...
    async def run_evaluation_tool(
        ...
        kwargs = {
            ...
            "num_procs": config.get('resource_limits', {}).get('evaluation_num_procs'), # configから取得
            "session_id": session_id,
            "code_version": code_version,
    ...
    async def run_grid_search_tool(
        ...
        kwargs = {
            "grid_config": grid_config,
            "num_procs": config.get('resource_limits', {}).get('grid_search_num_procs'), # configから取得
            "skip_existing": skip_existing,
            "best_metric": best_metric,
            "session_id": session_id,
            "code_version": code_version,
