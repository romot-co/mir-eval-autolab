...
from src.utils.exception_utils import (  # ConfigError を追加
    ConfigError,
    FileError,
    StateManagementError,
)
from src.utils.json_utils import (  # JsonNumpyEncoder をインポート (旧 JsonNumpyEncoder)
    NumpyEncoder,
)
from src.utils.misc_utils import generate_id, get_timestamp  # 変更後
from src.utils.path_utils import (  # validate_path_within_allowed_dirs を追加
    ensure_dir,
    get_db_dir,
...
DEFAULT_CONFIG = {
    ...
    "evaluation": {
        ...
        "mir_eval_options": {"onset_tolerance": 0.05, "pitch_tolerance": 50.0},
    },
    "grid_search": {"default_best_metric": "note.f_measure", "default_n_jobs": -1},
    "llm": {
        ...
        for key, default_value in cfg_dict.items():
            ...
            if env_value is not None:
                ...
                try:
                    ...
                    # ネストされた辞書構造を再現
                    keys = (
                        current_prefix.replace(prefix + separator, "")
                        .lower()
                        .split(separator)
                    )
                    if keys == [""]:
                    ...
                    logger.warning(
                        f"環境変数 '{env_var_name}' の値 '{env_value}' を型 '{original_type.__name__}' に変換できませんでした: {e}"
                    )
...
def load_config(config_path_str: Optional[str] = None) -> Dict[str, Any]:
    ...
    if config_path_str:
        ...
        if config_path.exists():
            ...
            logger.warning(
                f"指定された設定ファイルが見つかりません: {config_path}。デフォルトを試します。"
            )
    ...
    if not config_path_to_load:
        ...
        if default_yaml_path.exists():
            ...
            logger.info(
                "デフォルト設定ファイル config.yaml が見つかりません。デフォルト値と環境変数のみ使用します。"
            )
    ...
    if config_path_to_load:
        try:
            ...
            yaml_loaded = True
            logger.info(
                f"設定ファイル {config_path_to_load} を読み込み、デフォルト値を上書きしました。"
            )
            ...
            msg = (
                f"設定ファイル {config_path_to_load} の読み込み中に予期せぬエラー: {e}"
            )
    ...
    try:
        ...
        for key, relative_path_str in path_cfg.items():
            if not isinstance(relative_path_str, str):
                logger.warning(
                    f"パス設定 '.paths.{key}' の値が文字列ではありません: {relative_path_str}。スキップします。"
                )
                continue
            ...
            try:
                ...
                raise ConfigError(
                    f"パス設定 '.paths.{key}' ('{relative_path_str}') がワークスペース外または不正です: {path_err}"
                ) from path_err
            ...
            try:
                ...
                # ディレクトリ作成失敗などのエラー
                raise ConfigError(
                    f"パス設定 '.paths.{key}' ('{absolute_path}') の準備に失敗しました: {fe}"
                ) from fe
        ...
        logger.critical(
            f"設定ファイルの読み込み/パス解決中に予期せぬエラー: {e}", exc_info=True
        )
    ...
    if env_max_jobs:
        if env_max_jobs.lower() == "auto":
            ...
            try:
                ...
                logger.warning(
                    f"環境変数 MCP_MAX_CONCURRENT_JOBS の値 '{env_max_jobs}' は無効です。自動設定 ({default_jobs}) を使用します。"
                )
                limits["max_concurrent_jobs"] = default_jobs
    else:
        limits["max_concurrent_jobs"] = default_jobs
...
async def cleanup_workspace_files(config: Dict[str, Any]):
    ...
    if retention_days <= 0:
        logger.warning(
            "Workspace cleanup retention_days is not positive, skipping cleanup."
        )
        return

    cutoff_time = datetime.now() - timedelta(days=retention_days)
    logger.info(
        f"Running workspace file cleanup. Deleting items older than {cutoff_time} ({retention_days} days)..."
    )
    ...
    try:
        ...
        logger.error(
            f"クリーンアップ対象のベースディレクトリ取得に失敗しました: {e}. クリーンアップを中止します。"
        )
        return
    ...
    for base_dir in target_base_dirs:
        if not base_dir.is_dir():
            logger.warning(
                f"クリーンアップ対象のベースディレクトリが存在しません: {base_dir}"
            )
            continue
            ...
            try:
                ...
                if last_access_time < cutoff_time:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error cleaning up item {item}: {e}", exc_info=True
                        )
                        error_count += 1
    ...
    logger.info(
        f"Workspace cleanup finished. Deleted {deleted_count} old directories. Encountered {error_count} errors."
    )
    ...
    async def cleanup_run():
        while True:
            try:
                ...
                logger.info(
                    f"定期クリーンアップ完了 (所要時間: {duration:.2f}秒)。次の実行は {interval}秒後。"
                )
