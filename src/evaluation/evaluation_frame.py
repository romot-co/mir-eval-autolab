...
def evaluate_frame_pitches(
    ...
    if (len(ref_times) == 0 and len(ref_freqs) == 0) and (
        len(est_times) == 0 and len(est_freqs) == 0
    ):
        return {
            "voicing_recall": 1.0,
    ...
    if len(ref_times) != len(ref_freqs):
        logger.warning(
            f"参照データの時刻数とピッチ数が一致しません: {len(ref_times)} vs {len(ref_freqs)}"
        )
        return {
            "voicing_recall": 0.0,
    ...
    if len(est_times) != len(est_freqs):
        logger.warning(
            f"推定データの時刻数とピッチ数が一致しません: {len(est_times)} vs {len(est_freqs)}"
        )
        return {
            "voicing_recall": 0.0,
            "voicing_false_alarm": 1.0,
    ...
    try:
        ...
        raw_pitch_accuracy = mir_eval.melody.raw_pitch_accuracy(
            ref_voiced, ref_cents, est_voiced, est_cents, cent_tolerance=pitch_tolerance
        )
